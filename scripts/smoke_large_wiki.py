#!/usr/bin/env python3
"""Exercise Smash's query and graph path against a synthetic large wiki."""
...
def run_smoke(work_dir: Path, page_count: int, max_seconds: dict[str, float] | None = None) -> dict[str, object]:
    ...
    payload = {
        ...
        "wiki": str(wiki),
        "viewer": {
            "serve_command": f"python3 smash.py serve {work_dir}",
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Smoke test Smash against a synthetic large wiki.")
    ...
    work_dir = Path(args.work_dir).expanduser().resolve() if args.work_dir else Path(tempfile.mkdtemp(prefix="Smash-large-wiki-"))
    max_seconds = {
