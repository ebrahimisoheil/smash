...
AGENT_CONTRACT_REQUIREMENTS = {
    ...
    Path("integrations/_shared/smash-instructions.md"): (
        "status",
        "recall",
        "remember",
        "ingest",
    ...
    Path("integrations/_shared/smash-instructions-project.md"): (
        "status",
        "recall",
        "remember",
        "ingest",
...
def check_version_values(
    ...
    if not package_versions:
        findings.append("version mismatch: server.json has no smash-mcp package version")
...
def check_version_consistency(findings: list[str]) -> str | None:
    ...
    init_version = read_init_version(Path("mcp_package/smash_mcp/__init__.py"))
    core_version = read_core_version(Path("mcp_package/smash_core/version.py"))
    ...
    versions = {
        ...
        "mcp_package/smash_mcp/__init__.py": init_version,
        "mcp_package/smash_core/version.py": core_version,
...
def check_outbound_network_hygiene(findings: list[str], path: Path, text: str) -> None:
    """Block accidental outbound network code in Smash's local-first runtime."""
    rel = path.as_posix()
