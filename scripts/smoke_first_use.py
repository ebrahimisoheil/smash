#!/usr/bin/env python3
"""Exercise Smash's first-use path the way a new local user would."""
...
def run_link(*args: str, python: str = sys.executable) -> subprocess.CompletedProcess[str]:
    ...
    if result.returncode != 0:
        command = " ".join([python, "smash.py", *args])
        raise SmokeFailure(
...
def run_smoke(work_dir: Path, python: str = sys.executable) -> None:
    ...
    init_target = work_dir / "new-user-Smash"
    demo_target = work_dir / "demo-Smash"

    run_link("init", str(init_target), python=python)
    require((init_target / "smash.py").exists(), "init did not copy smash.py")
    ...
    require(
        init_prompts.get("prompts", [{}])[0].get("prompt") == "is Smash ready?",
    ...
    require(
        init_welcome.get("steps", [{}])[0].get("prompt") == "is Smash ready?",
    ...
    require(
        "query 'why does Smash help agents?'" in demo_result.stdout
        or 'query "why does Smash help agents?"' in demo_result.stdout,
    ...
    require("query Smash for why Smash helps agents" in start_here, "START_HERE.md did not include agent prompt")
    require("python3 smash.py query" in start_here, "START_HERE.md did not include CLI proof command")
    ...
    require("seed this project into Smash" in prompt_texts, "project prompts did not include seed guidance")
    require(
        any("this project uses Smash" in prompt for prompt in prompt_texts),
    ...
    require((demo_target / ".smash-backups" / str(backup["name"])).exists(), "backup archive is missing")

    query = run_json("query", "what is Smash agent memory?", str(demo_target), "--budget", "small", "--json", python=python)
    ...
    brief = run_json("brief", "testing Smash as local personal memory", str(demo_target), "--json", python=python)
    ...
    remembered = run_json(
        "remember",
        "User is testing Smash first-use smoke as local personal memory for agents.",
        str(demo_target),
        "--title",
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Smoke test Smash's first-use local workflow.")
    parser.add_argument("--work-dir", default="", help="directory for temporary smoke artifacts")
    parser.add_argument("--python", default=sys.executable, help="Python executable used to run smash.py")
    args = parser.parse_args()

    work_dir = Path(args.work_dir).expanduser().resolve() if args.work_dir else Path(tempfile.mkdtemp(prefix="Smash-first-use-"))
    try:
