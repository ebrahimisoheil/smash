"""Event stream for the Smash memory-hygiene benchmark.
...
- brief_echo:  Smash's own injected session brief appears in the transcript
...
ECHO_TEMPLATES = (
    ...
    "As Smash memory says, {claim} Noted again for this session.",
...
def build_event_stream() -> list[dict[str, str]]:
    ...
    for position, (name, _domain, title, _tldr, body, _queries) in enumerate(intents):
        ...
        if position % 2 == 0:
            ...
            day += 1
        # Smash's own brief shows up in transcripts periodically.
        if position % 3 == 0:
            events.append({
