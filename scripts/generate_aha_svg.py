"""Generate docs/assets/smash-aha.svg — the original recall-by-meaning demo.
...
OUT = Path(__file__).resolve().parents[1] / "docs/assets/smash-aha.svg"
T = 14.0
...
LINES = [
    (30, 78,  "cmd",  INK,   '$ smash remember "feat/short-topic branch names"',    0.5,  True,  6.1,  6.48),
    (30, 106, "out",  GREEN, "✓ saved to local memory",                           1.92, False, 6.1,  6.48),
    (30, 150, "cmd",  INK,   '$ smash recall "how should I name my git branches"',  2.5,  True,  6.1,  6.48),
    ...
    (30, 140, "out",  RUST,  "Smash memory · injected automatically",              8.22, False, 13.2, 13.58),
...
def main() -> None:
    ...
    svg = render_svg(
        title="smash · local agent memory",
        aria=(
            "Smash demo: recall finds a memory phrased in different words, and a "
