#!/usr/bin/env python3
"""Smoke test the generated Smash local HTTP viewer over a real localhost socket."""
...
def create_demo(target: Path, python: str) -> None:
    result = subprocess.run(
        [python, str(ROOT / "smash.py"), "demo", str(target), "--force"],
        cwd=ROOT,
...
def run_smoke(work_dir: Path, python: str) -> None:
    ...
    try:
        ...
        require("Smash" in html and "agent memory" in html, "home page did not render Smash shell")
        require(headers.get("X-Smash-API-Version") == "1", "home page missing API version header")
        ...
        require(
            "smash operations" in health_html or "smash.py operations" in health_html,
        ...
        require("X-Smash-Local-Action" in str(denied.get("error", "")), "mutation guard error was not actionable")
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Smoke test Smash's generated local HTTP viewer.")
    parser.add_argument("--work-dir", default="", help="directory for temporary smoke artifacts")
    parser.add_argument("--python", default=sys.executable, help="Python executable used to run Smash")
    args = parser.parse_args()
    work_dir = Path(args.work_dir).expanduser().resolve() if args.work_dir else Path(tempfile.mkdtemp(prefix="Smash-http-viewer-"))
    try:
