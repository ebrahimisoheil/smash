#!/usr/bin/env python3
"""Check that Smash retrieves the right memory with tiny token budgets."""
...
from smash_core.memory import memory_records, recall_memories  # noqa: E402
from smash_core.query import query_link  # noqa: E402
from smash_core.wiki import build_backlinks, build_wiki_cache, close_wiki_cache  # noqa: E402
...
def main() -> int:
    root = Path(tempfile.mkdtemp(prefix="Smash-recall-quality-"))
    wiki = build_quality_wiki(root)
    cache = build_wiki_cache(wiki)
    try:
        records = memory_records(wiki)
        direct = recall_memories(records, "auth setup", limit=2, project="Smash")
        require(direct and direct[0]["name"] == "prefer-oauth-login", "auth setup did not recall OAuth login memory first")
        packet = query_link(wiki, "auth setup", cache, records, budget="micro", project="Smash")
        ...
        release = query_link(wiki, "publishing version", cache, records, budget="micro", project="Smash")
