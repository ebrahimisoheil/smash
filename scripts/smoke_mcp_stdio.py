#!/usr/bin/env python3
"""Smoke test a real Smash MCP stdio server with the MCP client SDK."""
...
async def _assert_prompt_and_resource_contract(session: Any) -> None:
    ...
    resource = await session.read_resource(AnyUrl("smash://health"))
    resource_text = getattr(resource.contents[0], "text", "") if resource.contents else ""
    try:
        health = json.loads(resource_text)
    except json.JSONDecodeError as exc:
        raise RuntimeError(f"smash://health returned invalid JSON: {exc}") from exc
    if health.get("ready") is not True:
        raise RuntimeError("smash://health did not report a ready demo wiki")
    instructions = await session.read_resource(AnyUrl("smash://instructions"))
    ...
    if "recall(query=\"\", mode=\"brief\"" not in instructions_text or "Never silently save durable memory" not in instructions_text:
        raise RuntimeError("smash://instructions did not render the portable agent loop")
...
async def _run_full_smoke(session: Any) -> None:
    ...
    if prompts.get("project") != "mcp-smoke" or prompts.get("prompts", [{}])[0].get("prompt") != "is Smash ready?":
...
async def _run_smoke(wiki_dir: Path, python: str, surface: str) -> None:
    ...
    server_args = ["-m", "smash_mcp", "--wiki", str(wiki_dir)]
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Smoke test Smash MCP over stdio.")
    parser.add_argument("wiki", help="path to a Smash wiki directory")
    parser.add_argument("--python", default=sys.executable, help="Python executable used to run -m smash_mcp")
    ...
    if not (wiki_dir / "index.md").exists():
        print(f"MCP smoke failed: {wiki_dir} does not look like a Smash wiki", file=sys.stderr)
        return 1

    try:
        asyncio.run(_run_smoke(wiki_dir, args.python, args.surface))
    except ModuleNotFoundError as exc:
        missing = exc.name or "dependency"
        print(f"MCP smoke failed: missing Python dependency: {missing}", file=sys.stderr)
        print(
            "MCP smoke failed: source checkouts need the MCP package dependencies first; "
            "run `python3 -m pip install -e ./mcp_package` in your active venv or use the "
            "Python runtime from `smash verify-mcp`.",
            file=sys.stderr,
        )
        return 1
    except BaseException as exc:  # noqa: BLE001 - surface the real cause
        # The MCP session may wrap handler failures in nested TaskGroups, so a
        # plain str(exc) can hide the actual leaf error. Unwrap it for CI.
