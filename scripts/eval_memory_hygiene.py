#!/usr/bin/env python3
"""Smash memory-hygiene benchmark: memory quality over simulated months.
...
- gated:   Smash's real pipeline. Extraction drops Smash-injected output,
           ...
           lineage). Where Smash's conflict detector misses a contradiction,
...
from smash_core.agent_hooks import SMASH_ECHO_MARKERS  # noqa: E402
...
def run_pipeline(gated: bool, events: list[dict[str, str]], wiki: Path) -> dict[str, object]:
    ...
    for event in events:
        text = event["text"]
        # Layer 1 (gated only): drop Smash's own injected output.
        if gated and any(marker in text for marker in SMASH_ECHO_MARKERS):
            continue
