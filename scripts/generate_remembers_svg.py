"""Generate docs/assets/smash-remembers.svg — the automatic-memory loop.
...
OUT = Path(__file__).resolve().parents[1] / "docs/assets/smash-remembers.svg"
T = 26.0
...
LINES = [
    ...
    (1, 94,  "note", RUST,  "◆ Smash session-start · memory brief injected (empty, day one)", 1.2,  False),
    ...
    (1, 184, "note", RUST,  "◆ Smash session-end · captured 1 proposal for your review",      6.0,  False),
    (1, 210, "out",  AMBER, 'pending: "only push to develop, never straight to main"',       7.2,  False),
    (2, 78,  "cmd",  INK,   "$ smash review-memory only-push-to-develop",                      11.4, True),
    ...
    (3, 94,  "note", RUST,  "◆ Smash session-start · 1 memory injected",                      17.4, False),
...
def main() -> None:
    ...
    svg = render_svg(
        title="smash · Smash remembers",
        aria=(
            "Smash demo: a preference said once in an agent session is captured "
