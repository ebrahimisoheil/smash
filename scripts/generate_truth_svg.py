"""Generate docs/assets/smash-truth.svg — the 1.7 sibling of smash-aha.svg.

Scene 1: a new memory conflicts with an old one; Smash refuses to pile up
...
OUT = Path(__file__).resolve().parents[1] / "docs/assets/smash-truth.svg"
T = 18.0
...
LINES = [
    (1, 66,  "cmd",  INK,   '$ smash remember "we deploy from main every Friday"',            0.5,  True),
    (1, 92,  "out",  GREEN, "✓ saved to local memory",                                      2.0,  False),
    (1, 124, "dim",  DIM,   "— weeks later —",                                              2.8,  False),
    (1, 152, "cmd",  INK,   '$ smash remember "no longer Fridays - deploys ship Tuesdays"',   3.5,  True),
    ...
    (1, 234, "cmd",  INK,   "$ smash remember … --supersedes deploy-from-main-every-friday",  6.8,  True),
    (1, 262, "out",  GREEN, "✓ saved · old memory archived with lineage",                   8.6,  False),
    (2, 78,  "cmd",  INK,   '$ smash recall "when do we deploy"',                             10.6, True),
    (2, 106, "out",  GREEN, "✓ no longer Fridays - deploys ship Tuesdays",                  11.9, False),
    (2, 130, "note", RUST,  "→ only the current truth",                                     12.5, False),
    (2, 176, "cmd",  INK,   '$ smash recall "when do we deploy" --as-of 2026-05-01',          13.4, True),
...
def main() -> None:
    ...
    svg = render_svg(
        title="smash · memory that stays true",
        aria=(
            "Smash demo: a new memory conflicts with an old one; Smash replaces it "
