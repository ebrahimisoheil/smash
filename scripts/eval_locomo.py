#!/usr/bin/env python3
"""Third-party retrieval track: LoCoMo evidence retrieval with Smash recall.
...
- every dialog turn of a conversation becomes one Smash memory record;
- every evidence-annotated question becomes a recall query;
- we measure whether Smash's ranking returns the annotated evidence turns
    ...
    if args.mode == "real":
        ...
        if embedder is None:
            print(
                ...
                "`smash semantic --setup` (or `python3 -m smash_mcp --semantic-setup`) first.",
                file=sys.stderr,
            )
            return 2
