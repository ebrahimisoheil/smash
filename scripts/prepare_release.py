#!/usr/bin/env python3
"""Prepare local files for a Smash MCP release.
...
def release_files(root: Path = ROOT) -> ReleaseFiles:
    return ReleaseFiles(
        ...
        init=root / "mcp_package/smash_mcp/__init__.py",
        core_version=root / "mcp_package/smash_core/version.py",
...
def read_core_version(path: Path) -> str:
    match = re.search(r'^SMASH_VERSION\s*=\s*"([^"]+)"', path.read_text(encoding="utf-8"), flags=re.MULTILINE)
    if not match:
        raise ValueError(f"could not read SMASH_VERSION from {path}")
    return match.group(1)
...
def read_server_versions(path: Path) -> set[str]:
    ...
    versions.update(
        ...
        if package.get("identifier") == "smash-mcp"
    )
    return versions
...
def update_core_version(text: str, version: str) -> str:
    return replace_one(r'^SMASH_VERSION\s*=\s*"[^"]+"', f'SMASH_VERSION = "{version}"', text, "SMASH_VERSION")
...
HOMEPAGE_BANNER_RE = re.compile(r"(New \\u00b7 Smash )\d+\.\d+\.\d+( is out)")
...
def release_commands(version: str) -> list[str]:
    ...
    return [
        ...
        f"TWINE_USERNAME=__token__ python3 -m twine upload dist/smash_mcp-{version}*",
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Prepare Smash MCP release files.")
    ...
    print("")
    print(
        "Then bump the Homebrew tap (ebrahimisoheil/homebrew-Smash) to "
        f"{normalize_version(args.version)} so `brew install` serves this "
        "version — otherwise new users get an older Smash than the docs describe."
    )
    return 0
