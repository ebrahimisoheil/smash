#!/usr/bin/env python3
"""Benchmark Smash memory recall quality: lexical vs hybrid (semantic) recall.
...
- --mode real  the actual local model (pip install "smash-mcp[semantic]";
...
from smash_core.semantic import load_embedder, semantic_memory_scores  # noqa: E402
...
def main() -> int:
    ...
    if args.mode == "fake":
        ...
        if embedder is None:
            print(
                "Real model unavailable. Install with: pip install \"smash-mcp[semantic]\" "
                "and cache the model via `smash semantic --setup` (or pass --allow-download).",
                file=sys.stderr,
            )
            return 2
    ...
    if args.json:
        ...
        print(
            f"Smash recall benchmark — suite: {args.suite}, mode: {args.mode}, "
