...
impl MapStore {
    ...
    fn publish(
        ...
        id: MapVersionId,
        reviewer: &str,
    ) -> Result<MapVersionId, MapReviewError> {
        let draft = self
            .drafts
            .get(&id)
            .cloned()
            .ok_or(MapReviewError::NotFound)?;
...
mod tests {
    ...
    fn draft_never_publishes_on_capture() {
        ...
        let draft = store
            .propose_draft(
                MapVersionId::new_v7(),
                input("agent", MapPublicationPolicy::PersonalArea),
            )
            .unwrap();
    ...
    fn personal_self_approval_and_shared_independent_review_are_distinct() {
        ...
        let shared = store
            .propose_draft(
                MapVersionId::new_v7(),
                input("agent", MapPublicationPolicy::SharedArea),
            )
            .unwrap();
        assert_eq!(
