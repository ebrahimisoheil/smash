#!/usr/bin/env python3
"""Small Smash command runner.

Usage:
  python smash.py init [target]
  python smash.py serve [target]
  python smash.py api [target]
  python smash.py demo [target]
  python smash.py try [target]
  python smash.py proof [target]
  python smash.py onboard [target]
  python smash.py seed [project-dir] [target]
  python smash.py welcome [target]
  python smash.py prompts [target]
  python smash.py status [target]
  python smash.py health [target]
  python smash.py operations [target]
  python smash.py backup [target]
  python smash.py restore-backup <backup-name-or-path> [target]
  python smash.py compliance-export [target]
  python smash.py team-sync [target]
  python smash.py share <page-or-memory> [target]
  python smash.py snapshot [target]
  python smash.py doctor [target]
  python smash.py migrate [target]
  python smash.py validate [target]
  python smash.py ingest-status [target]
  python smash.py import-obsidian <vault> [target]
  python smash.py remember "memory text" [target]
  python smash.py propose-memories <file-or-text> [target]
  python smash.py session-end <file-or-text> [target]
  python smash.py capture-inbox [target]
  python smash.py update-memory <name-or-title> "new memory text" [target]
  python smash.py query "task or question" [target]
  python smash.py graph-summary ["topic"] [target]
  python smash.py benchmark ["query"] [target]
  python smash.py brief ["task or question"] [target]
  python smash.py recall "query" [target]
  python smash.py profile [target]
  python smash.py wins [target]
  python smash.py memory-audit [target]
  python smash.py archive-memory <name-or-title> [target]
  python smash.py restore-memory <name-or-title> [target]
  python smash.py forget-memory <name-or-title> [target] --confirm
  python smash.py memory-inbox [target]
  python smash.py memory-log [target]
  python smash.py review-memory <name-or-title> [target]
  python smash.py explain-memory <name-or-title> [target]
  python smash.py rebuild-index [target]
  python smash.py rebuild-backlinks [target]
  python smash.py verify-mcp [target]
  python smash.py connect <agent> [target]
"""
...
DEFAULT_DEMO_DIR = "smash-demo"
DEFAULT_PROOF_DIR = "Smash-proof"
PROOF_MARKER = ".smash-proof"
PROOF_MEMORY_TITLE = "Cross-agent Smash proof"
PROOF_MEMORY_TEXT = (
    "For the Smash cross-agent proof, remember that local agent memory should be "
...
if (_BUNDLED_CORE / "smash_core").exists():
    sys.path.insert(0, str(_BUNDLED_CORE))

from smash_core.memory import (
...
from smash_core.audit_export import (
...
from smash_core.cli_memory import (
...
from smash_core.capture import (
...
from smash_core.mcp_connect import (
...
from smash_core.agent_hooks import (
...
def _missing_wiki_error(wiki_dir: Path) -> int:
    ...
    print(
        "Point Smash at your workspace (for example: "
        f"{_display_command(['smash', 'status', str(Path.home() / 'Smash')])}) "
        f"or create one here with {_display_command(['smash', 'init', '.'])}.",
        file=sys.stderr,
    )
    return 1
...
def status(target: Path, include_validation: bool = False, json_output: bool = False) -> int:
    target = target.expanduser().resolve()
    wiki_dir = _resolve_wiki_dir(target)
    payload = _core_link_status(wiki_dir, version=SMASH_VERSION, include_validation=include_validation)
    if json_output:
        print(json.dumps(payload, indent=2))
        return 0 if payload["ready"] else 1

    code, text = _core_render_status_text(payload, wiki_dir=wiki_dir, version=SMASH_VERSION)
    _print_text(text)
    return code
...
def _render_health_text(payload: dict[str, object]) -> str:
    ...
    lines = [
        f"Smash health: {status_payload.get('wiki')}",
        "",
        f"Version: {status_payload.get('version', SMASH_VERSION)}",
...
def health(target: Path, json_output: bool = False) -> int:
    target = target.expanduser().resolve()
    wiki_dir = _resolve_wiki_dir(target)
    status_payload = _core_link_status(wiki_dir, version=SMASH_VERSION, include_validation=True)
...
def remember(
    ...
    try:
        ...
        if "missing wiki directory" in str(exc):
            print(
                f"No workspace yet? Create one with {_display_command(['smash', 'onboard'])} "
...
def _capture_review_summary(target: Path, project: str | None = None, limit: int = 3) -> dict[str, object]:
    ...
    summary["next_action"] = f'python3 smash.py capture-inbox "{root}"'
    if summary["project"]:
        summary["next_action"] = f'python3 smash.py capture-inbox "{root}" --project "{summary["project"]}"'
    return summary
...
def start(
    ...
    seed_command = _display_command(["Smash", "seed", ".", str(target)])
    ...
    payload = {
        ...
        "commands": {
            "health": _display_command(["Smash", "health", str(target)]),
            "query": _display_command(["Smash", "query", query_text, str(target), "--budget", "micro"]),
            "brief": _display_command(["Smash", "brief", query_text, str(target)]),
            "remember": _display_command(["Smash", "remember", "<approved memory>", str(target)]),
            "review": _display_command(["Smash", "memory-inbox", str(target)]),
        ...
        "project_seed": {
            ...
            "safety": (
                "Run from the project repo. Smash reads allowlisted project docs/rules, "
...
def semantic(target: Path, setup: bool = False, rebuild: bool = False, json_output: bool = False) -> int:
    ...
    if setup or rebuild:
        ...
        if embedder is None and setup and _core_python_is_externally_managed():
            ...
            # here would be refused, so provision Smash's managed venv and run
            # the setup under it — same venv the MCP server uses.
            if not json_output:
                _print_text(
                    f"{sys.executable} cannot host the semantic extras (externally managed). "
                    "Provisioning ~/.smash-mcp-venv with them instead..."
                )
            outcome = _core_provision_link_extras(sys.executable, SMASH_VERSION)
            for note in outcome.get("notes", []):
                if not json_output:
                    _print_text(f"  {note}")
            if outcome.get("ready"):
                rerun = [str(outcome["python"]), str(ROOT / "smash.py"), "semantic", str(root), "--setup"]
                if json_output:
                    rerun.append("--json")
                return subprocess.run(rerun, check=False).returncode
            action_error = (
                "Could not provision the semantic extras into ~/.smash-mcp-venv. "
                "Create it by hand: python3 -m venv ~/.smash-mcp-venv && "
                f'~/.smash-mcp-venv/bin/python -m pip install "smash-mcp[semantic,semantic-quality,rerank]=={SMASH_VERSION}" '
                f"then rerun: ~/.smash-mcp-venv/bin/python {ROOT / 'smash.py'} semantic {root} --setup"
            )
        elif embedder is None:
            install_hint = f'{sys.executable} -m pip install "smash-mcp[semantic]"'
            action_error = (
                f"Semantic provider unavailable for {sys.executable}. Install it first: {install_hint}"
                if setup
                else "Semantic model not available offline. Run: smash semantic --setup"
            )
            mcp_python = _core_resolve_mcp_python(target, wiki_dir, None, default_python=sys.executable)
            if mcp_python != sys.executable:
                action_error += (
                    f"\nYour Smash MCP Python is {mcp_python}. If you installed the extra there, "
                    f"set it up through the MCP runtime instead: "
                    f"{mcp_python} -m smash_mcp --semantic-setup --wiki {wiki_dir}"
                )
        else:
            ...
            if setup and _core_semantic_provider() == "fastembed":
                action_result += (
                    ...
                    "Set SMASH_SEMANTIC_PROVIDER=model2vec (fast tier)."
                )
...
def _hook_session_start(
    ...
    if not wiki_dir.exists():
        _emit_session_start(
            f"Smash: wiki missing at {wiki_dir}; run {_display_command(['smash', 'init', str(target)])} to restore it.",
            emit,
        )
        return 0
    ...
    status_payload = _core_link_status(wiki_dir, version=SMASH_VERSION, include_validation=False)
...
def _session_end_hook_state_path(target: Path) -> Path:
    return _resolve_link_root(target) / ".smash-cache" / "session-end-hook.hash"
...
def _hook_session_end(
    ...
    _trace(
        ...
        f"dropped {extraction_stats.get('dropped_link_output', 0)} carrying Smash's own injected output (echo guard, layer 1)."
    )
    ...
    # caught by containment — is Smash hearing itself through the agent.
    # Automatic capture keeps only fresh or conflicting proposals; deliberate
    # refinements still flow through manual `smash session-end`.
    records = _memory_records(wiki_dir)
    fresh = []
...
def run_agent_hook(
    ...
    try:
        ...
        print(f"Smash {event} hook failed: {exc}", file=sys.stderr)
    return 0
...
def _lnk_on_path_runs_this_runtime() -> bool:
    """True when a `smash` on PATH is a shim for this very runtime.

    Homebrew installs the runtime under .../Cellar/Smash/<ver>/libexec and a
    `smash` shim in bin — users should see `smash` in every generated command,
    never the interpreter + Cellar path leaking into the product's own
    suggested next steps.
    """
    if "/Cellar/Smash/" in str(ROOT):
        return True
    smash = shutil.which("smash")
    if not smash:
        return False
    try:
        shim = Path(smash).read_text(encoding="utf-8", errors="replace")
    except OSError:
        return False
    return str(ROOT / "smash.py") in shim


def _configure_link_command_display() -> None:
    if os.environ.get("SMASH_CLI_COMMAND"):
        _core_set_link_command_override(None)
    elif _lnk_on_path_runs_this_runtime():
        _core_set_link_command_override(None)
    else:
        _core_set_link_command_override([sys.executable, str(ROOT / "smash.py")])
...
def verify_mcp(
    ...
    import_check: Callable[[str], dict[str, object]] = _core_check_smash_mcp_import,
    agent: str | None = None,
) -> int:
    ...
    if agent:
        ...
        if not server.get("configured"):
            message = (
                f"{server.get('display_name')} has no Smash MCP server configured "
                f"({server.get('config_path')}).\n"
                "Write one with: "
                + _display_command(["smash", "connect", str(server.get("agent")), str(target), "--write"])
            )
            if json_output:
        ...
        agent_note = (
            f"Verifying the Smash server {server.get('display_name')} is configured to run "
            f"({server.get('config_path')})."
        )
    target = target.expanduser().resolve()
    wiki_dir = _resolve_wiki_dir(target)
    status = _core_build_mcp_verify_status(
        target=target,
        wiki_dir=wiki_dir,
        init_command=[sys.executable, str(ROOT / "smash.py"), "init", str(target)],
        expected_version=SMASH_VERSION,
...
def connect_mcp(
    ...
    *,
    ...
    if hooks:
        runtime_script = target / "smash.py"
        runtime_note = ""
        if runtime_script.exists():
            ...
            stale_version = _core_workspace_runtime_is_older(target, SMASH_VERSION)
            if not (target / "smash_core" / "agent_hooks.py").exists():
                if write:
                    _copy_runtime_files(target)
                    runtime_note = f"Refreshed the Smash runtime at {target}: it predated session hooks."
                else:
                    runtime_note = (
                        f"The Smash runtime at {target} predates session hooks; "
                        "--write will refresh it automatically (or run "
                        f"{_display_command(['smash', 'init', str(target)])} first)."
                    )
            elif stale_version:
                if write:
                    _copy_runtime_files(target)
                    runtime_note = (
                        f"Refreshed the Smash runtime at {target}: "
                        f"{stale_version} → {SMASH_VERSION} (hooks run the workspace copy)."
                    )
                else:
                    runtime_note = (
                        f"The Smash runtime at {target} is {stale_version} but installed Smash is "
                        f"{SMASH_VERSION}; --write will refresh it (or run "
                        f"{_display_command(['smash', 'init', str(target)])})."
                    )
        else:
            runtime_script = ROOT / "smash.py"
...
def onboard(
    ...
    *,
        ...
        try:
            ...
            if status_value == "already_seeded":
                seed_result["next_commands"] = [
                    _display_command(["Smash", "seed", str(seed_project_root), str(target), "--overwrite"]),
                    _display_command(["Smash", "query", "what is this project about?", str(target), "--budget", "small"]),
                    _display_command(["Smash", "health", str(target)]),
                ]
            elif status_value == "needs_attention":
                seed_result["next_commands"] = [
                    "redact blocked project files, then rerun: "
                    + _display_command(["Smash", "seed", str(seed_project_root), str(target)])
                ]
                ...
                seed_result["next_commands"] = [
                    _display_command(["Smash", "query", "what is this project about?", str(target), "--budget", "small"]),
                    _display_command(["Smash", "brief", f"working on {seed_result.get('project_title') or 'this project'}", str(target)]),
                    _display_command(["Smash", "health", str(target)]),
                ]
            ...
            seed_result = {
                ...
                "read_error_count": 1,
                "next_commands": [_display_command(["Smash", "seed", seed_project, str(target)])],
            }
        ...
        try:
            ...
            if hooks and _core_supports_agent_hooks(agent):
                connection["session_hooks"] = _core_build_agent_hooks_payload(
                    target=target,
                    agent=agent,
                    runtime_script=(target / "smash.py") if (target / "smash.py").exists() else ROOT / "smash.py",
                    python_cmd=sys.executable,
                    write=write,
                )
            elif hooks:
    ...
    commands = {
        "health": _display_command(["Smash", "health", str(target)]),
        "serve": _display_command(["Smash", "serve", str(target), "--port", str(port)]),
        "seed_project": _display_command(["Smash", "seed", ".", str(target)]),
        "memory_inbox": _display_command(["Smash", "memory-inbox", str(target)]),
        "ingest_status": _display_command(["Smash", "ingest-status", str(target)]),
        "brief": _display_command(["Smash", "brief", "working with Smash", str(target)]),
    }
    payload: dict[str, object] = {
        "target": str(target),
        "created": created,
        ...
        "commands": commands,
        "agent_examples": [
            _display_command(["Smash", "onboard", str(target), "--agent", agent])
    ...
    *,
    ...
    if status_value == "already_seeded":
        payload["next_commands"] = [
            _display_command(["Smash", "seed", str(project_root), str(target), "--overwrite"]),
            _display_command(["Smash", "query", "what is this project about?", str(target), "--budget", "small"]),
            _display_command(["Smash", "health", str(target)]),
        ]
    elif status_value == "needs_attention":
        payload["next_commands"] = [
            "redact blocked project files, then rerun: "
            + _display_command(["Smash", "seed", str(project_root), str(target)])
        ]
    elif status_value == "empty":
        payload["next_commands"] = [
            "add README.md, AGENTS.md, CLAUDE.md, .cursorrules, or agent rule files, then rerun seed"
        ...
        payload["next_commands"] = [
            _display_command(["Smash", "query", "what is this project about?", str(target), "--budget", "small"]),
            _display_command(["Smash", "brief", f"working on {payload.get('project_title') or 'this project'}", str(target)]),
            _display_command(["Smash", "health", str(target)]),
        ]
    if json_output:
        print(json.dumps(payload, indent=2, default=str))
...
def serve_wiki(target: Path, port: int = 3000) -> int:
    ...
    if not serve_path.exists():
        print(f"Smash viewer missing: {serve_path}")
        print("")
        print("Next:")
        print(f"  {_display_command(['Smash', 'init', str(target)])}")
        return 1
    if not (target / "wiki").exists():
        print(f"Smash wiki missing: {target / 'wiki'}")
        print("")
        print("Next:")
        print(f"  {_display_command(['Smash', 'init', str(target)])}")
        return 1
...
def serve_api(target: Path, port: int = 3001) -> int:
    target = target.expanduser().resolve()
    if port < 1 or port > 65535:
        print("--port must be between 1 and 65535")
        return 1
    ...
    if not api_path.exists():
        api_path = target / "api.py"
    if not api_path.exists():
        print(f"Smash API runner missing: {api_path}")
        print("")
        print("Next:")
        print(f"  {_display_command(['Smash', 'init', str(target)])}")
        return 1
    ...
    try:
        return subprocess.run(
            [sys.executable, str(api_path), "--root", str(target), "--port", str(port)]
        ).returncode
    except KeyboardInterrupt:
        return 130
...
def create_demo(target: Path, force: bool = False) -> int:
    ...
    code, text = _core_render_demo_text(
        ...
        serve_command=_display_command(["python3", str(target / "smash.py"), "serve", str(target)]),
        next_command=_display_command(["python3", str(target / "smash.py"), "next", str(target)]),
        ...
        audit_command=_display_command(["python3", str(target / "smash.py"), "memory-audit", str(target)]),
    )
    _print_text(text)
    return code
    ...
    *,
    ...
    if target.exists() and any(target.iterdir()) and (force or created):
        ...
        if not marker.exists():
            print(f"{target} does not look like a Smash proof directory; refusing to overwrite it.", file=sys.stderr)
            return 1
        if force:
            shutil.rmtree(target)
            created = True
    target.mkdir(parents=True, exist_ok=True)
    _core_atomic_write_text(target / PROOF_MARKER, "Smash proof directory\n")
    _copy_runtime_files(target)
    _apply_doctor_fixes(target)
    ...
    if memory_result.get("created") and memory_result.get("name"):
        try:
            review_result = _mark_memory_reviewed(
                ...
                note="Reviewed automatically because the user explicitly ran smash proof.",
            )
    ...
    payload = {
        ...
        "prompts": {
            ...
            "agent_b": "start with Smash before we continue, then tell me what Smash remembers about cross-agent proof",
        },
        "commands": {
            "start": _display_command(["Smash", "start", command_target, "--task", "cross-agent proof"]),
            "recall": _display_command(["Smash", "query", query_text, command_target, "--budget", "micro"]),
            "mcp": _display_command(["Smash", "connect", "codex", command_target]),
            "serve": _display_command(["Smash", "serve", command_target, "--port", str(port)]),
        },
        "url": f"http://127.0.0.1:{port}",
    }
    if json_output:
...
def try_link(
    ...
    *,
    ...
    wiki_dir = _resolve_wiki_dir(target)
    status_payload = _core_link_status(wiki_dir, version=SMASH_VERSION, include_validation=True)
    query_payload = _query_link(wiki_dir, "why does Smash help agents?", budget="small")
    ...
    payload = {
        ...
        "commands": {
            "serve": _display_command(["Smash", "serve", str(target), "--port", str(port)]),
            "next": _display_command(["Smash", "next", str(target)]),
            "health": _display_command(["Smash", "health", str(target)]),
            "query": _display_command(["Smash", "query", "why does Smash help agents?", str(target), "--budget", "small"]),
            "brief": _display_command(["Smash", "brief", "working on agent memory", str(target)]),
            "benchmark": _display_command(["Smash", "benchmark", "agent memory", str(target)]),
        },
        "url": f"http://127.0.0.1:{port}",
    }
    if json_output:
...
# the default target (.) in a directory that has no Smash wiki, fall back to
# the default workspace (SMASH_WORKSPACE or ~/Smash) instead of dead-ending —
# `smash onboard` creates ~/Smash and the very next thing every new user types
# is `smash remember "..."` with no path. Creator commands (init, demo, try,
# proof, onboard) are excluded on purpose: they must act where they are told.
_WORKSPACE_COMMANDS = {
    "remember", "recall", "recipes", "query", "query-Smash", "brief", "start",
...
def _default_workspace() -> Path:
    return Path(os.environ.get("SMASH_WORKSPACE") or (Path.home() / "Smash")).expanduser()
...
def _apply_default_workspace(args) -> None:
    ...
    if (workspace / "wiki").exists():
        ...
        print(
            f"Workspace: {workspace} (no Smash wiki in the current directory; "
            "pass a path or set SMASH_WORKSPACE to change)",
            file=sys.stderr,
        )
...
def main(argv: list[str] | None = None) -> int:
    ...
    os.environ.setdefault("SMASH_SEMANTIC_SURFACE", "cli")
    ...
    try:
        return _core_dispatch_cli_command(args, {
            ...
            "connect": connect_mcp,
            "version": lambda: print(f"Smash {SMASH_VERSION}") or 0,
        })
    except ValueError as exc:
