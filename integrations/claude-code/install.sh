#!/bin/bash
# Smash integration for Claude Code
# One command: steering + wiki scaffold + MCP registration
#
# Usage:
#   bash install.sh             → global: ~/.claude/CLAUDE.md + central wiki at ~/Smash/
...
# Steering
link_upsert_instructions "$TARGET" "$INSTRUCTIONS_FILE" "Smash steering"
...
try:
    ...
    print("  ✓ Smash MCP registered in ~/.claude.json")
