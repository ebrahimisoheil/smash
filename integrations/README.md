...
One-step setup for local agents. The default mode creates one central Smash wiki
at `~/Smash/` and teaches your agent how to use it as local personal memory.

## Quick start

```bash
git clone https://github.com/ebrahimisoheil/smash.git ~/Smash-repo
bash ~/Smash-repo/integrations/codex/install.sh
```

On Windows PowerShell:

```powershell
git clone https://github.com/ebrahimisoheil/smash.git $HOME\Smash-repo
& $HOME\Smash-repo\integrations\codex\install.ps1
```
...
| ----------- | ------------------------------------------ | ---------------------------------------- | --------------------------------- |
| Kiro        | `bash integrations/kiro/install.sh`        | `.\integrations\kiro\install.ps1`        | `~/.kiro/steering/Smash.md`       |
...
| Cursor      | `bash integrations/cursor/install.sh`      | `.\integrations\cursor\install.ps1`      | `~/.cursor/rules/Smash.mdc`       |
...
- **Default (global):** `bash install.sh` or `.\install.ps1` — installs tool instructions globally + scaffolds central wiki at `~/Smash/`. One wiki for everything.
...
1. Upserts a small Smash instruction block without overwriting your existing instructions.
2. Scaffolds wiki structure at `~/Smash/` or the current directory with `--project`.
3. Installs or upgrades `smash-mcp`, using `~/.smash-mcp-venv` when system Python is externally managed.
4. Writes `.smash-mcp-python` so clients can use the Python that actually has `smash-mcp`.
5. Adds a short `smash` command wrapper for global installs, so checks are short: `smash health`.
...
explicit approval, and read `SMASH.md` only when it needs the full local protocol.
...
`Smash` MCP entry from the relevant JSON config if you need to undo it manually.
...
agent knows how to use Smash without wasting context.

Before changing an installer:

1. Keep the agent instruction small. It should point the agent to Smash tools,
   not paste the whole protocol into every prompt file.
2. Preserve existing user instructions. Upsert Smash blocks or config entries;
   never rewrite the whole agent config file.
3. Keep global and project mode behavior explicit. Global mode uses `~/Smash/`;
...
5. Reuse `.smash-mcp-python` when possible. MCP clients should run the Python
   environment that actually has `smash-mcp` installed.
