#!/bin/bash
# Smash integration for VS Code (Copilot Chat)
# One command: settings.json + wiki scaffold + smash-mcp install
#
# Usage:
#   bash install.sh             → .vscode/settings.json + central wiki at ~/Smash/
...
SMASH_INSTRUCTIONS_FILE="$INSTRUCTIONS_FILE" python3 - << 'PYEOF'
import json, os
target = ".vscode/settings.json"
instructions_text = open(os.environ["SMASH_INSTRUCTIONS_FILE"], encoding="utf-8").read()
settings = {}
...
print(f"Smash instructions → {target}")
PYEOF
...
echo "  { \"servers\": { \"Smash\": { \"type\": \"stdio\", \"command\": \"$MCP_PYTHON\", \"args\": [\"-m\", \"smash_mcp\", \"--wiki\", \"$WIKI_PATH\", \"--surface\", \"slim\"] } } }"
