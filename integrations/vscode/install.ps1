...
if (Test-Path $target) {
    try {
        ...
        if ($raw.Trim()) {
            $settings = Smash-ToHashtable ($raw | ConvertFrom-Json)
        }
    } catch {
        $settings = @{}
    }
...
if ($settings.ContainsKey($key) -and ($settings[$key] -is [System.Array])) {
    $items = @($settings[$key] | Where-Object {
        ...
        -not (
            $text -like "*## Smash — Local Agent Memory*" -or
            $text -like "*## Smash — Personal Knowledge Wiki*" -or
            $text -like "*Smash, an LLM-maintained knowledge wiki*"
        )
    })
}
...
Write-Host "Smash instructions -> $target"
...
Smash-UpsertMcpJson -Path ".vscode\mcp.json" -Command $mcpPython -WikiPath $wikiPath -TopKey "servers" -IncludeType
