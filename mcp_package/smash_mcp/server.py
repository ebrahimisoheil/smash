...
Exposes the Smash personal knowledge wiki over MCP.
...
Usage:
  python -m smash_mcp --surface slim                 # recommended agent surface
  python -m smash_mcp --wiki /path/wiki --surface slim
  python -m smash_mcp --wiki /path/wiki --surface full # compatibility surface
...
from smash_core.version import SMASH_VERSION
...
# explicitly first: without this, `python -m smash_mcp --help` would start
# the stdio server and hang silently waiting for MCP messages.
if "-h" in sys.argv[1:] or "--help" in sys.argv[1:]:
    print(__doc__.strip())
    print(
        "\nOptions:\n"
        "  --wiki PATH        wiki directory (default: ~/Smash/wiki)\n"
        "  --surface SURFACE  tool surface: slim (recommended) or full\n"
        "  --version          print the smash-mcp version and exit\n"
...
if args.version:
    print(f"smash-mcp {SMASH_VERSION}")
    sys.exit(0)

if args.wiki:
    WIKI_DIR = Path(args.wiki).expanduser().resolve()
else:
    WIKI_DIR = Path.home() / "Smash" / "wiki"

if args.semantic_setup:
    # One-time explicit opt-in for MCP-only installs (no `smash` CLI): fetch
    # the local embedding model and build the semantic index, then exit.
    # This is the only smash-mcp entry point allowed to touch the network.
    from smash_core.memory import memory_records as _setup_memory_records
    ...
    if not WIKI_DIR.exists():
        print(f"[smash-mcp] Wiki not found at {WIKI_DIR}; pass --wiki /path/to/wiki.", file=sys.stderr)
        sys.exit(2)
    print(
        f"[smash-mcp] Setting up semantic recall: this may download {_setup_model_name()} "
    ...
    if setup_embedder is None:
        print(
            "[smash-mcp] Semantic provider unavailable. Install it first: "
            "pip install \"smash-mcp[semantic]\"",
            file=sys.stderr,
        )
        sys.exit(2)
    ...
    print(f"[smash-mcp] Semantic recall ready: indexed {len(setup_items)} memories.")
    sys.exit(0)

MCP_SURFACE = (args.surface or os.environ.get("SMASH_MCP_SURFACE") or "slim").strip().lower()
if MCP_SURFACE not in {"full", "slim"}:
    print(
        f"[smash-mcp] Invalid surface {MCP_SURFACE!r}. Use --surface full or --surface slim.",
        file=sys.stderr,
    )
    sys.exit(2)
...
if not WIKI_DIR.exists():
    print(
        f"[smash-mcp] Wiki not found at {WIKI_DIR}. "
        "Initialize Smash first with `smash init` or `python3 smash.py init`, "
...
try:
    ...
    print("[smash-mcp] mcp package not found. Install with: pip install mcp", file=sys.stderr)
    sys.exit(1)

def _instructions(surface: str) -> str:
    if surface == "slim":
        return (
            "Smash is local personal memory for agents. Use status when "
            "connecting to Smash or troubleshooting readiness. Use recall for "
    ...
    return (
        "Smash is local personal memory for agents. This full MCP surface is "
        ...
        "connecting to Smash or troubleshooting setup/readiness. Start with "
        ...
        "what Smash remembers, and memory_inbox to find memories needing review. "
...
mcp = FastMCP("Smash", instructions=_instructions(MCP_SURFACE))
...
@mcp.resource(
    "smash://instructions",
    name="Smash agent instructions",
    description="Short, portable instructions for using Smash memory safely in any MCP client.",
    mime_type="text/markdown",
)
def link_instructions_resource() -> str:
    return (
        "# Smash Agent Instructions\n\n"
        "Use Smash as local, source-backed agent memory.\n\n"
        ...
        "If Smash session hooks are installed for this agent, the startup brief is injected automatically — "
        ...
        "provenance when explaining why Smash knows something.\n"
    )
...
@mcp.resource(
    "smash://health",
    name="Smash health",
    description="Current Smash readiness, validation, schema, and safe next actions.",
...
@mcp.resource(
    "smash://profile",
    name="Smash memory profile",
    description="Summary of what Smash remembers by type, scope, status, tags, and recency.",
    mime_type="application/json",
)
...
@mcp.resource(
    "smash://project",
    name="Smash project prompts",
    description="First-run prompts and checks for the configured Smash project/wiki.",
    mime_type="application/json",
)
...
@mcp.prompt(
    name="link_start",
    title="Smash: start a session",
    description="Begin work with Smash's safe readiness and recall loop.",
)
def link_start_prompt(task: str = "") -> str:
    task_text = task.strip() or "<current task>"
    return (
        "Start this session with Smash. If readiness is unknown, call status(include_validation=true). "
...
@mcp.prompt(
    name="link_brief",
    title="Smash: brief before work",
    description="Prime the agent with local Smash memory before a task.",
)
def link_brief_prompt(task: str = "") -> str:
    task_text = task.strip() or "<current task>"
    return (
        "Before answering or coding, get compact local context from Smash. "
...
@mcp.prompt(
    name="link_remember",
    title="Smash: remember explicit context",
...
def link_remember_prompt(memory: str = "") -> str:
    ...
    return (
        "Save this only if the user asked Smash to remember it. "
...
def link_session_end_prompt(summary: str = "") -> str:
    ...
    return (
        "End this session with Smash without silently saving durable memory. "
...
@mcp.prompt(
    name="link_ingest",
    title="Smash: ingest raw sources",
...
def link_ingest_prompt(path: str = "") -> str:
    ...
    return (
        ...
        "If Smash reports secret warnings or unreadable files, stop and ask the user to fix or redact them. "
...
def status(include_validation: bool = False) -> str:
    """Check Smash readiness and safe next actions.

    Use this first when connecting, troubleshooting, or deciding whether Smash
...
def remember(
    ...
    Use only when the user asks Smash to remember something or approves a memory
...
def admin(action: str, arguments: str = "{}") -> str:
    """Escape hatch for less common Smash maintenance and advanced workflows.
...
def query_link(query: str, budget: str = "medium", project: str = "") -> str:
    """Build a compact answer-ready Smash context packet.
...
def link_status(include_validation: bool = False) -> str:
    """Return a compact Smash readiness summary.

    Use this when connecting to Smash or troubleshooting setup. It reports the
...
def link_operations(limit: int = 20) -> str:
    """Inspect interrupted or active local Smash write operations.
...
def starter_prompts(project: str = "") -> str:
    """Return first-run Smash prompts and local checks.

    Use this when a user asks what to try after installing Smash, or when an
...
def backup_wiki(label: str = "mcp", include_raw: bool = False, list_only: bool = False) -> str:
    """Create or list local backup archives for this Smash wiki.

    Use before broad repairs or risky local wiki edits. Backups stay under
    .smash-backups/ next to the wiki. raw/ is excluded by default because it may
...
def migrate_wiki() -> str:
    """Apply safe Smash wiki schema migrations.
...
def search_wiki(query: str, limit: int = 20) -> str:
    """Search the Smash wiki by title, alias, tag, and full-text content.
...
def memory_profile(limit: int = 10, project: str = "") -> str:
    """Summarize what Smash currently remembers.
...
def memory_audit(limit: int = 10, project: str = "") -> str:
    ...
    Use this when the user asks what Smash knows, what needs attention, or
...
def memory_log(limit: int = 50, include_captures: bool = True) -> str:
    ...
    Use this when the user asks what Smash remembered, updated, reviewed,
...
def memory_wins(limit: int = 6, project: str = "") -> str:
    """Summarize local proof signals for Smash memory value.

    Use this when the user asks whether Smash is useful, what memory value has
    accumulated, or how to demonstrate the local memory loop. The result is
    based on local wiki metadata only; Smash does not track telemetry.
    """
...
def update_memory(
    ...
    asks to update something Smash already remembers. The update is appended to
...
def forget_memory(identifier: str, confirm: bool = False) -> str:
    ...
    the user asks Smash to permanently forget a memory; the tool refuses to
...
def get_context(topic: str) -> str:
    """Get full context for a topic from the Smash wiki.
    ...
    - inbound: pages that Smash TO this page
...
def get_pages(
    ...
    """List Smash wiki pages with metadata, bounded by default.
...
def get_backlinks(page_name: str, limit: int = 100, offset: int = 0, include_all: bool = False) -> str:
    """Get pages that Smash to or from a given wiki page, bounded by default.

    Returns:
    - inbound: pages that Smash TO this page (who references it)
    - forward: pages this page links TO (what it references)
    - inbound_count / forward_count: total available Smash counts
    - returned_inbound / returned_forward: returned Smash counts
    - follow_up: pagination and context actions when truncated

    Useful for understanding a page's position in the knowledge graph.
    Set include_all=true only when the user explicitly asks for a full Smash
    export.
    """
...
def get_graph_summary(topic: str = "", limit: int = 40, depth: int = 1, max_edges: int = 120) -> str:
    ...
    Args:
    - topic: optional topic/query. When provided, Smash returns a bounded
      neighborhood around matching pages. When omitted, Smash returns a
