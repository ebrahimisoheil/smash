...
<!-- mcp-name: io.github.ebrahimisoheil/smash -->

MCP server for [Smash](https://github.com/ebrahimisoheil/smash), local personal memory for agents. Exposes memories and wiki context as MCP tools so agents can recall preferences, decisions, project context, sources, and graph neighborhoods without reading files directly.

Listed on the [official MCP Registry](https://registry.modelcontextprotocol.io) as `io.github.ebrahimisoheil/smash`.

Release notes: [CHANGELOG.md](https://github.com/ebrahimisoheil/smash/blob/main/CHANGELOG.md)

## What You Need

`smash-mcp` is the MCP server. It needs a Smash wiki to read from. The normal
wiki location is `~/Smash/wiki`, created by the main Smash installers.

Recommended setup:

```bash
git clone https://github.com/ebrahimisoheil/smash.git
bash Smash/integrations/codex/install.sh   # or claude-code, cursor, kiro, vscode
```

The installer scaffolds `~/Smash/`, installs or upgrades `smash-mcp`, writes agent
instructions, and prints the exact MCP config for your machine.

If Smash is already installed and you only need to wire MCP into an agent, use
the CLI helper from the main Smash package:
...
query Smash for what you know about this project
```

CLI-first agents and Smash skills can run the same startup loop directly:
...
Use this when you already have a Smash wiki and only need the MCP package.
...
```powershell
py -m venv .smash-mcp-venv
.\.smash-mcp-venv\Scripts\python -m pip install --upgrade pip smash-mcp
```
...
```json
{
  "mcpServers": {
    "Smash": {
      "command": "/Users/YOU/.smash-mcp-venv/bin/python",
      "args": [
        "-m",
        "smash_mcp",
        "--wiki",
...
`~/Smash/wiki/`; override with `--wiki /path/to/wiki`. `--surface slim` is the
...
New MCP configs should expose Smash through six model-facing tools:
...
Smash also exposes MCP prompts `link_start`, `link_brief`, `link_remember`,
`link_session_end`, `link_ingest`, and `link_review`, plus resources
`smash://instructions`, `smash://health`, `smash://brief`, `smash://profile`, and
`smash://project` for clients that support prompt/resource attachment.
`link_start` and `smash://instructions` are the portable startup loop: check
...
or ask the user to run `smash seed . ~/Smash` from the repo before retrying recall.
...
For local CLI setup checks, `Smash verify-mcp --json` returns structured
...
memory loop automatically: `smash connect <agent> --hooks --write` (from a Smash
...
```bash
pip install "smash-mcp[semantic]"          # fast tier: tiny static model
pip install "smash-mcp[semantic-quality]"  # quality tier: contextual ONNX model
python3 -m smash_mcp --semantic-setup --wiki ~/Smash/wiki   # explicit one-time model fetch
```

The models load offline-only at recall time (a query can never trigger a
download), embeddings live in plain JSON under `.smash-cache/`, and there is no
...
Measured results: <https://github.com/ebrahimisoheil/smash/blob/main/benchmarks/RESULTS.md>.
...
[benchmarks/RESULTS.md](https://github.com/ebrahimisoheil/smash/blob/main/benchmarks/RESULTS.md).

## Privacy and Scale

- Local-first: `smash-mcp` reads the wiki path you configure and does not call
...
| `link_operations(limit?)`                                                                                                                                     | Inspect pending, failed, or interrupted local Smash write operations with safe next actions.                                                                                                                                                                                                                                                                       |
...
| `backup_wiki(label?, include_raw?, list_only?)`                                                                                                               | Create or list local `.smash-backups/` archives before broad repairs or risky wiki edits; raw sources are excluded by default.                                                                                                                                                                                                                                     |
...
| `memory_profile(limit?, project?)`                                                                                                                            | Summarize what Smash remembers by type, scope, status, recency, preferences, decisions, and project context.                                                                                                                                                                                                                                                       |
...
| `memory_wins(limit?, project?)`                                                                                                                               | Summarize local proof signals for what Smash memory is carrying without telemetry.                                                                                                                                                                                                                                                                                 |
...
Default: `~/Smash/wiki/`. Override with `--wiki /path/to/wiki`.

## Requirements

- Python 3.10+
- A Smash wiki (scaffolded by `install.sh`)
