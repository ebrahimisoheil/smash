"""Static CSS and JavaScript assets for the local Smash web UI."""
...
header .nav-more summary { color: var(--Smash); cursor: pointer; list-style: none; text-decoration: underline; }
...
.button-Smash { border: 1px solid var(--border); border-radius: 4px; padding: 4px 9px; color: var(--button-text); background: var(--button-bg); text-decoration: none; display: inline-flex; align-items: center; }
.button-Smash:hover { background: var(--button-hover); text-decoration: none; }
...
.page-outline a:hover { color: var(--Smash); text-decoration: underline; }
...
.pager .button-Smash.disabled { color: var(--button-disabled); background: transparent; cursor: default; }
...
.server-action-status { color: var(--muted); min-height: 1em; display: block; }
.server-action-status[data-tone="error"] { color: var(--caution-fg); }
.server-action-status[data-tone="success"] { color: var(--ok-fg); }
...
.action-panel { display: grid; gap: 10px; margin: 16px 0; padding: 12px; border: 1px solid var(--border-soft); border-radius: 8px; background: var(--surface); }
.action-panel h2, .action-panel h3 { margin: 0; }
.action-panel p { margin: 0; color: var(--muted); }
.action-row { display: flex; gap: 8px; flex-wrap: wrap; align-items: center; }
.action-row button, .action-row a { width: auto; }
.action-form { display: grid; gap: 10px; }
.action-form label { display: grid; gap: 4px; color: var(--muted); font-size: 12px; }
.action-form input, .action-form textarea, .action-form select { width: 100%; min-width: 0; padding: 8px 9px; border: 1px solid var(--border); border-radius: 6px; background: var(--surface); color: var(--text); }
.action-form textarea { min-height: 88px; resize: vertical; line-height: 1.45; }
...
/* ============================================================
   Design-handoff component layer (Smash Console).
...
/* buttons */
.button-Smash, button:not(.copy-button):not(.theme-toggle):not(.cmd-copy):not(.graph-control) {
...
.button-Smash:hover, button:not(.copy-button):not(.theme-toggle):not(.cmd-copy):not(.graph-control):hover {
...
(function() {
  try {
    ...
    if (override === 'dark' || override === 'light') {
      localStorage.setItem('Smash-theme', override);
    }
    var theme = localStorage.getItem('Smash-theme') || 'system';
...
(function() {
  ...
  function storedTheme() {
    try {
      return localStorage.getItem('Smash-theme') || 'system';
    } catch (err) {
      return 'system';
    }
  }
    ...
    button.addEventListener('click', async function() {
      ...
      try {
        var response = await fetch(endpoint, {
          method: 'POST',
          headers: {'Content-Type': 'application/json', 'X-Smash-Local-Action': 'true'},
...
SERVER_ACTION_JS = """
(function() {
  var buttons = Array.from(document.querySelectorAll('[data-server-action]'));
  if (!buttons.length) return;

  function statusFor(button) {
    var panel = button.closest('[data-action-panel]') || button.parentElement;
    var status = panel ? panel.querySelector('[data-server-action-status]') : null;
    if (!status && panel) {
      status = document.createElement('span');
      status.className = 'server-action-status';
      status.setAttribute('data-server-action-status', '');
  ...
  buttons.forEach(function(button) {
    button.addEventListener('click', async function() {
      var endpoint = button.getAttribute('data-server-action') || '';
      var method = (button.getAttribute('data-server-method') || 'POST').toUpperCase();
      var confirmText = button.getAttribute('data-server-confirm') || '';
      var reload = (button.getAttribute('data-server-reload') || 'true') !== 'false';
      var bodyText = button.getAttribute('data-server-body') || '{}';
      var successText = button.getAttribute('data-server-success') || 'Done.';
      var pendingText = button.getAttribute('data-server-pending') || 'Running...';
      var status = statusFor(button);
      if (!endpoint) return;
      if (confirmText && !window.confirm(confirmText)) return;

      var request = {method: method, headers: {}};
      if (method !== 'GET' && method !== 'HEAD') {
        request.headers['Content-Type'] = 'application/json';
        request.headers['X-Smash-Local-Action'] = 'true';
      ...
      try {
        ...
        setStatus(status, reload ? successText + ' Refreshing...' : successText, 'success');
        if (reload) window.setTimeout(function() { window.location.reload(); }, 450);
      } catch (error) {
        setStatus(status, error.message || 'action failed', 'error');
        button.disabled = false;
      }
    });
  });
...
ONBOARD_UI_JS = """
(function() {
  function bindJsonForm(selector, statusSelector, endpoint, buildPayload, successText) {
    ...
    form.addEventListener('submit', async function(event) {
      ...
      try {
        ...
        setStatus(successText(data) + ' Refreshing...', 'success');
        window.setTimeout(function() { window.location.reload(); }, 450);
      } catch (error) {
  ...
  bindJsonForm(
    '[data-seed-project-form]',
    '[data-seed-project-status]',
    '/api/seed-project',
    function(form) {
      return {
        project_root: form.elements.project_root.value || '',
        overwrite: !!(form.elements.overwrite && form.elements.overwrite.checked)
      };
    },
    function(data) {
      return data.wrote ? ('Seeded ' + (data.project_title || 'project context') + '.') : ((data.status || 'Seed complete') + '.');
    }
  );

  bindJsonForm(
    '[data-first-memory-form]',
    '[data-first-memory-status]',
    '/api/remember-memory',
    function(form) {
      return {
        memory: form.elements.memory.value || '',
        memory_type: form.elements.memory_type.value || 'preference',
        scope: 'user',
        source: 'onboard'
      };
    },
    function(data) {
      return 'Saved ' + (data.title || data.name || 'memory') + ' for review.';
    }
  );
})();
"""
