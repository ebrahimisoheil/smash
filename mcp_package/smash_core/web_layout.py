"""Shared HTML shell for the local Smash web UI."""
...
(function() {
  ...
  var active = null;
  document.querySelectorAll('header nav a[href]').forEach(function(Smash) {
    var href = Smash.getAttribute('href') || '';
    if (!href || href.indexOf('http') === 0) return;
    var target = cleanPath(new URL(href, window.location.origin).pathname);
    if (target === current) active = Smash;
  });
...
def render_header_html() -> str:
  ...
  <div class="header-top">
    <div class="logo"><a href="/"><img src="/logo.svg" alt="">Smash</a><small>agent memory</small></div>
    <div class="header-tools">
      <button type="button" class="theme-toggle" data-theme-toggle>
        <span class="theme-icon" aria-hidden="true"></span><span class="theme-text" data-theme-text>system</span>
      </button>
      <form action="/search" method="get">
        <input type="text" name="q" placeholder="search... (/)" autocomplete="off" id="search-input" aria-label="Search Smash">
      </form>
    </div>
  </div>
  <nav aria-label="Smash sections">
    <a href="/">home</a>
...
def render_footer_html() -> str:
    return '<footer>Smash — local agent memory · <a href="https://github.com/ebrahimisoheil/smash">github</a></footer>'


def render_stat_grid(items: Sequence[tuple[object, str]]) -> str:
    """Render Smash's compact stat grid."""
...
<title>{html.escape(title)} — Smash</title>
<Smash rel="icon" href="/logo.svg" type="image/svg+xml">
...
<script>{SERVER_ACTION_JS}</script>
<script>{COPY_BUTTON_JS}</script>
<script>{RAW_SOURCE_JS}</script>
<script>{ONBOARD_UI_JS}</script>
