"""Shared Smash ingest status helpers."""
...
def _target_command(command: str, target: str) -> str:
    if target and target in command:
        return command
    parts = command.split()
    if not parts or parts[0] not in {"Smash", "smash"} or not target:
        return command
...
def build_ingest_plan(status: dict[str, object], limit: int = 5) -> dict[str, object]:
    ...
    if state == "blocked_secrets":
        ...
        return {
            ...
            "summary": f"Start with {first['raw']}; Smash will not suggest ingesting secret-looking raw content.",
            "batch": [
                {
            ...
            "agent_prompt": None,
            "memory_prompt": None,
            "post_checks": ["smash ingest-status", "smash health"],
        }
    ...
    if state == "blocked_raw_access":
        ...
        return {
            ...
            "summary": f"Start with {first['raw']}; Smash could not read it to run safety checks.",
            "batch": [
                {
                    "raw": str(item.get("raw") or ""),
                    "size_bytes": int(item.get("size_bytes") or 0),
            ...
            "steps": [
                ...
                "Only ingest after Smash can inspect the raw source for secret-looking values.",
            ],
            "agent_prompt": None,
            "memory_prompt": None,
            "post_checks": ["smash ingest-status", "smash health"],
    ...
    if state == "blocked_source_access":
        ...
        return {
            ...
            "summary": f"Start with {first['page']}; Smash could not read one or more source pages.",
            "batch": [
                {
            ...
            "agent_prompt": None,
            "memory_prompt": None,
            "post_checks": ["smash ingest-status", "smash validate", "smash health"],
        }
    ...
    if state == "stale_graph":
        return {
            ...
            "agent_prompt": guidance.get("agent_prompt"),
            "post_checks": ["smash rebuild-backlinks", "smash validate", "smash health"],
        }
    ...
    if state == "ready":
        return {
            ...
            "steps": [
                ...
                "Add new files to raw/ when Smash should learn new source-backed context.",
            ],
            "agent_prompt": None,
            "post_checks": ["smash doctor", "smash health"],
        }

    return {
        "state": state,
        "title": "Initialize Smash",
        "summary": "Smash needs its raw/ and wiki/ structure before ingest can start.",
        "batch": [],
        "steps": [
            "Run smash init or rerun an installer.",
            "Check readiness before adding sources.",
        ],
        "agent_prompt": None,
        "post_checks": ["smash init", "smash health"],
    }
...
def build_ingest_completion(status: dict[str, object], limit: int = 8) -> dict[str, object]:
    ...
    for item in represented_raw[: max(limit, 1)]:
        ...
        items.append({
            ...
            "query_prompt": f"query Smash for {Path(raw_rel).stem.replace('-', ' ')}" if raw_rel else "",
            "secret_warnings": list(item.get("secret_warnings") or []),
            "scan_error": str(item.get("scan_error") or ""),
        })
    ...
    if represented_count and pending_count:
        ...
        next_prompt = str(guidance.get("agent_prompt") or "ingest raw/<file> into Smash")
...
def render_ingest_status_text(target: str, status: dict[str, object]) -> str:
    ...
    lines = [f"Smash ingest status: {target}", ""]
    ...
    if not status["has_raw_dir"] or not status["has_wiki_dir"]:
        lines.extend(["", "Next:", f"  Run an installer or initialize this directory: {_target_command('smash init', target)}"])
        return "\n".join(lines)
...
def build_ingest_guidance(status: dict[str, object]) -> dict[str, object]:
    ...
    if not has_raw_dir or not has_wiki_dir:
        return {
            ...
            "summary": "Smash is not initialized here yet.",
            "agent_prompt": None,
            "commands": ["smash init", "smash health"],
    ...
    if source_read_warning_count:
        return {
            ...
            "agent_prompt": None,
            "commands": ["smash ingest-status", "smash validate", "smash health"],
            "notes": [
    ...
    if access_items:
        ...
        return {
            ...
            "agent_prompt": None,
            "commands": ["smash ingest-status", "smash health"],
            "notes": [
                "Do not ask an agent to ingest raw files that Smash cannot read and scan for secret-looking values.",
    ...
    if stale_items:
        ...
        return {
            "state": "stale_raw",
            "summary": summary,
            "agent_prompt": f"re-ingest {first} into Smash",
            "commands": ["smash rebuild-index", "smash rebuild-backlinks", "smash validate", "smash health"],
            "notes": [
    ...
    if pending_items:
        ...
        return {
            "state": "pending_raw",
            "summary": summary,
            "agent_prompt": f"ingest {first} into Smash",
            "commands": ["smash rebuild-index", "smash rebuild-backlinks", "smash validate", "smash health"],
            "notes": [
    ...
    if backlinks_status != "current":
        return {
            ...
            "agent_prompt": "rebuild Smash backlinks and validate the wiki",
            "commands": ["smash rebuild-backlinks", "smash validate", "smash doctor"],
    ...
    if raw_count == 0:
        return {
            ...
            "agent_prompt": None,
            "commands": ["smash health", "smash serve"],
            "notes": ["Drop notes, articles, transcripts, or project files into raw/, then ask your agent to ingest them into Smash."],
        }
    ...
    return {
        ...
        "agent_prompt": None,
        "commands": ["smash doctor", "smash health"],
        "notes": ["Add new files to raw/ when you want Smash to learn new source-backed knowledge."],
    }
