"""Bundled first-run demo wiki content for Smash."""
...
RUNTIME_FILES = ("api.py", "serve.py", "smash.py", "SMASH.md", ".smashignore")
BRAND_FILES = ("logo.png", "logo.svg")
WORKSPACE_GITIGNORE = """# Smash private local source material
raw/*
!raw/.gitkeep

# Smash local runtime state
.smash-backups/
.smash-cache/
.smash-mcp-python
...
def copy_runtime_files(source_root: Path, target: Path) -> None:
    """Copy the runtime files needed by a standalone Smash wiki directory."""
    ...
    core_src = source_root / "mcp_package" / "smash_core"
    if not core_src.exists():
        core_src = source_root / "smash_core"
    if core_src.exists():
        core_target = target / "smash_core"
    ...
    for name in BRAND_FILES:
        src = source_root / name
        if not src.exists():
            src = source_root / "docs" / "assets" / name
        dst = target / name
        if src.exists() and src.resolve() != dst.resolve():
            shutil.copy2(src, dst)
...
def create_demo_workspace(target: Path, *, source_root: Path, force: bool = False) -> dict[str, object]:
    ...
    if target.exists() and any(target.iterdir()):
        ...
        if not force:
            raise DemoError(f"{target} already exists. Re-run with --force to replace a Smash demo directory.")
        if not marker.exists():
            raise DemoError(f"{target} does not look like a Smash demo directory; refusing to overwrite it.")
        shutil.rmtree(target)

    target.mkdir(parents=True, exist_ok=True)
    atomic_write_text(target / DEMO_MARKER, "Smash demo directory\n")
...
DEMO_FILES: dict[str, str] = {
    "START_HERE.md": """# Smash Demo: Start Here
...
```text
is Smash ready?
query Smash for why Smash helps agents
start with Smash before we continue
what does Smash remember about local personal memory?
explain why Smash remembers local personal memory
```

## Try These CLI Checks

```bash
python3 smash.py query "why does Smash help agents?" . --budget small
python3 smash.py brief "working on agent memory" .
python3 smash.py memory-audit .
python3 smash.py health .
```
...
- [[Smash]] exposes search, context, backlinks, and graph tools.
...
- Connects [[Smash]] to agent workflows through MCP.
...
> **TLDR:** A demo release note showing Smash as a local wiki viewer, graph, and MCP memory server.
...
The note connects [[Smash]] with [[knowledge-graph]] and [[local-first-software]], showing how local markdown can become both a human-readable wiki and agent-readable memory. *Source: [[local-release-notes]]* `[confidence: high]`

## Key Claims

- **Smash exposes search, context, backlinks, and graph tools.** `[confidence: high]`
...
- Describes [[Smash]] product surfaces.
...
Agent memory addresses a common failure mode in AI workflows: each new session starts without the full project history. In Smash, memory is stored as markdown wiki pages compiled from immutable raw sources. *Source: [[agent-memory-session]]* `[confidence: high]`
...
3. Smash builds search indexes and backlinks.
...
- [[Smash]] - provides the local wiki and MCP layer.
...
In Smash, the retrieval layer is a local markdown wiki exposed through search, context, backlinks, and graph tools. This makes [[agent-memory]] inspectable instead of hidden in a proprietary store. *Source: [[transformer-reading-notes]]* `[confidence: high]`
...
- [[Smash]] - provides the local retrieval surface.
...
Smash follows this model by storing raw sources and wiki pages as markdown files. The graph and MCP server read those files rather than sending them to a hosted backend. *Source: [[local-release-notes]]* `[confidence: high]`
...
- [[Smash]] - implements local-first agent memory.
...
In Smash, the knowledge graph makes relationships between sources, concepts, and entities visible. This helps users inspect what an agent has connected and where a claim came from. *Source: [[local-release-notes]]* `[confidence: high]`
...
- [[local-release-notes]]
""",
    "wiki/entities/Smash.md": """---
type: entity
title: "Smash"
entity_type: project
aliases: ["Smash wiki", "Smash MCP"]
date_created: "2026-05-02"
date_updated: "2026-05-02"
...
> **TLDR:** Smash is a local-first wiki and MCP server that turns source notes into durable memory for AI agents.

## Overview

Smash stores source material in `raw/` and compiled wiki pages in `wiki/`. The web viewer makes the wiki readable by humans, while MCP tools make the same knowledge readable by agents. *Source: [[local-release-notes]]* `[confidence: high]`

The demo positions Smash as a local [[agent-memory]] layer. It keeps knowledge inspectable through markdown and navigable through a [[knowledge-graph]]. *Source: [[agent-memory-session]]* `[confidence: high]`
...
> **TLDR:** The user wants Smash to be local personal memory for agents, with the wiki as the inspectable storage format.

## Memory

The user wants [[Smash]] to feel like local personal memory for agents rather than only a wiki. Agents should remember user preferences, project context, decisions, and why those memories exist.

## Use This When

- Positioning Smash in product copy or onboarding.
...
When answering from Smash, keep responses short and point to the [[knowledge-graph]] pages behind each claim instead of repeating whole documents.

## Use This When

- Formatting any answer that uses Smash context.
...
> **TLDR:** The local Smash viewer serves this wiki at http://127.0.0.1:3000 by default.

## Memory

`smash serve` binds the read-only local viewer for [[Smash]] to `127.0.0.1:3000`. It is loopback-only and needs no account or network access.
...
""",
    "wiki/explorations/why-Smash-helps-agents.md": """---
type: exploration
title: "Why Smash helps agents"
date_created: "2026-05-02"
query: "Why does Smash help AI agents?"
aliases: ["agent memory demo answer"]
tags: [agents, memory, demo]
---

# Why Smash helps agents

> **Query:** Why does Smash help AI agents?

## Answer

Smash helps agents because it turns past project material into durable, queryable context. Instead of starting each session from a blank chat, an agent can ask for [[agent-memory]] and receive the main page plus related concepts, sources, and entities.
...
The answer combines [[agent-memory-session]], [[transformer-reading-notes]], and [[local-release-notes]]. Together they show Smash as a local retrieval layer for AI workflows: sources become pages, pages form a [[knowledge-graph]], and MCP exposes that graph to agents.
...
- [[retrieval-augmented-generation]]
""",
    "wiki/index.md": """# Smash Demo Wiki Index
...
### entities
- [[Smash]] - Local-first wiki and MCP memory server for agents. growing - 2 sources - also: Smash MCP

### memories
- [[prefer-local-personal-memory]] - User preference that Smash should behave as local personal memory for agents. preference · user
- [[keep-answers-short-and-cite-wiki-sources]] - Reviewed style preference for answers built from Smash context. preference · user
...
- [[local-release-notes]] - Demo note on Smash surfaces and graph visibility. high

### explorations
- [[why-Smash-helps-agents]] - Filed answer explaining Smash as durable agent memory.
...
""",
    "wiki/log.md": """# Smash Demo Wiki Log
...
- Created: explorations/why-Smash-helps-agents.md
