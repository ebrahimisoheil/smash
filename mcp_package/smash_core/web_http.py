"""Shared local HTTP guard helpers and API metadata for Smash."""
...
BROWSER_SOURCE_LOCAL_ONLY = "Origin/Referer must match local Smash viewer"
...
LOCAL_API_RECOMMENDED_ENDPOINTS = {
...
def local_api_discovery_payload(api_version: str) -> dict[str, object]:
    """Return the stable route inventory for the loopback Smash HTTP API."""
    return {
        "api_version": str(api_version),
        "name": "Smash local HTTP API",
        "description": "Loopback-only API for Smash local agent memory.",
        "local_only": True,
        "recommended": dict(LOCAL_API_RECOMMENDED_ENDPOINTS),
        "endpoints": {
            "read": list(LOCAL_API_READ_ENDPOINTS),
            "write": list(LOCAL_API_WRITE_ENDPOINTS),
        },
...
def local_security_headers(
    ...
    return (
        ("X-Smash-API-Version", str(api_version)),
