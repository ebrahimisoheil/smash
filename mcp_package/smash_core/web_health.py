"""HTML helpers for Smash's local health page."""
...
def _render_browser_actions() -> str:
    return (
        '<section class="action-panel" data-action-panel>'
        '<h2>Repair In Browser</h2>'
        '<p>Run the core local repairs here, then let the page refresh with the updated state.</p>'
        '<div class="action-row">'
        '<button type="button" data-server-action="/api/validate" data-server-method="GET" '
        'data-server-reload="false" data-server-pending="Validating wiki..." '
        'data-server-success="Validation finished.">Validate</button>'
        '<button type="button" data-server-action="/api/migrate" data-server-method="POST" '
        'data-server-pending="Applying safe migration..." data-server-success="Migration finished.">Migrate</button>'
        '<button type="button" data-server-action="/api/rebuild-index" data-server-method="POST" '
        'data-server-pending="Rebuilding search index..." data-server-success="Search index rebuilt.">Rebuild index</button>'
        '<button type="button" data-server-action="/api/rebuild-backlinks" data-server-method="POST" '
        'data-server-pending="Rebuilding backlinks..." data-server-success="Backlinks rebuilt.">Rebuild backlinks</button>'
        '</div>'
        '<span class="server-action-status" data-server-action-status aria-live="polite"></span>'
        '</section>'
    )
...
def _render_primary_next_action(status: Mapping[str, object], operations: Mapping[str, object]) -> str:
    ...
    if operation_actions:
        ...
        detail = str(action.get("description") or action.get("detail") or "Run this before relying on Smash.")
        ...
        action = {"tool": "memory_brief", "arguments": {"query": "working with Smash"}}
        label = "Ready for agent work"
        detail = "Prime the agent with a brief or query Smash for project context."
...
def render_health_page(
    ...
    *,
    ...
    body = (
        '<div class="breadcrumb"><a href="/">Smash</a> / health</div>'
        "<h1>Health</h1>"
        ...
        f"{_render_issue_list('Interrupted Operations', operation_items, 'No pending, failed, or interrupted Smash operations.')}"
