"""Agent session-hook configuration helpers for Smash.

Hooks let supported agents run the Smash memory loop automatically:
...
class AgentHookConfig:
    ...
    restart_hint: str = "Restart the agent; new sessions will start with the Smash memory brief."
...
HOOK_AGENT_CONFIGS: tuple[AgentHookConfig, ...] = (
    ...
    AgentHookConfig(
        ...
        restart_hint=(
            ...
            "new sessions will then start with the Smash memory brief."
        ),
    ),
    AgentHookConfig(
        name="cursor",
...
def hook_supported_agents() -> tuple[str, ...]:
    """Return canonical agent names that support `smash connect --hooks`."""
...
def build_agent_hooks_payload(
    *,
    ...
    behavior = [
        f"{config.start_event}: injects a bounded Smash memory brief into new agent sessions.",
    ]
    if config.end_event:
        behavior.append(
        ...
        behavior.append(
            f"{config.display_name} has no session-end hook event; end sessions with `smash session-end` "
...
# Text that Smash itself injected into the session (the session-start brief,
# consolidation plans, session-end output). Messages containing these markers
# are Smash's own voice: extracting them back into memory proposals would
...
SMASH_ECHO_MARKERS = (
    ...
    "Smash memory brief",
    "Smash consolidation plan (read-only)",
    "Smash session end",
)
...
def extract_transcript_text(
    ...
    *,
    ...
    tool calls/results, meta entries, and any message carrying Smash's own
    injected output (see SMASH_ECHO_MARKERS), and returns the most recent
    ...
    With `keep_head`, when the turns exceed the budget Smash keeps the opening
    ...
    for line in raw.splitlines():
        ...
        if not text:
            continue
        if any(marker in text for marker in SMASH_ECHO_MARKERS):
