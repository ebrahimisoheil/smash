"""Local backup helpers for Smash wiki data."""
...
def create_backup(
    ...
    *,
    ...
    """Create a timestamped local backup archive for a Smash root.
    ...
    if not wiki_dir.exists() or not wiki_dir.is_dir():
        raise FileNotFoundError(f"Smash wiki not found at {wiki_dir}")
...
def list_backups(link_root: Path, *, limit: int = 20) -> dict[str, Any]:
    """Return recent local backups for a Smash root."""
    root = link_root.expanduser().resolve()
    backup_dir = root / BACKUP_DIR_NAME
...
def restore_backup(
    ...
    *,
    ...
    """Restore wiki/ and optionally raw/ from a local Smash backup archive."""
    root = link_root.expanduser().resolve()
    ...
    with tempfile.TemporaryDirectory(prefix=".smash-restore-", dir=root) as tmp_name:
