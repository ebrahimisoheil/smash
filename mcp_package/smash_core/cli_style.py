"""TTY-only styling for Smash's human CLI output."""
...
def supports_color(stream: TextIO | None = None) -> bool:
    ...
    if os.environ.get("NO_COLOR") or os.environ.get("SMASH_PLAIN"):
        return False
...
def style_cli_text(text: str, *, stream: TextIO | None = None) -> str:
    ...
    for line in text.splitlines():
        ...
        if not stripped:
            styled.append(line)
        elif re.match(r"^(Smash|No Smash)\b", stripped):
            ...
            styled.append(_wrap(line, YELLOW))
        elif stripped.startswith(("smash ", "python3 ", "py ", "brew ", "bash ", "http://", "https://")):
