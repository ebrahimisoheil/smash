"""Shared Smash runtime status helpers."""
...
def link_status(
    ...
    *,
    ...
    if version:
        ...
        if stale_runtime:
            warnings.append({
                "code": "stale_runtime",
                "message": (
                    f"The workspace runtime is {stale_runtime} but installed Smash is {version}; "
                    "session hooks run the workspace copy."
                ),
                "detail": f"Refresh it with: smash init {wiki_dir.parent}",
            })
        ...
        try:
            ...
            if stale_operations:
                warnings.append({
                    ...
                    "message": f"{len(stale_operations)} incomplete Smash operation(s) need review.",
                    "detail": str([
                        {
                ...
                warnings.append({
                    ...
                    "message": f"{len(fresh_operations)} Smash operation(s) are currently in progress.",
                    "detail": str([
                        {
                            "operation": item.get("operation"),
                            "description": item.get("description"),
            ...
            warnings.append(_warning(
                ...
                "Could not inspect Smash operation markers.",
                exc,
            ))
        ...
        try:
            ...
            if pages and search_backend != "sqlite-fts":
                warnings.append({
                    ...
                    "message": "SQLite FTS search is not active; Smash is using the slower token-index fallback.",
    ...
    if missing:
        next_actions.append(_action("repair or scaffold Smash structure", "doctor", {"fix": True}))
    if schema.get("status") in {"missing", "old"}:
        next_actions.append(_action("write current Smash wiki schema marker", "admin", {"action": "migrate"}))
    elif schema.get("status") == "invalid":
        next_actions.append(_action("inspect invalid Smash wiki schema marker", "doctor"))
    elif schema.get("status") == "newer":
        next_actions.append(_action("upgrade Smash before writing this wiki", "upgrade_link"))
    ...
    if stale_operation_degraded:
        next_actions.append(_action("inspect interrupted Smash operation markers", "doctor"))
