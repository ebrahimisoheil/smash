"""Optional local semantic recall for Smash.

Lexical recall stays the default and the fallback. When the optional local
embedding provider is installed (`pip install "smash-mcp[semantic]"`) and its
small static-embedding model has been fetched once through the explicit
`smash semantic --setup` command, memory recall additionally retrieves close
...
- No services, no vector database: embeddings live in a plain JSON cache
  under `.smash-cache/semantic/`, similarity is brute-force cosine in pure
...
SEMANTIC_MODEL_ENV = "SMASH_SEMANTIC_MODEL"
SEMANTIC_DISABLE_ENV = "SMASH_SEMANTIC"
...
SEMANTIC_PROVIDER_ENV = "SMASH_SEMANTIC_PROVIDER"
...
# SMASH_SEMANTIC_PROVIDER always wins.
SEMANTIC_SURFACE_ENV = "SMASH_SEMANTIC_SURFACE"
...
def semantic_provider() -> str | None:
    ...
    if surface == "cli" and _model2vec_installed():
        ...
        # quality tier. SMASH_SEMANTIC_PROVIDER overrides this split.
        return "model2vec"
...
RERANK_DISABLE_ENV = "SMASH_RERANK"
RERANK_MODEL_ENV = "SMASH_RERANK_MODEL"
...
def semantic_index_path(root: Path) -> Path:
    return root.expanduser().resolve() / ".smash-cache" / "semantic" / "memories.json"
    ...
    *,
    ...
    if disabled:
        ...
        if externally_managed:
            ...
            # refused, so --setup provisions Smash's managed venv instead.
            next_actions.append(
                f"smash semantic {command_target} --setup"
                "  # installs the semantic extras into ~/.smash-mcp-venv and fetches the models"
            )
        else:
            next_actions.append('pip install "smash-mcp[semantic]"  # fast tier (tiny static model)')
            next_actions.append('pip install "smash-mcp[semantic-quality]"  # quality tier (contextual model)')
            next_actions.append(f"smash semantic {command_target} --setup")
    elif not ready:
        next_actions.append(f"smash semantic {command_target} --setup")
    elif index_items < memory_count:
        next_actions.append(f"smash semantic {command_target} --rebuild")
    if installed and provider == "model2vec" and not _fastembed_installed() and not externally_managed:
        next_actions.append(
            'optional quality upgrade: pip install "smash-mcp[semantic-quality]" then rerun --setup'
        )
    ...
    if rerank_disabled():
        ...
        next_actions.append(f"smash semantic {command_target} --setup  # also fetches the rerank model")
    else:
        rerank_state = "not installed"
        if not externally_managed:
            next_actions.append('optional rerank tier: pip install "smash-mcp[rerank]" then rerun --setup')
    ...
    return {
        ...
        "network_policy": (
            ...
            "Only `smash semantic --setup` may fetch the model, once, with your approval."
        ),
