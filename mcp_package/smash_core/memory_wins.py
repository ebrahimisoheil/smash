"""Local proof signals for Smash memory value."""
...
def _link_command(*parts: object, project: str = "") -> str:
    command = ["Smash", *(str(part) for part in parts if str(part) != "")]
    if project:
...
def memory_wins_payload(
    ...
    *,
    ...
    """Summarize local, non-telemetry signals that Smash memory is carrying value."""
    ...
    wins = [
        ...
        _win(
            ...
            "what does Smash remember about me?",
        ),
        _win(
            "review_gate",
            "Review gate",
            ...
            "review Smash memory inbox",
        ),
        _win(
            "source_grounding",
            "Provenance present",
            len(source_backed),
            "Memories with source metadata make it easier to inspect why Smash knows something.",
            "explain the most important Smash memory",
        ),
        _win(
            "project_continuity",
            "Project continuity",
            ...
            "brief me from Smash for this project",
        ...
        _win(
            ...
            len(updated_records),
            "Updated memories show where Smash has evolved rather than staying as a one-time demo.",
            "what changed in Smash memory recently?",
        ),
    ]
    return {
        "schema": "smash-memory-wins-v1",
        ...
        "honest_note": (
            ...
            "Smash does not track whether an agent used a memory."
        ),
    }
        ...
        return [
            {
                ...
                "reason": "A wins report becomes useful after Smash has at least one durable memory.",
