"""Text rendering helpers for Smash query and graph CLI commands."""
...
def render_query_text(
    ...
    *,
    ...
    if not payload.get("found"):
        lines = [f"No Smash context found for: {query_text}"]
        ...
        lines.extend([
            "",
            "Next:",
            "- Add source material under raw/ and ask your agent: ingest the new raw Smash files",
            f"- Run: {display_command(['Smash', 'ingest-status', *target_parts])}",
            f"- Then rerun: {display_command(['Smash', 'query', query_text, *target_parts])}",
        ])
        return 0, "\n".join(lines)

    lines = [f"Smash context packet: {payload['query']}"]
