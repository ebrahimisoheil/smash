"""Argument parser for the Smash command-line interface."""
...
from .version import SMASH_VERSION
...
COMMAND_GROUPS: tuple[tuple[str, tuple[str, ...]], ...] = (
    ...
    ("Sharing & viewing", (
        "serve", "api", "share", "snapshot", "graph-summary", "team-sync",
        "compliance-export",
    )),
    ("Utilities", (
        "version", "benchmark", "wins", "profile", "rebuild-index",
        "rebuild-backlinks", "query-Smash",
    )),
)
...
def _grouped_epilog() -> str:
    ...
    lines.append("\nRun `smash.py <command> --help` for that command's options.")
    return "\n".join(lines)
...
def build_cli_parser(
    ...
    """Build the Smash CLI argument parser."""
    parser = argparse.ArgumentParser(
        prog="smash.py",
        description="Smash — local, review-gated memory for AI agents. New? Run: smash.py try",
        epilog=_grouped_epilog(),
        formatter_class=_GroupedCommandHelp,
    )
    parser.add_argument("--version", action="version", version=f"Smash {SMASH_VERSION}")
    sub = parser.add_subparsers(dest="command", required=True, metavar="<command>")

    sub.add_parser("version", help="print the Smash CLI version")

    init_cmd = sub.add_parser("init", help="create or repair a normal Smash wiki")
    init_cmd.add_argument("target", nargs="?", default=".")

    serve_cmd = sub.add_parser("serve", help="start the local Smash web viewer")
    serve_cmd.add_argument("target", nargs="?", default=".")
    serve_cmd.add_argument("--port", type=int, default=3000)

    api_cmd = sub.add_parser("api", help="start the local Smash HTTP API runner")
    api_cmd.add_argument("target", nargs="?", default=".")
    api_cmd.add_argument("--port", type=int, default=3001)

    demo = sub.add_parser("demo", help="create a pre-ingested sample Smash wiki")
    demo.add_argument("target", nargs="?", default=default_demo_dir)
    demo.add_argument("--force", action="store_true", help="replace an existing Smash demo directory")
    ...
    try_cmd.add_argument("--force", action="store_true", help="replace an existing Smash demo directory")
    ...
    proof_cmd.add_argument("--force", action="store_true", help="replace an existing Smash proof workspace")
    ...
    onboard_cmd = sub.add_parser("onboard", help="set up a real Smash workspace and print the agent-first next steps")
    onboard_cmd.add_argument("target", nargs="?", default="~/Smash")
    ...
    seed_cmd = sub.add_parser("seed", help="seed Smash with source-backed context from this project")
    seed_cmd.add_argument("project", nargs="?", default=".", help="project directory to inspect")
    seed_cmd.add_argument("target", nargs="?", default="~/Smash", help="Smash workspace to seed")
    ...
    welcome_cmd = sub.add_parser("welcome", help="print the shortest first-use path for Smash")
    ...
    status_cmd = sub.add_parser("status", help="show Smash readiness, counts, and next actions")
    ...
    operations_cmd = sub.add_parser("operations", help="inspect interrupted or active Smash write operations")
    ...
    restore_backup_cmd = sub.add_parser("restore-backup", help="preview or restore a local Smash backup archive")
    restore_backup_cmd.add_argument("backup", help="backup filename from .smash-backups/ or path to a .tar.gz archive")
    ...
    team_sync_cmd = sub.add_parser("team-sync", help="print a safe Git plan for sharing reviewed Smash memory")
    ...
    snapshot_cmd.add_argument("--output", default="Smash-snapshot", help="directory to write the snapshot into")
    ...
    snapshot_cmd.add_argument("--title", default="Smash", help="snapshot title")
    snapshot_cmd.add_argument("--json", action="store_true", help="print machine-readable snapshot status")

    doctor_cmd = sub.add_parser("doctor", help="check a Smash wiki for common health issues")
    ...
    migrate_cmd = sub.add_parser("migrate", help="apply safe Smash wiki schema migrations")
    ...
    obsidian_cmd = sub.add_parser("import-obsidian", help="copy Obsidian Markdown notes into raw/ for Smash ingest")
    ...
    remember_cmd.add_argument("--applies-when", default=None, dest="applies_when", help='scoping conditions, e.g. "project:Smash, task:cutting a release, path:*repo*" (OR semantics)')
    ...
    query_cmd = sub.add_parser("query", aliases=["query-Smash"], help="build a compact answer-ready Smash context packet")
    ...
    start_cmd = sub.add_parser("start", help="start a session with Smash readiness and a memory brief")
    ...
    profile_cmd = sub.add_parser("profile", help="show what Smash remembers")
    ...
    wins_cmd = sub.add_parser("wins", help="show local proof signals for what Smash memory is carrying")
    ...
    verify_mcp_cmd = sub.add_parser(
        "verify-mcp",
        help="verify smash-mcp import and print MCP config; pass an agent name to check what that agent is configured to run",
    )
    ...
    connect_cmd.add_argument(
        "--hooks",
        action="store_true",
        help="also configure session hooks so new sessions start with the Smash brief (Claude Code, Codex, Cursor)",
    )
    connect_cmd.add_argument(
        "--hooks-settings",
        default=None,
...
def dispatch_cli_command(args: Any, handlers: Mapping[str, CliHandler]) -> int:
    """Dispatch parsed Smash CLI arguments to runtime-provided handlers."""
    ...
    if command == "api":
        return handlers["api"](Path(args.target), port=args.port)
    if command == "demo":
    ...
    if command in {"query", "query-Smash"}:
