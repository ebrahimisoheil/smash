"""HTML page renderers for Smash's memory web views."""
...
def render_brief_page(
    ...
    *,
    ...
    query_action = (
        copy_button(f"query Smash for {query_prompt}", "Copy query prompt")
        if query_prompt else ""
    )
    relevant_memories = _dict_list(brief.get("relevant_memories"))
    body = (
        '<div class="breadcrumb"><a href="/">Smash</a> / brief</div>'
        '<h1>Memory Brief</h1>'
...
def _render_empty_brief_actions(query: str) -> str:
    ...
    proposal_prompt = (
        f"propose memories about {query_text} from Smash raw sources"
        if query_text else "propose memories from Smash raw sources"
    )
    return (
        '<div class="memory-next"><strong>Teach Smash before the next brief</strong>'
        "<ul>"
...
def _brief_prompt(query: str, project: str = "") -> str:
    ...
    if task and project_name:
        return f"brief me from Smash about {task} for project {project_name}"
    if task:
        return f"brief me from Smash about {task}"
    if project_name:
        return f"brief me from Smash for project {project_name}"
    return "start with Smash before we continue"
...
def _memory_overview_prompt(project: str = "") -> str:
    project_name = str(project or "").strip()
    if project_name:
        return f"what does Smash remember about project {project_name}?"
    return "what does Smash remember about me?"


def _audit_prompt(project: str = "") -> str:
    project_name = str(project or "").strip()
    if project_name:
        return f"audit Smash memory for project {project_name}"
    return "audit Smash memory"


def _inbox_prompt(project: str = "") -> str:
    project_name = str(project or "").strip()
    if project_name:
        return f"review Smash memory inbox for project {project_name}"
    return "review Smash memory inbox"


def _capture_prompt(project: str = "") -> str:
    project_name = str(project or "").strip()
    if project_name:
        return f"review Smash raw captures for project {project_name}"
    return "review Smash raw captures"
...
def render_memory_dashboard_page(
    ...
    *,
    ...
    body = (
        '<div class="breadcrumb"><a href="/">Smash</a> / memory</div>'
...
def render_profile_page(
    ...
    *,
    ...
    body = (
        '<div class="breadcrumb"><a href="/">Smash</a> / profile</div>'
        '<h1>Memory Profile</h1>'
        '<div class="memory-profile">'
        '<p class="summary">What Smash currently remembers about the user, projects, decisions, and preferences.</p>'
        f'{profile_actions}'
        f'{_project_line(project)}'
        f'{stats}'
...
def render_memory_audit_page(
    ...
    *,
    ...
    body = (
        '<div class="breadcrumb"><a href="/">Smash</a> / audit</div>'
        '<h1>Memory Audit</h1>'
        '<div class="memory-profile">'
...
def render_captures_page(inbox: Mapping[str, object], *, layout: PageLayout) -> str:
    ...
    if warning_count:
        warning_html = (
            ...
            '<code>python3 smash.py redact-capture raw/memory-captures/&lt;capture&gt;.md .</code></div>'
        )
    read_warning_html = ""
    ...
    body = (
        '<div class="breadcrumb"><a href="/">Smash</a> / captures</div>'
...
def render_inbox_page(
    ...
    *,
    ...
    body = (
        '<div class="breadcrumb"><a href="/">Smash</a> / inbox</div>'
...
def render_memory_log_page(log_payload: Mapping[str, object], *, layout: PageLayout) -> str:
    ...
    if entries:
        ...
        content = (
            ...
            '<p><a class="button-Smash" href="/propose">Create memory proposals</a></p>'
        )
    body = (
        '<div class="breadcrumb"><a href="/">Smash</a> / memory log</div>'
...
def render_memory_wins_page(
    ...
    *,
    ...
    body = (
        '<div class="breadcrumb"><a href="/">Smash</a> / wins</div>'
        '<h1>Memory Wins</h1>'
        '<div class="memory-profile">'
        '<p class="summary">Local proof signals for what Smash memory is carrying. These are computed from the wiki, not telemetry.</p>'
...
def render_memory_explanation_page(
    ...
    *,
    ...
    graph_html = (
        '<h2>Graph</h2>'
        f'<p><a class="button-Smash" href="{html.escape(graph_href, quote=True)}">Open local graph</a></p>'
    ...
    body = (
        '<div class="breadcrumb"><a href="/">Smash</a> / explain memory</div>'
