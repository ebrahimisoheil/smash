"""Obsidian vault import helpers for Smash."""
...
def import_obsidian_vault(
    ...
    *,
    ...
    next_commands = [f"smash ingest-status {root}", f"smash validate {root}", f"smash health {root}"]
    if not has_wiki_dir:
        next_commands = [f"smash init {root}", *next_commands]
    return {
        "status": status,
        "target": str(root),
        ...
        "next_prompt": f"ingest raw/obsidian/{vault_slug} into Smash",
...
def render_import_obsidian_text(payload: dict[str, object]) -> tuple[int, str]:
    ...
    if not payload.get("has_wiki_dir"):
        lines.append("Smash wiki is not initialized yet; run the init command below before ingest.")
