"""HTML helpers for Smash's local onboarding page."""
...
def _agent_cards(target: str, agents: Sequence[str]) -> str:
    cards = ""
    for agent in agents:
        preview = display_command(["Smash", "onboard", target, "--agent", agent])
        write = display_command(["Smash", "onboard", target, "--agent", agent, "--write"])
        cards += (
...
def _render_browser_setup(target: str) -> str:
    target_hint = html.escape(target or ".", quote=True)
    return (
        '<section><h2>Run Setup Here</h2>'
        '<div class="action-panel">'
        '<h3>Seed project context</h3>'
        '<p>Write source-backed project context into <code>raw/</code> and <code>wiki/sources/</code>. Use <code>.</code> when the viewer is already running in the project repo.</p>'
        '<form class="action-form" data-seed-project-form>'
        f'<label>Project root<input name="project_root" value="{target_hint}" autocomplete="off"></label>'
        '<label><input type="checkbox" name="overwrite"> Overwrite an existing project seed if one is already present</label>'
        '<div class="action-row"><button type="submit">Seed project</button><span>Writes source-backed context, not durable memory.</span></div>'
        '<span class="server-action-status" data-seed-project-status aria-live="polite"></span>'
        '</form>'
        '</div>'
        '<div class="action-panel">'
        '<h3>Save one explicit memory</h3>'
        '<p>Start with a durable preference, decision, or project fact. This creates a reviewable memory page immediately.</p>'
        '<form class="action-form" data-first-memory-form>'
        '<label>Memory text<textarea name="memory" placeholder="I prefer concise release notes."></textarea></label>'
        '<label>Memory type<select name="memory_type"><option value="preference">preference</option><option value="decision">decision</option><option value="project">project</option><option value="note">note</option></select></label>'
        '<div class="action-row"><button type="submit">Save memory for review</button><span>Nothing is hidden: the new memory appears in the local inbox.</span></div>'
        '<span class="server-action-status" data-first-memory-status aria-live="polite"></span>'
        '</form>'
        '</div>'
        '</section>'
    )
...
def render_onboard_page(
    ...
    *,
    ...
    """Render a guided first-run page for humans setting up Smash with agents."""
    ...
    health_command = display_command(["Smash", "health", command_target])
    onboard_command = display_command(["Smash", "onboard", command_target])
    seed_onboard_command = display_command(["Smash", "onboard", command_target, "--seed-project", "."])
    seed_command = display_command(["Smash", "seed", ".", command_target])
    ...
    brief_command = display_command(["Smash", "brief", "working with Smash", command_target])
    ingest_command = display_command(["Smash", "ingest-status", command_target])
    memory_inbox_command = display_command(["Smash", "memory-inbox", command_target])
    ...
    setup_cards = (
        '<section class="onboard-steps" aria-label="Smash onboarding steps">'
        ...
        '<p>Start with one explicit preference or decision. Smash saves it for review.</p>'
    ...
    body = (
        '<div class="breadcrumb"><a href="/">Smash</a> / onboard</div>'
        "<h1>Onboard</h1>"
        ...
        f"{stats}"
        f"{setup_cards}"
        f"{_render_browser_setup(command_target)}"
        '<section><div class="section-heading"><h2>Agent Wiring</h2><a href="/prompts">starter prompts</a></div>'
        '<p class="summary">Preview commands are safe. Add <code>--write</code> only when you want Smash to update an agent config.</p>'
        f'<div class="onboard-agent-grid">{agent_cards}</div></section>'
        '<section><h2>Ask Your Agent First</h2>'
        '<p class="summary">These are the prompts that make Smash feel automatic without hiding what changed.</p>'
