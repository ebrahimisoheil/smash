"""Shared memory logic for Smash CLI, HTTP, and MCP runtimes."""
...
def stem_memory_token(token: str) -> str:
    ...
    match "answer" without changing Smash's local-first story.
    """
...
def memory_action_hints(
    ...
    if status == "archived":
        add(_memory_action(
            ...
            command=_shell_words("python3", "smash.py", "restore-memory", name, command_target),
            tool="restore_memory",
        ...
        add(_memory_action(
            ...
            command=_shell_words("python3", "smash.py", "explain-memory", name, command_target),
            tool="explain_memory",
            arguments={"identifier": name},
            priority="medium",
        ))
        add(_memory_action(
            ...
            command=_shell_words("python3", "smash.py", "forget-memory", name, command_target, "--confirm"),
            tool="forget_memory",
    ...
    if issue_codes & {"needs_update", "missing_summary"}:
        add(_memory_action(
            ...
            command=_shell_words("python3", "smash.py", "update-memory", name, "new detail", command_target),
            tool="update_memory",
    ...
    if issue_codes & {"stale_status", "expired"}:
        ...
        add(_memory_action(
            ...
            command=_shell_words("python3", "smash.py", "archive-memory", name, command_target, "--reason", reason),
            tool="archive_memory",
    ...
    if issue_codes & {"pending_review", "review_due"} and not any(
        ...
        add(_memory_action(
            ...
            command=_shell_words("python3", "smash.py", review_cli, name, command_target),
            tool=review_tool,
            arguments={"identifier": name},
            priority="high",
        ))
    ...
    if "archive" not in seen:
        add(_memory_action(
            ...
            command=_shell_words("python3", "smash.py", "archive-memory", name, command_target, "--reason", "why"),
            tool="archive_memory",
        ...
        return [
            {
                ...
                "command": f'python3 smash.py memory-inbox "{root}"{project_arg}',
                "recommended": review_recommended,
            },
            {
                "label": "Review raw captures",
                "command": f'python3 smash.py capture-inbox "{root}"{project_arg}',
                "recommended": capture_recommended,
            },
            {
                "label": "Run doctor",
                "command": f'python3 smash.py doctor "{root}"',
...
def render_recipes_text(recipes: Sequence[Mapping[str, object]], target: object = ".") -> tuple[int, str]:
    lines = ["Smash recipes (procedural memory)"]
    if not recipes:
        lines.extend([
            "",
            "No recipes yet. Save one after a multi-step task:",
            f"  {display_command(['smash', 'remember', '<steps>', str(target), '--type', 'procedure', '--trigger', '<when to use>'])}",
        ])
        return 0, "\n".join(lines)
...
def parse_applies_when(value: object) -> list[tuple[str, str]]:
    ...
    e.g. "project:Smash, task:cutting a release, path:*picochat*".
...
def recall_abstention(results: list[dict[str, object]]) -> dict[str, object]:
    ...
    an agent dress a weak match up as an answer. Smash already computes the
...
def _shell_words(*parts: object) -> str:
    ...
    if not words:
        return ""
    if len(words) >= 2 and words[0].startswith("python") and words[1] == "smash.py":
        return display_command(["Smash", *words[2:]])
...
def memory_proposal_action(proposal: Mapping[str, object], *, command_target: str | Path = ".") -> dict[str, object]:
    ...
    if conflict_list:
        ...
        action = _memory_action(
            ...
            command=_shell_words("python3", "smash.py", "explain-memory", identifier, command_target),
            tool="explain_memory",
...
def classify_memory_segment(segment: str) -> dict[str, object] | None:
    ...
    checks: list[tuple[str, str, int, str, tuple[str, ...]]] = [
        ...
        (
            ...
            (
                r"\b(?:project|repo|repository|Smash)\s+(?:uses|requires|runs|stores|keeps|ships|releases)\b",
