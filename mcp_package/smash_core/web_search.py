"""HTML helpers for Smash's local search page."""
...
def render_search_page(
    ...
    *,
    ...
    if not normalized:
        return layout(
            "Search",
            '<div class="breadcrumb"><a href="/">Smash</a> / search</div>'
            "<h1>Search</h1>"
    ...
    actions = (
        ...
        f'<a class="button-Smash" href="{html.escape(graph_href, quote=True)}">Open graph search</a>'
        f'<a class="button-Smash" href="{html.escape(brief_href, quote=True)}">Open memory brief</a>'
        f'{copy_button(f"query Smash for {query}", "Copy query prompt")}'
        "</div>"
    )
    if total == 0:
        ingest_prompt = "ingest the new raw Smash files"
        proposal_prompt = f"propose memories about {query} from Smash raw sources"
        ...
        return layout(
            f"Search: {query}",
            f'<div class="breadcrumb"><a href="/">Smash</a> / search</div>'
...
def render_search_refine_form(query: str, *, active_type: str = "") -> str:
    ...
    return (
        ...
        'placeholder="search titles, tags, and page text" autocomplete="off" aria-label="Search Smash">'
        f"{type_input}"
