"""HTML renderer for Smash's guided ingest web view."""
...
def render_ingest_page(
    ...
    *,
    ...
    ingest_prompt = agent_prompt or f"ingest {first_raw} into Smash"
    ...
    body = (
        '<div class="breadcrumb"><a href="/">Smash</a> / ingest</div>'
        '<h1>Ingest</h1>'
...
def _render_local_checks() -> str:
    return (
        '<section class="action-panel" data-action-panel>'
        '<h2>Run Local Checks</h2>'
        '<p>Validate or repair the local graph without leaving the ingest workflow.</p>'
...
def _render_next_step(
    *,
    ...
    if agent_prompt:
        ...
        next_detail = "Fix raw file access before asking any agent to ingest it. Smash could not inspect the source for safety."
        next_code = f"inspect {first_raw}"
        next_extra = ""
    elif state == "blocked_source_access":
        next_detail = "Fix source page access before relying on ingest state. Smash could not inspect represented source pages."
        ...
        next_detail = "No ingest is pending. Ask Smash for context, or add another source when there is new material."
        next_code = _link_command(command_target, "brief", "current task")
        next_extra = ""
    else:
        next_detail = "Initialize or repair the Smash folder before ingesting sources."
...
def _render_pending(pending: list[dict[str, object]], represented: list[dict[str, object]]) -> str:
    ...
    for item in pending[:50]:
        ...
        ingest_prompt = f"ingest {raw_path} into Smash" if raw_path else "ingest raw/<file> into Smash"
        actions = ""
...
def _render_raw_source_form() -> str:
    return (
        ...
        '<p class="summary">Paste a note, article excerpt, transcript, or project context. Smash saves it under '
