...
The map a new maintainer needs before touching anything. For what Smash _is_,
read the [README](README.md); this is how it works and where things live.

## The one-paragraph version

Smash is a local memory layer for AI agents. A **workspace** (default
`~/Smash`) holds immutable raw sources (`raw/`), an agent-compiled Markdown
wiki (`wiki/`), and reviewed **memory pages** (`wiki/memories/*.md` — plain
Markdown with YAML frontmatter). The shared core now sits behind five local
surfaces: the CLI (`smash.py`), a dedicated JSON API runner (`api.py`), the
MCP server (`mcp_package/smash_mcp/`), agent session hooks, and the local web
viewer (`serve.py`). Nothing durable is written without user approval, no LLM
runs inside the memory layer, and the runtime never touches the network.

## Components

```
smash.py                     CLI shell: arg wiring, output rendering, hooks entry
api.py                      local JSON API runner (127.0.0.1 only; --host refused)
serve.py                    local web viewer with the same loopback API
mcp_package/
  smash_core/                ALL shared logic lives here
    ...
    mcp_verify.py           MCP config generation + `smash`-vs-path command display
  smash_mcp/server.py        MCP tool surface (slim = canonical, full = compat)
...
`smash.py`, `api.py`, and `serve.py` are intentionally standalone-runnable (a workspace
carries copies so `python3 smash.py` works with zero installs); the
`check_runtime_duplication.py` guard keeps root and package logic from
drifting apart.

## Runtime boundary

Phase 0 rule: treat `mcp_package/smash_core/` as the product kernel and every
top-level runtime as a thin shell around it.

- `smash.py`: CLI argument parsing, command dispatch, human-readable output
- `serve.py`: loopback HTML viewer plus local HTTP mutation/read surface
- `api.py`: JSON-only loopback API runner
- `mcp_package/smash_mcp/server.py`: MCP transport and tool exposure
- `apps/SmashBar/`: optional macOS shell; not a source of business logic

If a fix touches ranking, validation, ingest, memory writes, graph logic, or
wiki operations, it belongs in `smash_core`, not duplicated in one surface.
...
source-linked; `.smash-cache/` holds derived state only (semantic index as
...
2. **Automatic** — session-end hooks: transcript text is extracted with
   Smash's own injected output dropped (echo guard layer 1), proposals are
   ...
   sessions are skipped, and everything lands as a _proposal-only capture_
...
_standout_ (z-score vs the corpus, never raw cosine thresholds; semantic-only
...
Phase 0 baseline means these local setups are explicitly understood:

- macOS + Homebrew Python:
  use `smash connect ... --write` / `smash onboard ... --write` when you want
  Smash to provision `~/.smash-mcp-venv` automatically for MCP.
- plain Python venv from a source checkout:
  install test/runtime deps in the active venv, then install the MCP package
  from the repo with `python3 -m pip install -e ./mcp_package`.
- future Docker target:
  intentionally not implemented yet; Phase 0 only documents it as the next
  runtime surface, not a supported baseline.

When someone reports a runtime bug, pin the report to one row in this matrix
...
- **New CLI command**: subparser in `cli_parser.py`, handler in `smash.py`,
  dispatch entry in `main()`, a group in `COMMAND_GROUPS` (guard test fails
  if you forget), tests in `tests/test_smash_cli.py`.
- **New memory field**: frontmatter write in `write_memory_page`, read in
  ...
  keep it out of `memory_claim_text` unless it _is_ claim.
...
- **Release**: `scripts/prepare_release.py <version>` on develop → PR to
  ...
  (ebrahimisoheil/homebrew-Smash). The script prints the full runbook.
