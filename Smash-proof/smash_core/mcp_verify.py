"""Shared MCP verification helpers for Smash."""
...
PREFERRED_LINK_COMMAND = "smash"
LEGACY_LINK_COMMAND = "Smash"
_link_command_override: list[str] | None = None


def set_link_command_override(parts: list[str] | tuple[str, ...] | None) -> None:
    """Override generated Smash CLI commands for the current runtime."""
...
def _configured_link_command() -> list[str]:
    ...
    env_command = os.environ.get("SMASH_CLI_COMMAND", "").strip()
    if env_command:
        return [env_command]
...
def normalize_command_parts(parts: list[str]) -> list[str]:
    """Use Smash's non-conflicting CLI command name in generated user commands."""
...
def check_smash_mcp_import(python_cmd: str) -> dict[str, object]:
    """Check whether smash-mcp and its MCP SDK dependency import in a Python runtime."""
    code = (
        "import json\n"
        ...
        "    status['version'] = getattr(smash_mcp, '__version__', 'unknown')\n"
    ...
    try:
        ...
        return {"installed": False, "version": None, "error": "could not parse smash_mcp import output"}
    return {
...
def python_is_externally_managed(python_cmd: str | None = None) -> bool:
    ...
    arrival — point them at Smash's managed venv instead.
    """
...
SMASH_EXTRAS = ("semantic", "semantic-quality", "rerank")
...
def provision_link_extras(
    ...
    *,
    extras: tuple[str, ...] = SMASH_EXTRAS,
    venv_dir: Path | None = None,
    run: Callable[..., subprocess.CompletedProcess] = subprocess.run,
) -> dict[str, object]:
    """Install smash-mcp with the given extras into Smash's managed venv.

    Used when the runtime python cannot host the optional tiers itself
    (PEP 668). Returns {"ready", "python", "notes"}.
    """
    notes: list[str] = []
    venv_root = venv_dir or (Path.home() / ".smash-mcp-venv")
    venv_python = default_mcp_venv_python(venv_dir)
    spec = f"smash-mcp[{','.join(extras)}]=={expected_version}"
    steps: list[list[str]] = []
...
def default_mcp_venv_python(venv_dir: Path | None = None) -> str:
    """Path to the python inside Smash's standard MCP venv (~/.smash-mcp-venv)."""
    venv = venv_dir or (Path.home() / ".smash-mcp-venv")
    if os.name == "nt":
...
def ensure_smash_mcp_runtime(
    python_cmd: str,
    expected_version: str,
    *,
    provision: bool = False,
    venv_dir: Path | None = None,
    import_check: Callable[[str], dict[str, object]] = check_smash_mcp_import,
    run: Callable[..., subprocess.CompletedProcess] = subprocess.run,
) -> dict[str, object]:
    """Find or create a Python runtime whose smash-mcp matches Smash's version.

    Order: the configured python, then an existing ~/.smash-mcp-venv, then —
    only when `provision` is set — create that venv and install the pinned
    smash-mcp from PyPI. Returns {"ready", "python", "status", "provisioned",
    ...
    steps = (
        ...
        [venv_python, "-m", "pip", "install", "--upgrade", "pip", f"smash-mcp=={expected_version}"],
    )
    for step in steps:
        try:
            result = run(step, check=False, stdout=subprocess.PIPE, stderr=subprocess.PIPE, text=True)
    ...
    if _runtime_ready(final_status, expected_version):
        notes.append(f"provisioned {venv_root} with smash-mcp {expected_version}")
...
def mcp_config(python_cmd: str, wiki_dir: Path) -> dict[str, object]:
    return {
        "mcpServers": {
            "Smash": {
                "command": python_cmd,
                "args": ["-m", "smash_mcp", "--wiki", str(wiki_dir), "--surface", "slim"],
            }
        }
    }
...
def build_mcp_verify_status(
    *,
    ...
    return {
        ...
        "expected_version": expected_version,
        "version_matches": version_matches,
        "smash_mcp": normalized_import_status,
        "wiki": {
            "path": str(wiki_dir),
            "exists": wiki_exists,
        },
...
def mcp_verify_guidance(
    *,
    ...
    if not installed:
        issues.append({
            "code": "smash_mcp_missing",
            "message": "smash-mcp is not importable from the configured Python.",
        })
        next_actions.append(
            mcp_verify_action(
                "install_smash_mcp",
                "Install smash-mcp in the configured Python environment",
                [python_cmd, "-m", "pip", "install", "--upgrade", "smash-mcp"],
            )
        )
    else:
        if not mcp_sdk_ready:
            issues.append({
                ...
                "message": "smash-mcp is installed, but the MCP SDK dependency is missing.",
            })
            next_actions.append(
                mcp_verify_action(
                    "reinstall_smash_mcp",
                    f"Reinstall smash-mcp dependencies for Smash {expected_version}",
                    [python_cmd, "-m", "pip", "install", "--upgrade", f"smash-mcp=={expected_version}"],
                )
            )
        if not version_matches:
            issues.append({"code": "version_mismatch", "message": f"smash-mcp must match Smash {expected_version}."})
            next_actions.append(
                mcp_verify_action(
                    "upgrade_smash_mcp",
                    f"Upgrade smash-mcp to Smash {expected_version}",
                    [python_cmd, "-m", "pip", "install", "--upgrade", f"smash-mcp=={expected_version}"],
                )
            )
    if not wiki_exists:
        issues.append({
            "code": "wiki_missing",
            "message": "The configured Smash wiki directory does not exist.",
        })
        next_actions.append(
            mcp_verify_action(
                "init_wiki",
                "Create or repair the local Smash wiki",
                init_command,
            )
        )
...
def render_mcp_verify_text(status: Mapping[str, object]) -> tuple[int, str]:
    ...
    import_status = status.get("smash_mcp") if isinstance(status.get("smash_mcp"), Mapping) else {}
    ...
    lines = [
        f"Smash MCP verification: {status.get('target', '')}",
        "",
        f"Python: {python_cmd}",
    ]
    if installed:
        lines.append(f"smash-mcp: installed ({import_status.get('version')})")
        if not mcp_sdk_ready:
        ...
        lines.append("smash-mcp: missing")
        error = import_status.get("error")
        if error:
            lines.append(f"Import error: {error}")
    ...
    if not installed:
        if import_status.get("externally_managed"):
            ...
            lines.append("  This Python refuses direct pip installs (PEP 668). Use Smash's managed venv:")
            lines.append("    " + display_command(["smash", "connect", "<agent>", ".", "--write"]) + "  # provisions ~/.smash-mcp-venv automatically")
            lines.append("  Or by hand:")
            lines.append("    python3 -m venv ~/.smash-mcp-venv")
            lines.append("    ~/.smash-mcp-venv/bin/python -m pip install --upgrade pip smash-mcp")
            lines.append("    Then rerun with: " + display_command(["smash", "verify-mcp", ".", "--python", "~/.smash-mcp-venv/bin/python"]))
        else:
            action = _action_by_tool(status, "install_smash_mcp")
            lines.append(f"  Install: {action.get('command_text') or display_command([python_cmd, '-m', 'pip', 'install', '--upgrade', 'smash-mcp'])}")
            lines.append("  macOS/Homebrew fallback:")
            lines.append("    python3 -m venv ~/.smash-mcp-venv")
            lines.append("    ~/.smash-mcp-venv/bin/python -m pip install --upgrade pip smash-mcp")
            lines.append("    Then rerun with: python3 smash.py verify-mcp . --python ~/.smash-mcp-venv/bin/python")
    elif not mcp_sdk_ready:
        action = _action_by_tool(status, "reinstall_smash_mcp")
        lines.append(f"  Reinstall smash-mcp dependencies for Smash {expected_version}:")
        lines.append(f"    {action.get('command_text')}")
    elif not version_matches:
        action = _action_by_tool(status, "upgrade_smash_mcp")
        lines.append(f"  Upgrade smash-mcp to match Smash {expected_version}:")
        lines.append(f"    {action.get('command_text')}")
    if not wiki_exists:
        lines.append("  Create a wiki with an installer, or try: python3 smash.py init")
