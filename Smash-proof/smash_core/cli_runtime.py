"""Text rendering helpers for Smash setup-oriented CLI commands."""
...
def render_init_text(*, target: object, fixes: Sequence[str]) -> tuple[int, str]:
    ...
    lines = [f"Smash wiki ready at {target}"]
    if fixes:
    ...
    lines.extend([
        "",
        "Next:",
        f"  {display_command(['smash', 'health', command_target])}",
        f"  {display_command(['smash', 'onboard', command_target])}",
        f"  {display_command(['smash', 'serve', command_target])}",
        "  Open http://127.0.0.1:3000/onboard for the browser checklist",
        "  Drop sources into raw/ and ask your agent: ingest raw/<file> into Smash",
    ])
    return 0, "\n".join(lines)


def render_starter_prompts_text(payload: Mapping[str, object]) -> tuple[int, str]:
    lines = [f"Smash starter prompts: {payload['target']}"]
    if payload["project"]:
...
def render_start_text(payload: Mapping[str, object]) -> tuple[int, str]:
    ...
    if not isinstance(status, Mapping):
        raise ValueError("Invalid Smash start payload")
    ...
    if not status.get("ready"):
        ...
        if isinstance(next_actions, Sequence) and not isinstance(next_actions, (str, bytes)):
            for item in next_actions[:3]:
                if isinstance(item, Mapping):
                    lines.append(f"- {item.get('label', item.get('tool', 'inspect Smash'))}")
...
def render_welcome_text(payload: Mapping[str, object]) -> tuple[int, str]:
    """Render a short first-use guide for humans trying Smash with an agent."""
    lines = [f"Smash welcome: {payload['target']}"]
    if payload["project"]:
        lines.append(f"Project: {payload['project']}")
    lines.extend([
        "",
...
def render_demo_text(
    *,
    ...
    return 0, "\n".join([
        f"Smash demo created at {target}",
        "",
        "View it:",
        f"  {serve_command}",
        "",
...
def render_proof_text(payload: Mapping[str, object]) -> tuple[int, str]:
    ...
    lines = [
        ...
        f"    {display_command(['smash', 'onboard'])}",
        "    (this proof workspace is a demo — your memory will live at ~/Smash)",
        "",
        "Try it with two agents",
        f"Agent A: {prompts.get('agent_a', 'remember that this project uses Smash')}",
        f"Agent B: {prompts.get('agent_b', 'start with Smash before we continue')}",
        "",
        "No viewer required",
...
def render_onboard_text(payload: Mapping[str, object]) -> tuple[int, str]:
    """Render the guided first-run path for a normal Smash workspace."""
    ...
    lines = [
        f"Smash onboard: {payload.get('target')}",
        "",
        "Workspace",
        ...
        for connection in connections:
            ...
            if state == "preview":
                ...
                if runtime.get("provisioned"):
                    lines.append("  MCP runtime: provisioned ~/.smash-mcp-venv so the agent can start Smash's MCP server.")
                if restart_hint:
...
def render_mcp_connect_text(payload: Mapping[str, object]) -> tuple[int, str]:
    ...
    lines = [
        f"Smash connect: {payload.get('display_name')}",
        "",
    ...
    if runtime:
        smash_mcp = runtime.get("smash_mcp") if isinstance(runtime.get("smash_mcp"), Mapping) else {}
        version = smash_mcp.get("version") or "missing"
        if runtime.get("ready") and runtime.get("provisioned"):
            lines.append(f"MCP runtime: ready — provisioned ~/.smash-mcp-venv (smash-mcp {version})")
        elif runtime.get("ready"):
            lines.append(f"MCP runtime: ready (smash-mcp {version})")
        elif not requested:
            lines.append(
                f"MCP runtime: needs attention (smash-mcp {version}, need {payload.get('expected_version')}); "
                "--write provisions ~/.smash-mcp-venv automatically"
            )
    lines.append("")
    if requested:
...
def render_agent_hooks_text(payload: Mapping[str, object]) -> tuple[int, str]:
    ...
    code = 0 if not requested or ok else 1
    lines = [
        f"Smash session hooks: {payload.get('display_name')}",
        "",
...
def render_session_start_hook_text(payload: Mapping[str, object]) -> tuple[int, str]:
    ...
    lines = [
        "Smash memory (local, source-backed)"
        + (f" · project {project}" if project else ""),
    ]
    if not status.get("ready"):
        lines.extend([
            "Smash is not ready; skipping the memory brief.",
            f"Check with: {display_command(['smash', 'health', target])}",
        ])
        return 0, "\n".join(lines)
    ...
    if (
        ...
        lines.extend([
            ...
            f"{display_command(['smash', 'seed', '.', target])} (source-backed, no durable memory).",
    ...
    if seed_recommended:
        lines.extend([
            ...
            f"from this repo's docs, suggest: {display_command(['smash', 'seed', '.', target])}",
        ])
    ...
    lines.extend([
        ...
        f"For task-specific context: {display_command(['smash', 'query', '<topic>', target, '--budget', 'micro'])} "
        "or the Smash MCP recall tool. Save durable memory only after explicit user approval.",
    ])
    return 0, "\n".join(lines)
