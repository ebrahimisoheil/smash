"""Shared benchmark health helpers for Smash."""
...
def benchmark_value_evidence(cache: Mapping[str, object], packet: Mapping[str, object]) -> dict[str, object]:
    ...
    the bounded Smash query packet against the body text currently indexed in
    the local wiki.
    """
    ...
    notes = [
        ...
        "This does not score answer quality; use it to see whether Smash is reducing context budget waste.",
    ]
    if broad_chars == 0:
...
def build_benchmark_payload(
    ...
    *,
    ...
    """Build benchmark timings and scale metadata for a Smash wiki."""
...
def benchmark_health(payload: Mapping[str, object]) -> dict[str, object]:
    ...
    if warnings:
        ...
        recommendations = [
            "Run smash doctor --fix and smash benchmark again after repairing wiki/index state.",
        ]
...
def render_benchmark_text(payload: Mapping[str, object]) -> str:
    ...
    lines = [
        f"Smash benchmark: {payload.get('target', '')}",
