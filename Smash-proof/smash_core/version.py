...
def workspace_runtime_version(root: Path) -> str | None:
    ...
    Workspaces carry their own runtime (smash.py + smash_core) so hooks run
    ...
    """
    version_file = root / "smash_core" / "version.py"
    if not (root / "smash.py").exists() or not version_file.exists():
        return None
    try:
        text = version_file.read_text(encoding="utf-8", errors="replace")
    except OSError:
        return None
    match = re.search(r'SMASH_VERSION\s*=\s*"([^"]+)"', text)
...
def workspace_runtime_is_older(root: Path, installed: str = SMASH_VERSION) -> str | None:
