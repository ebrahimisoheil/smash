"""HTML helpers for Smash's local memory web views."""
    ...
    *,
    ...
    if capture_warning_count:
        actions.append({
            ...
            "href": "/captures",
            "command": "python3 smash.py redact-capture raw/memory-captures/<capture>.md .",
            "priority": "high",
        })
    if review_count:
        ...
        actions.append({
            ...
            "href": "/inbox",
            "command": "python3 smash.py memory-inbox .",
            "priority": "high",
        })
    if updated_count:
        actions.append({
            ...
            "href": "/memory",
            "command": "python3 smash.py profile .",
            "priority": "medium",
        })
    if archived_count:
        actions.append({
    ...
    if capture_count and not capture_warning_count:
        actions.append({
            ...
            "href": "/captures",
            "command": "python3 smash.py accept-capture raw/memory-captures/<capture>.md . --index 1",
            "priority": "medium",
        })
    if not memory_count:
        actions.append({
            ...
            "href": "",
            "command": 'python3 smash.py remember "User prefers ..." . --type preference --scope user',
            "priority": "high",
        })
    if not actions:
        actions.append({
...
def render_capture_card(capture: dict[str, object]) -> str:
    ...
    # What Accept will save, and how Smash decided — the pipeline made reviewable.
    ...
    if trail:
        ...
        trail_html = (
            '<details class="capture-trail"><summary>How Smash read this session</summary>'
