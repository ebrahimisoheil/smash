"""HTML renderer for Smash's memory proposal page."""
...
def render_propose_page(project: str = "", source: str = "", *, layout: PageLayout) -> str:
    body = (
        '<div class="breadcrumb"><a href="/">Smash</a> / propose</div>'
        '<h1>Propose Memories</h1>'
        '<p class="summary">Paste source notes, session notes, or a raw excerpt. Smash returns memory candidates without writing anything.</p>'
        ...
        '<textarea name="text" placeholder="Paste notes here. Example: I prefer short release notes. We decided to keep Smash local-first."></textarea>'
        '</label>'
...
def _render_proposal_path() -> str:
    return (
        ...
        '<h3>Propose</h3><p>Smash returns candidates only. This step never writes durable memory.</p>'
        ...
        '<code>smash memory-inbox</code></article>'
        '</section>'
    )
...
def _render_after_approval() -> str:
    return (
        ...
        '<a class="button-Smash" href="/inbox">Open memory inbox</a>'
        '<a class="button-Smash" href="/audit">Open memory audit</a>'
        f'{copy_button("start with Smash before we continue", "Copy brief prompt")}'
        f'{copy_button("query Smash for what you remember about this task", "Copy query prompt")}'
        '</div></div></section>'
    )
