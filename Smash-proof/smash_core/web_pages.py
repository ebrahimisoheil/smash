"""HTML helpers for Smash wiki page lists."""
...
def render_wiki_page(
    ...
    *,
    ...
    crumb = '<div class="breadcrumb"><a href="/">Smash</a>'
    if category:
    ...
    if graph_href:
        action_links.append(
            f'<a class="button-Smash" href="{html.escape(graph_href, quote=True)}">Open local graph</a>'
        )
    if proposal_href:
        action_links.append(
            f'<a class="button-Smash" href="{html.escape(proposal_href, quote=True)}">Propose memories</a>'
        )
    if proposal_prompt:
...
def render_all_pages(
    ...
    *,
    ...
    return layout(
        "All Pages",
        '<div class="breadcrumb"><a href="/">Smash</a> / all pages</div>'
...
def render_page_controls(*, total: int, limit: int, offset: int, active_type: str = "") -> str:
    ...
    prev_link = (
        f'<a class="button-Smash" href="{prev_href}">Previous</a>'
        if offset > 0
        else '<span class="button-Smash disabled">Previous</span>'
    )
    next_link = (
        f'<a class="button-Smash" href="{next_href}">Next</a>'
        if next_offset < total
        else '<span class="button-Smash disabled">Next</span>'
    )
