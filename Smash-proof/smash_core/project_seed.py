"""Project seeding helpers for day-one Smash context."""
...
def discover_project_seed_files(project_root: Path, *, limit: int = DEFAULT_FILE_LIMIT) -> list[dict[str, object]]:
    """Return project files Smash can safely consider for a day-one seed."""
    root = project_root.expanduser().resolve()
...
def _adr_decision_candidates(included: list[dict[str, object]], limit: int = 5) -> list[dict[str, str]]:
    ...
    its Decision section and suggests one reviewable `decision` memory. Smash
...
def _format_raw_seed(
    *,
    ...
    parts = [
        ...
        "",
        "This source was generated by `smash seed` from allowlisted project context files.",
...
def _format_source_page(
    *,
    ...
    return "\n".join([
        ...
        "## Summary",
        "",
        f"{summary}{git_note} This page exists so the first Smash recall can return useful project context immediately.",
        "",
        "## Key Context",
        "",
        *file_lines,
...
def _ensure_wiki_structure(root: Path) -> None:
    ...
    if not log_path.exists():
        atomic_write_text(log_path, "# Smash Wiki Log\n\n")
...
def seed_project_context(
    ...
    *,
    ...
    """Seed Smash with source-backed context from common project files."""
    ...
    next_commands = [
        f"smash query \"what is this project about?\" {root}",
        f"smash brief \"working on {title}\" {root}",
        f"smash health {root}",
    ]
    if status == "already_seeded":
        next_commands.insert(0, f"smash seed {project} {root} --overwrite")
    if status == "needs_attention":
        next_commands = [f"redact blocked project files, then rerun: smash seed {project} {root}"]
    elif status == "empty":
    ...
    return {
        ...
        "next_prompt": f"query Smash for {title} project context",
...
def render_seed_project_text(payload: dict[str, object]) -> tuple[int, str]:
    ...
    lines = [
        f"Smash project seed: {payload['project_root']}",
