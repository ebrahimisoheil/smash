"""MCP client configuration helpers for Smash."""
...
class AgentMcpConfig:
    ...
    restart_hint: str = "Restart the agent, then ask: is Smash ready?"
...
def supported_agents() -> tuple[str, ...]:
    """Return canonical agent names supported by `smash connect`."""
...
def _agent_by_name(agent: str) -> AgentMcpConfig:
    ...
    raise ValueError(f"unsupported agent for smash connect: {agent}. Try one of: {choices}")
...
def _json_config(config: AgentMcpConfig, python_cmd: str, wiki_dir: Path) -> dict[str, object]:
    return {
        config.top_key: {
            "Smash": _server_config(config, python_cmd, wiki_dir),
        }
    }
...
def _codex_toml_snippet(python_cmd: str, wiki_dir: Path) -> str:
    return "\n".join([
        ...
        f'args = ["-m", "smash_mcp", "--wiki", {json.dumps(str(wiki_dir))}, "--surface", "slim"]',
    ])
...
def _write_json_config(path: Path, config: AgentMcpConfig, python_cmd: str, wiki_dir: Path) -> None:
    ...
    if not isinstance(existing, dict):
        existing = {}
    existing["Smash"] = _server_config(config, python_cmd, wiki_dir)
...
def _write_codex_config(path: Path, python_cmd: str, wiki_dir: Path) -> None:
    ...
    pattern = re.compile(r"(?ms)^\[mcp_servers\.Smash\]\r?\n.*?(?=^\[|\Z)")
...
def read_agent_link_server(agent: str, config_path: str | None = None) -> dict[str, object]:
    """Read the Smash MCP server an agent is actually configured to run.

    Returns {"agent", "display_name", "config_path", "configured", "python",
    "wiki"}. `configured` is False when the config file or its Smash server
    entry is missing — the caller should point at `smash connect`.
    """
    ...
    args: list[str] = []
    if config.config_format == "codex-toml":
        block = re.search(r"(?ms)^\[mcp_servers\.Smash\]\r?\n(.*?)(?=^\[|\Z)", text)
        if not block:
            return result
        ...
        try:
            ...
            return result
        server = payload.get(config.top_key, {}).get("Smash") if isinstance(payload, dict) else None
...
def build_mcp_connect_payload(
    *,
    ...
    write: bool = False,
    runtime_check: Any = ensure_smash_mcp_runtime,
) -> dict[str, object]:
    """Build or write an MCP client configuration for a supported local agent.

    Before writing, the chosen Python is verified to actually serve smash-mcp
    at Smash's version; if it cannot, Smash falls back to (or provisions)
    ~/.smash-mcp-venv rather than writing a config the agent cannot start.
    """
    config = _agent_by_name(agent)
    ...
    if runtime_ready and chosen_python != resolved_python:
        ...
        if write:
            try:
                atomic_write_text(root / ".smash-mcp-python", resolved_python + "\n")
            except OSError:
                pass
    path = _config_path(config.default_config, config_path)
    ...
    if write and not runtime_ready:
        fix = display_command([resolved_python, "-m", "pip", "install", "--upgrade", f"smash-mcp=={expected_version}"])
        write_status = {
            "requested": True,
            "ok": False,
            "message": (
                f"not written: {resolved_python} cannot serve smash-mcp {expected_version} "
                f"and provisioning ~/.smash-mcp-venv failed. Fix the runtime first: {fix}"
            ),
        }
    elif write:
        try:
    ...
    connect_command = ["smash", "connect", config.name, str(target)]
    if config_path:
    ...
    return {
        ...
        "next_actions": [
            ...
            },
            {
                ...
                "command": ["smash", "verify-mcp", str(target), "--python", resolved_python],
                "command_text": display_command(["smash", "verify-mcp", str(target), "--python", resolved_python]),
            },
            {
