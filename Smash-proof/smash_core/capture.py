"""Shared raw capture helpers for Smash CLI and MCP runtimes."""
...
Captured locally for Smash memory review. This raw note is proposal-only until the user approves durable memories.
...
def resolve_capture_file(root: Path, capture: str, *, max_len: int | None = None) -> Path | None:
    """Resolve a user-provided raw capture path without escaping the Smash root."""
...
def capture_proposal_source(text: str) -> str | None:
    ...
    # Drop the leading explanatory sentence Smash writes above the turns.
    section = re.sub(
...
def capture_decision_trail(text: str) -> list[str]:
    """Return the `## How Smash Read This Session` bullet lines when present."""
    _, body = parse_frontmatter(text)
    match = re.search(r"^## How Smash Read This Session\s*(.*?)(?=^## |\Z)", body, flags=re.MULTILINE | re.DOTALL)
    if not match:
        return []
    return [
        line.lstrip("- ").strip()
...
def cli_capture_commands(rel_path: str, command_target: str | Path = ".") -> dict[str, str]:
    return {
        "accept": _shell_words("python3", "smash.py", "accept-capture", rel_path, command_target, "--index", "1"),
        "redact": _shell_words("python3", "smash.py", "redact-capture", rel_path, command_target),
        "delete": _shell_words("python3", "smash.py", "delete-capture", rel_path, command_target, "--confirm"),
    }
...
def render_accept_capture_text(payload: dict[str, object], *, target: object = ".") -> tuple[int, str]:
    ...
    lines.extend(["", "Next:", f"  {_shell_words('python3', 'smash.py', 'review-memory', result.get('name'), target)}"])
    return 0, "\n".join(lines)
...
def render_delete_capture_text(payload: dict[str, object], *, target: object = ".") -> tuple[int, str]:
    ...
    if payload.get("confirmation_required"):
        return 1, "\n".join([
            ...
            f"Run: {_shell_words('python3', 'smash.py', 'delete-capture', path, target, '--confirm')}",
        ])
