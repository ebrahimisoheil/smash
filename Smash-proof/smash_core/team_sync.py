"""Read-only Git team-sync guidance for Smash workspaces."""
...
def _shared_git_paths(root: Path, *, absolute: bool) -> list[str]:
    """Return share-safe Smash paths for Git commands.
...
def build_team_sync_payload(target: Path, *, remote: str | None = None) -> dict[str, object]:
    """Return a read-only plan for sharing a Smash workspace through Git."""
    ...
    warnings: list[str] = []
    if not wiki_dir.exists():
        warnings.append("Smash wiki is missing. Run smash init before preparing team sync.")
    ...
    sync_actions: list[dict[str, str]] = [
        _action("check Smash health", ["smash", "health", str(root)]),
        _action("review pending memories", ["smash", "memory-inbox", str(root)]),
        _action("validate before sharing", ["smash", "validate", str(root)]),
        _action("backup before sharing", ["smash", "backup", str(root)]),
    ]
    if git_root is None:
        setup_actions.extend([
            ...
            _action("commit shared memory baseline", ["git", "-C", str(root), "commit", "-m", "Initialize Smash shared memory"]),
        ])
        if remote_clean:
        ...
        sync_actions.extend([
            ...
            _action("commit reviewed memory updates", ["git", "-C", str(git_root), "commit", "-m", "Update Smash shared memory"]),
        ])
    ...
    return {
        ...
        "notes": [
            "Share wiki/ and SMASH.md for team agent memory.",
...
def render_team_sync_text(payload: Mapping[str, object]) -> tuple[int, str]:
    ...
    lines = [
        f"Smash team sync: {payload.get('target')}",
        "",
