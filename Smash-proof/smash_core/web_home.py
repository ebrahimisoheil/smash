"""HTML helpers for Smash's local home page."""
...
def render_home_page(
    ...
    *,
    ...
    body = (
        "<h1>Smash</h1><p>Local agent memory. Knowledge compounds here.</p>"
...
def _render_page_sections(pages: Sequence[dict[str, object]], *, page_href: PageHref) -> str:
    ...
    if not categories:
        return (
            ...
            f"<li>{copy_button('ingest the new raw Smash files', 'Copy ingest prompt')}</li>"
            "</ul></div>"
        )
...
def _render_product_lanes() -> str:
    return (
        '<div class="product-lanes" aria-label="How Smash stores context">'
        '<section class="product-lane"><h2>1. Sources become wiki knowledge</h2>'
        '<p>Drop files into <code>raw/</code> and say <code>ingest raw/file.md into Smash</code>. '
        'Smash creates source-backed pages, concepts, backlinks, index entries, and logs.</p></section>'
        ...
        '<p>Ask <code>query Smash for ...</code> or open a memory brief. Smash combines reviewed memory, wiki pages, and graph context.</p></section>'
        '</div>'
    )
...
def _render_prompt_strip(starter_prompts: Mapping[str, object]) -> str:
    ...
    return (
        '<section class="prompt-strip" aria-label="First Smash prompts">'
        '<h2>Try These Prompts</h2>'
        '<p>Ask from Codex, Claude, Cursor, Kiro, or any agent with Smash installed. <a href="/prompts">Open starter prompts</a>.</p>'
