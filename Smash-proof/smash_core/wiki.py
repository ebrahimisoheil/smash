"""Shared wiki indexing, search, context, and graph helpers for Smash."""
...
def _persistent_cache_path(wiki_dir: Path) -> Path:
    return wiki_dir.parent / ".smash-cache" / f"wiki-cache-v{PERSISTENT_CACHE_SCHEMA_VERSION}.json"


def _persistent_fts_path(wiki_dir: Path) -> Path:
    return wiki_dir.parent / ".smash-cache" / "page-fts-v1.sqlite"
...
def page_link_summary(
    ...
    return {
        ...
        "forward": forward,
        "agent_guidance": [
            "This page Smash list may be paginated for context safety.",
...
def build_index_markdown(
    ...
    """Build a deterministic, human-readable catalog for a Smash wiki."""
            ...
            "next_actions": [
                {
                    ...
                    "command": "smash rebuild-backlinks",
