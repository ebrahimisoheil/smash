"""Small safe Markdown renderer used by the local Smash web UI."""
...
def inline_markdown(text: str, page_href: Callable[[str], str] = default_page_href) -> str:
    """Render the inline subset Smash supports while escaping unsafe HTML."""
    html_spans: list[str] = []
...
def markdown_to_html(markdown: str, page_href: Callable[[str], str] = default_page_href) -> str:
    """Render Smash's intentionally small wiki Markdown subset to HTML."""
    out: list[str] = []
    in_code = False
    in_table = False
    in_list = False
