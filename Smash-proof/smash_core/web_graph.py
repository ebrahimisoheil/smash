...
(function() {{
  ...
  var copyLinkButton = document.getElementById('graph-copy-Smash');
  ...
  var nodeById = {{}};
  var GRAPH_STORAGE_KEY = 'Smash.graph.controls.v1';
  ...
  async function copyGraphLink() {{
    ...
    try {{
      ...
      copyLinkButton.textContent = 'Copied Smash';
    }} catch (error) {{
      copyLinkButton.textContent = 'Copy failed';
    }}
    window.setTimeout(function() {{ copyLinkButton.textContent = previous || 'Copy Smash'; }}, 1400);
  }}
  ...
  function updateInspector() {{
    ...
    neighbors.slice(0, 10).forEach(function(id) {{
      ...
      var Smash = document.createElement('a');
      Smash.href = pageHref(id);
      Smash.textContent = target ? target.title : id;
      inspectorLinks.appendChild(Smash);
    }});
...
def render_graph_empty_body() -> str:
    ...
    return (
        '<div class="breadcrumb"><a href="/">Smash</a> / graph</div>'
        "<h1>Knowledge Graph</h1>"
        '<div class="graph-empty">'
        "<strong>No graph pages yet.</strong><br>"
        "Add sources, ingest them, then rebuild backlinks."
        '<div class="page-actions">'
        '<a class="button-Smash" href="/ingest">Open ingest</a>'
        f"{copy_button('ingest the new raw Smash files', 'Copy ingest prompt')}"
        "</div>"
        "</div>"
    )
...
def render_graph_page_body(
    *,
    ...
    return (
        ...
        '<button id="graph-copy-Smash" type="button">Copy Smash</button>'
        f"{load_full_button}"
