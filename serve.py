#!/usr/bin/env python3
"""Smash — local wiki viewer. python serve.py → http://127.0.0.1:3000"""
...
from smash_core.frontmatter import (
...
from smash_core.markdown import (
...
from smash_core.web_assets import CSS  # noqa: F401 - kept as serve.CSS for tests and compatibility
from smash_core.web_memory import (
...
from smash_core.web_memory_pages import (
...
from smash_core.web_layout import (
...
from smash_core.web_graph import (
...
from smash_core.web_proposals import (
...
LOCAL_ACTION_HEADER = "X-Smash-Local-Action"
...
def _capture_review_summary(project: str | None = None, limit: int = 3) -> dict[str, object]:
    ...
    summary["command"] = f'python3 smash.py capture-inbox "{WIKI_DIR.parent}"{project_arg}'
    return summary
...
def _render_more():
    links = [
        ...
        ("/profile", "Profile", "See what Smash remembers by type, scope, status, and recency."),
        ("/wins", "Wins", "Show local proof signals for what Smash memory is carrying."),
    ...
    return _layout(
        "More",
        '<div class="breadcrumb"><a href="/">Smash</a> / more</div>'
        "<h1>More Tools</h1>"
        '<p class="summary">Advanced Smash views for prompts, proposal review, audit, captures, and full wiki browsing.</p>'
...
def _render_page(page_path):
    ...
    query_prompt = f"query Smash for {title}"
...
def _migrate_wiki_payload() -> dict[str, object]:
    try:
        payload = _core_migrate_wiki(WIKI_DIR)
    except OSError as exc:
        return {"ok": False, "migrated": False, "error": f"Could not migrate wiki: {exc}"}
    _invalidate_pages_cache()
    return payload


def _seed_project_payload(payload: dict[str, object]) -> tuple[dict[str, object], int]:
    project_root = _clean_text_input(payload.get("project_root"), max_len=1000)
    if not project_root:
        return {"wrote": False, "error": "project_root required"}, 400
    try:
        result = _core_seed_project_context(
            WIKI_DIR.parent,
            Path(project_root),
            project_name=_clean_text_input(payload.get("project"), max_len=80) or None,
            overwrite=bool(payload.get("overwrite")),
            dry_run=False,
            rebuild=True,
        )
    except ValueError as exc:
        return {"wrote": False, "error": str(exc)}, 400
    except OSError as exc:
        return {"wrote": False, "error": f"Could not seed project: {exc}"}, 500
    if result.get("wrote"):
        _invalidate_pages_cache()
    return result, 200
...
def _api_discovery_payload() -> dict[str, object]:
    return _core_local_api_discovery_payload(API_VERSION)
...
class Handler(http.server.BaseHTTPRequestHandler):
    ...
    def do_OPTIONS(self):
        ...
        if not self._require_allowed_host():
            return
        self._json(
            {"error": "CORS preflight is not supported; Smash is localhost-only"},
            status=405,
    ...
    def _handle_api_post(self, path: str) -> None:
        if path == "/api/validate":
            payload = self._read_json_or_reply({"passed": False})
            if payload is None:
                return
            strict = bool(payload.get("strict"))
            result = _validate_wiki_payload(strict=strict)
            self._json(result, status=200 if result.get("passed") else 422)
            return
        if path == "/api/migrate":
            if not self._require_local_action_header({"migrated": False}):
                return
            if self._read_json_or_reply({"migrated": False}) is None:
                return
            result = _migrate_wiki_payload()
        ...
        if path == "/api/rebuild-backlinks":
            ...
            return
        if path == "/api/seed-project":
            if not self._require_local_action_header({"wrote": False}):
                return
            payload = self._read_json_or_reply({"wrote": False})
            if payload is None:
                return
            result, http_status = _seed_project_payload(payload)
    ...
    def _handle_api_get(self, path: str, query: dict[str, list[str]]) -> None:
        ...
        elif path == "/api/seed-project":
            self._json({"error": "use POST with JSON body: {\"project_root\": \"...\"}"}, status=405)
    ...
    def _method_not_allowed(self) -> None:
        ...
        if not self._require_allowed_host():
            return
        self._json(
            {"error": "method not allowed; Smash supports GET, HEAD, and POST"},
            status=405,
            headers={"Allow": "GET, HEAD, POST"},
        )
...
class APIHandler(Handler):
    """Dedicated JSON-only loopback API surface for Smash."""
    ...
    def do_GET(self):
        ...
        self._json(
            {
                "error": "API runner only serves /api endpoints",
                "discovery": "/api",
                "available": {
                    "read": list(_CORE_LOCAL_API_READ_ENDPOINTS),
                    "write": list(_CORE_LOCAL_API_WRITE_ENDPOINTS),
                },
            },
            status=404,
        )
...
def _parse_serve_args(argv: list[str], default_port: int = PORT, default_root: Path = ROOT) -> tuple[int, Path]:
    ...
    while index < len(argv):
        ...
        if arg in {"--host", "--bind"} or arg.startswith("--host=") or arg.startswith("--bind="):
            raise SystemExit("Smash serve is local-only; host/bind options are not supported.")
        if arg == "--port":
            if index + 1 >= len(argv):
        ...
        raise SystemExit(
            ...
            "Use 'python serve.py --root /path/to/Smash' or 'smash serve /path/to/Smash'."
        )
...
def _serve_bind_error_message(exc: OSError, port: int) -> str:
    if exc.errno in {errno.EADDRINUSE, 48, 98}:
        ...
        return (
            f"Smash could not start because 127.0.0.1:{port} is already in use.\n"
            f"Try another port, for example: python serve.py --port {next_port}"
        )
    return f"Smash could not start local server on 127.0.0.1:{port}: {exc}"
...
def _api_startup_lines(port: int) -> list[str]:
    base_url = f"http://127.0.0.1:{port}"
    return [
        f"  Smash API -> {base_url}/api",
        "  Recommended endpoints:",
        f"    {base_url}/api/health         readiness and validation",
        f"    {base_url}/api/query-Smash    answer-ready context",
        f"    {base_url}/api/graph-summary  bounded graph overview",
        "  Local-only: bound to 127.0.0.1; no public host mode.",
        "  No auth: do not expose this server without your own authentication layer.",
        "  This runner serves JSON only; use serve.py for the HTML viewer.",
    ]
...
def run_local_server(
    *,
    ...
    startup_lines: Callable[[int], list[str]] = _serve_startup_lines,
) -> int:
    ...
    try:
        with ThreadingLocalTCPServer(("127.0.0.1", PORT), handler_class) as server:
...
def main():
    ...
    raise SystemExit(run_local_server(port=port, root=root))
