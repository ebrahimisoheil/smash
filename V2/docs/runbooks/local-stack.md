...
API is at `http://localhost:${ENGRAVE_API_PORT:-3000}`, web at
`http://localhost:${ENGRAVE_WEB_PORT:-3001}`, MinIO API at
`http://localhost:${ENGRAVE_MINIO_PORT:-9000}`, and the MinIO console at
`http://localhost:${ENGRAVE_MINIO_CONSOLE_PORT:-9001}`.
...
The Engrave namespace is a clean local cutover. Compose uses fresh
`engrave_*` volumes; any existing `smash_*` volumes are left untouched and are
not adopted automatically. Export/import is required to carry old local data
forward.
