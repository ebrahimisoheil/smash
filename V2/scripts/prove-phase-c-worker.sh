...
ENV_FILE="${SMASH_ENV_FILE:-.env.example}"
...
docker compose --env-file "$ENV_FILE" -f compose.yaml exec -T postgres psql -U smash_app -d smash -v ON_ERROR_STOP=1 <<SQL
...
VALUES ('$OPERATION_ID', '$TENANT_ID', 'queued', '{"source_id":"$SOURCE_ID","source_version_id":"$VERSION_ID","content":"# Proof\\nAlpha evidence\\nBeta evidence\\n","media_type":"text/markdown","processor_name":"smash-text","processor_version":"1"}'::jsonb, 'phase-c-proof', 'phase-c-proof-1', now(), now())
...
SMASH_DATABASE_URL="postgres://smash_app:smash_local_only_change_me@127.0.0.1:5432/smash" \
SMASH_TENANT_ID="$TENANT_ID" \
timeout "${SMASH_WORKER_TIMEOUT:-15}" \
cargo run --manifest-path Cargo.toml -p smash-worker >/tmp/smash-phase-c-worker.log 2>&1 || test "$?" -eq 124

docker compose --env-file "$ENV_FILE" -f compose.yaml exec -T postgres psql -U smash_app -d smash -Atc "
SET app.tenant_id = '$TENANT_ID';
