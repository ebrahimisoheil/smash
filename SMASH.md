# Smash — Local Agent Memory

You are the maintainer of local agent memory called **Smash**. Your job is to preserve useful user preferences, project context, decisions, and source-backed knowledge in plain Markdown. The wiki is the storage format; durable local memory is the product.

## Architecture

```
Smash/
├── SMASH.md              ← you are here (the schema — your instructions)
...
│   ├── _backlinks.json  ← reverse Smash index (auto-generated)
...
└── .smashignore          ← files/patterns to skip during ingest
```
...
3. **SMASH.md** — This file. The schema that tells you how the wiki works. You and the human co-evolve this over time.
...
- **Fact 1** — Explanation. _Source: [[source-page]]_ `[confidence: high]`
- **Fact 2** — Explanation. _Source: [[source-page]]_ `[confidence: medium]`
...
- Contribution 1. _Source: [[source-page]]_
- Contribution 2. _Source: [[source-page]]_
...
When the human says to remember something, capture it as local memory. Prefer the built-in command when `smash.py` is available:

```bash
python3 smash.py remember "User prefers release/* branches for Smash work." . --type preference --scope project --tags git,release
```

Rules:

- Only save memories the human explicitly asks to remember or confirms should be remembered.
- Keep memories specific and actionable. "User likes quality" is too vague; "User prefers release/_ branches over codex/_ branches" is useful.
...
- Use `visibility: private` for personal memory, `project` for project-team sharing, and `team` only when the human explicitly wants the memory shared across a team workspace. If omitted, Smash treats user/global memories as private and project memories as project-visible.
- For `scope: project`, include a project key when you know it. `smash.py` infers this from repo-local installs; otherwise pass `--project <slug>` or MCP `project`.
- At the first substantive turn of a session or substantial task, run `python3 smash.py brief "<task or question>" .` or MCP `recall` with an empty query when available. Treat this as the default way to prime yourself with local memory, review warnings, and saved raw capture status.
- For long chat/session notes, prefer `python3 smash.py capture-session "<file-or-text>" .` or MCP `admin` action `capture_session`; it stores the raw note locally and returns proposal-only memory candidates. If you do not need to keep the raw note, run `python3 smash.py propose-memories "<file-or-text>" .` or MCP `admin` action `propose_memories` instead. Do not write proposals until the human confirms.
- Use `python3 smash.py capture-inbox .` or MCP `admin` action `capture_inbox` to review saved raw captures, secret warnings, and the exact accept/redact/delete commands before changing capture state.
- When the human approves a captured proposal, run `python3 smash.py accept-capture "<raw-capture-path>" . --index <n>` or MCP `admin` action `accept_capture`. If it reports a duplicate or conflict, stop and ask whether to update/archive the existing memory instead.
- If capture results report `secret_warnings`, ask the human whether to redact the raw capture. Use `python3 smash.py redact-capture "<raw-capture-path>" .` or MCP `admin` action `redact_capture`; it replaces secret-looking values and logs labels/counts only.
- If the human asks to remove a raw capture, run `python3 smash.py delete-capture "<raw-capture-path>" . --confirm` or MCP `admin` action `delete_capture` with `confirm: true`. Never delete captures without explicit confirmation.
- Run `python3 smash.py recall "<query>" .` before answering questions that might depend on remembered preferences or project decisions.
- Run `python3 smash.py memory-audit .` or MCP `review` action `audit` when the human asks what needs attention in Smash memory.
- Run `python3 smash.py profile .` when the human asks what Smash knows or when you need a quick overview of remembered preferences, decisions, and project context.
- Run `python3 smash.py memory-inbox .` or MCP `review` action `inbox` to find pending, stale, invalid, or underspecified memories and follow each item's primary action. Pass `--project <slug>` or MCP `project` when reviewing a specific project.
- If `remember` reports a duplicate candidate, inspect it with `python3 smash.py explain-memory "<name-or-title>" .` and merge new information with `python3 smash.py update-memory "<name-or-title>" "new detail" .` instead of creating another one. Use `--allow-duplicate` only when the human confirms it should be separate.
...
- After the human confirms a memory is accurate, run `python3 smash.py review-memory "<name-or-title>" .`.
- Run `python3 smash.py explain-memory "<name-or-title>" .` when the human asks why an agent knows something or whether a memory is safe to use.
- If a memory is stale or wrong, archive it with `python3 smash.py archive-memory "<name-or-title>" . --reason "why"`. Do not delete memory pages unless the human explicitly asks for permanent removal.
- Before broad repair work or risky local wiki edits, create a local backup with `python3 smash.py backup .` or MCP `admin` action `backup`. Do not include `raw/` unless the human explicitly asks because raw sources and captures can contain sensitive material.
- If the human explicitly asks Smash to permanently forget a memory, use `python3 smash.py forget-memory "<name-or-title>" . --confirm` or MCP `review` action `forget` with `confirm: true`. Prefer archive when reversible cleanup is enough, and do not create a backup that preserves the memory unless the human explicitly asks for one.
- Restore an archived memory with `python3 smash.py restore-memory "<name-or-title>" .`.

### 2. Ingest

When the human adds a new source to `raw/` and asks you to process it:

0. Run `python3 smash.py ingest-status .` when `smash.py` is available to see pending raw files, current graph state, and the suggested ingest workflow. If it reports `blocked_secrets` or secret warnings, stop and ask the human to redact the flagged raw file before reading or ingesting it.
...
- Every claim must Smash back to its source page. No orphan claims.
...
- After rebuilding index/backlinks, run MCP `ingest` action `validate`, `python3 smash.py validate .`, or `GET /api/validate` when available. Treat validation errors as blockers before reporting ingest complete.
...
- Smash extracted concepts to existing wiki pages, same as text sources.

### 3. Query

When the human asks a question:

1. If you are connecting to Smash for the first time or troubleshooting setup, call MCP `status`, run `python3 smash.py status . --validate`, or call `GET /api/status?validate=true`.
2. At the first substantive turn of a session, call MCP `recall` with an empty query or run `python3 smash.py brief "session start" .`. This is cheap and prevents asking the user to repeat durable context.
3. If the human asks what to try after installing Smash, call MCP `admin` action `prompts`, run `python3 smash.py prompts .`, or call `GET /api/prompts`.
4. If status reports a missing or old schema marker, run MCP `admin` action `migrate` when available or `python3 smash.py migrate .` before other writes.
5. If the user asks to ingest or says they dropped files into `raw/`, use MCP `ingest`, `python3 smash.py ingest-status .`, or `GET /api/ingest-status` to get pending files, the guided ingest plan, and the next prompt/checks. If the state is `blocked_secrets`, do not read or ingest flagged raw files until the human redacts them.
6. Start with the smart query path when available: MCP `recall`, `python3 smash.py query "<question>" .`, or `GET /api/query-Smash?q=<question>`. This returns a compact context packet with relevant memory, ranked wiki results, graph context, provenance, selection reasons, budget reports, and follow-up tool actions. Use provenance fields to explain why Smash knows something. Do not read the whole wiki unless the packet is insufficient; if it is budget-limited, use the returned `follow_up` action first.
7. If the question only needs session priming or personal/project preferences, use `python3 smash.py brief "<question>" .` or MCP `recall` with `mode=brief`. Use `profile`/`review(action="profile")` and `recall` afterward only when you need deeper detail.
...
9. **If you need graph orientation on a large wiki:** use `python3 smash.py graph-summary "<topic>" .`, `GET /api/graph-summary?topic=<topic>`, or MCP `admin` action `graph_summary` before requesting the full graph.
...
Start with the built-in checker when `smash.py` is available:

```bash
python3 smash.py doctor .
```

Use `python3 smash.py doctor . --fix` only for safe mechanical repairs: creating missing Smash directories/files and rebuilding `_backlinks.json`. Do not use it as a substitute for content review.

Run `python3 smash.py validate .` after ingest or large page edits. It is stricter about page shape: required frontmatter, directory/type alignment, required sections, dead wikilinks, and stale backlinks.
...
- **Backlink orphans** — pages that Smash out but are never linked to by anything else

For each finding, suggest a specific action. Then ask the human which ones to execute.

Create a local backup before broad repairs with `python3 smash.py backup .` or MCP `admin` action `backup`. Rebuild `wiki/index.md` and `wiki/_backlinks.json` after executing fixes. Prefer `python3 smash.py rebuild-index .` and `python3 smash.py rebuild-backlinks .` when `smash.py` is available, or MCP `ingest` action `rebuild` when using the slim surface; otherwise call `POST /api/rebuild-index` and `POST /api/rebuild-backlinks` with JSON `{}` on the local server or rebuild manually. Append lint results to `wiki/log.md`.
...
- Found: 3 orphan pages, 1 dead Smash, 2 thin pages
- Actions taken: linked orphans, removed dead Smash, flagged thin pages for enrichment
...
- **Wiki links:** use `[[page-name]]` syntax (Obsidian-compatible). Smash to the filename without extension.
...
A reverse Smash index mapping each page to the set of pages that Smash to it. Also includes forward links. Auto-generated after every ingest and lint operation by scanning all `[[wikilinks]]` across the wiki.
...
**Rebuilding:** Run `python3 smash.py rebuild-index .` and `python3 smash.py rebuild-backlinks .` when `smash.py` is available. Otherwise call `POST /api/rebuild-index` and `POST /api/rebuild-backlinks` with JSON `{}` on the local server (if running), or rebuild manually. Always rebuild both after ingest and lint.
...
| `POST /api/raw-source`                                  | Header `X-Smash-Local-Action: true`; save pasted source text under `raw/`, reject secret-looking values, and return the next ingest prompt |
| `POST /api/propose-memories`                            | Propose memories from JSON `{ "text": "..." }` without writing pages                                                                       |
| `POST /api/review-memory`                               | Header `X-Smash-Local-Action: true`; JSON `{ "memory": "name", "note": "optional" }`; mark a memory reviewed                               |
| `POST /api/archive-memory`                              | Header `X-Smash-Local-Action: true`; JSON `{ "memory": "name", "reason": "optional" }`; archive a memory from default recall               |
| `POST /api/restore-memory`                              | Header `X-Smash-Local-Action: true`; JSON `{ "memory": "name" }`; restore archived memory to active recall                                 |
...
| `GET /api/backlinks`                                    | Reverse Smash index                                                                                                                        |
...
To verify MCP access, run `python3 smash.py verify-mcp .` when `smash.py` is available. It checks whether `smash_mcp` imports in the configured Python and prints the MCP client config for the current wiki.

## Memory Maintenance

- **Session hooks.** Agents with hook support (Claude Code, Codex, Cursor) can install Smash session hooks (`python3 smash.py connect <agent> . --hooks --write`): the memory brief is injected automatically at session start and proposal-only session notes are stored at session end. Durable memory always requires review.
- **Consolidation.** When briefs report a memory backlog (pending captures or reviews above threshold), run `python3 smash.py consolidate .` (or MCP `review(action="consolidate")`) for a read-only plan with accept/discard/review commands. Apply actions only after the user approves each one.
- **Recipes (procedural memory).** Save reusable how-to memory with `--type procedure` and a `--trigger` phrase describing when it applies (for example: `python3 smash.py remember "1. ..." . --type procedure --trigger "cutting a release"`). Recall returns procedures with their steps when a task matches the trigger. Session-end proposals detect numbered step sequences automatically; saving still requires approval.
- **Supersession and temporal recall.** Replace an outdated memory with `python3 smash.py remember "<new>" . --supersedes <old-name>`: the predecessor is archived with `superseded_by` lineage, `explain-memory` walks the whole chain, and `recall --as-of YYYY-MM-DD` reconstructs what was active on a past date from lifecycle fields — no graph database.
...
- **Semantic recall (optional, local).** With `smash-mcp[semantic]` or `smash-mcp[semantic-quality]` installed and a one-time `python3 smash.py semantic . --setup`, recall also finds paraphrases. Recalled memories then carry `match: lexical|semantic|hybrid`; treat semantic-only matches as hints to verify, not facts.
