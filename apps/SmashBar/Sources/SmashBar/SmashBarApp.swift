...
/// SmashBar — the review gate in your menu bar.
///
/// Smash's promise is that nothing becomes durable memory without your
/// approval; SmashBar makes approving ambient instead of a chore. The icon
/// is Smash's memory-graph mark, the badge is everything pending your
/// judgment (memories to review + captures to accept), the popover is the
/// inbox. The `smash` CLI's --json output is the entire backend.
@main
struct SmashBarApp: App {
    @StateObject private var store = SmashStore()
        ...
        MenuBarExtra {
            ...
            HStack(spacing: 3) {
                ...
                if store.pendingCount > 0 {
                    ...
                    // No pending reviews, but a Smash surface needs attention
    ...
    static func notifyTestIfRequested() {
        let marker = "/tmp/smashbar-notify-probe"
        ...
        NotificationManager.shared.probeAuthorization { result in
            ...
            try? line.write(toFile: "/tmp/smashbar-notify-result.txt", atomically: true, encoding: .utf8)
    ...
    @MainActor
    static func snapshotIfRequested(store: SmashStore) {
    ...
    /// The Smash mark as a template image so it adapts to menu bar appearance.
