    ...
    private var field: some View {
        HStack(spacing: 10) {
            Image(systemName: isRemember ? "plus.circle.fill" : "magnifyingglass")
                .font(.system(size: 15))
                .foregroundStyle(isRemember ? AnyShapeStyle(SmashBrand.rust) : AnyShapeStyle(.secondary))
                ...
                Button {
                    ...
                    HStack(alignment: .firstTextBaseline, spacing: 8) {
                        Text(index == 0 ? "\u{21A9}" : "\u{00B7}")
                            ...
                            .foregroundStyle(index == 0 ? AnyShapeStyle(SmashBrand.rust) : AnyShapeStyle(.tertiary))
                            .frame(width: 12)
