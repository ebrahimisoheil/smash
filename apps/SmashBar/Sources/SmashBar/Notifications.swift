...
final class NotificationManager: NSObject, UNUserNotificationCenterDelegate {
    ...
    private let acceptAction = "SMASH_ACCEPT"
    private let reviewAction = "SMASH_REVIEW"
    private let captureCategory = "SMASH_CAPTURE"
    private weak var store: SmashStore?
    ...
    /// once SmashBar ships signed + notarized.
    private var authorized = false

    func install(store: SmashStore) {
        self.store = store
    ...
    private func post(for capture: CaptureItem) {
        ...
        content.title = "Smash captured a memory"
    ...
    // Show banners even while SmashBar is frontmost (it's a menu-bar agent).
    func userNotificationCenter(_ center: UNUserNotificationCenter,
