...
/// SmashBar's design tokens: native macOS materials carry the structure,
/// Smash's rust carries the opinion, one serif wordmark is the signature.
enum SmashBrand {
    /// One source of truth for the app version shown in footer + settings.
    static let version = "1.0"

    /// Rust — Smash's accent. Lifted and desaturated slightly in dark mode
        ...
        HStack(spacing: 6) {
            ...
            if let count, count > 0 {
                Text("\(count)")
                    ...
                    .background(highlight ? AnyShapeStyle(SmashBrand.rust) : AnyShapeStyle(.quaternary))
                    .clipShape(Capsule())
            }
        }
    }
...
/// The Smash wordmark: the one serif moment in the app.
struct Wordmark: View {
    var body: some View {
        HStack(spacing: 7) {
            Circle()
                .fill(SmashBrand.rust)
        ...
        HStack(alignment: .bottom, spacing: 3) {
            ForEach(Array(values.enumerated()), id: \.offset) { _, value in
                RoundedRectangle(cornerRadius: 1.5)
                    .fill(value > 0 ? AnyShapeStyle(SmashBrand.rust.opacity(0.55 + 0.45 * Double(value) / Double(peak)))
...
/// A colored health dot for a Smash surface (green / amber / red / neutral).
