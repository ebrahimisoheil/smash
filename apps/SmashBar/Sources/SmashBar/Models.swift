...
/// Decodable views over `smash ... --json` payloads. Tolerant on purpose:
...
extension Date {
    /// Parse Smash's UTC stamps ("2026-07-12T18:00:00Z").
    static func fromSmashStamp(_ stamp: String) -> Date? {
...
struct LogEntry: Decodable, Identifiable {
    ...
    var date: Date? { Date.fromSmashStamp(timestamp) }
}
...
/// `smash verify-mcp --json`: is the MCP server reachable and version-matched?
struct MCPVerify: Decodable {
    ...
    enum CodingKeys: String, CodingKey {
        ...
        case linkMcp = "smash_mcp"
        case nextActions = "next_actions"
    }
}

/// `smash semantic --json`: which recall power is active (lexical/fast/quality + rerank).
...
/// One row in the Status dashboard: a Smash surface and its live health.
