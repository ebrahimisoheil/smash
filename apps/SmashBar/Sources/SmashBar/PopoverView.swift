...
struct PopoverView: View {
    ...
    @EnvironmentObject var store: SmashStore
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 0) {
            header
                .padding(.horizontal, SmashBrand.pad)
                .padding(.top, SmashBrand.pad)
            tabBar
                .padding(.horizontal, SmashBrand.pad)
                .padding(.top, 10)
            ...
            HStack(spacing: 4) {
                ...
                if badge > 0 {
                    Text("\(badge)")
                        ...
                        .background(SmashBrand.rust, in: Capsule())
                } else if alert {
                    Circle().fill(Color(red: 0.90, green: 0.62, blue: 0.20)).frame(width: 6, height: 6)
                }
            }
            .foregroundStyle(active ? AnyShapeStyle(SmashBrand.rust) : AnyShapeStyle(.secondary))
            .padding(.horizontal, 9).padding(.vertical, 4)
            .background(active ? AnyShapeStyle(SmashBrand.rust.opacity(0.12)) : AnyShapeStyle(.clear),
    ...
    private var inboxTab: some View {
        VStack(alignment: .leading, spacing: SmashBrand.betweenSections) {
    ...
    private var memoryTab: some View {
        VStack(alignment: .leading, spacing: SmashBrand.inGroup) {
            HStack(spacing: 5) {
    ...
    private func filterChip(_ label: String, _ value: String?) -> some View {
        ...
        return Button { memoryTypeFilter = value } label: {
            Text(label)
                ...
                .padding(.horizontal, 8).padding(.vertical, 3)
                .background(active ? AnyShapeStyle(SmashBrand.rust) : AnyShapeStyle(.quaternary.opacity(0.5)), in: Capsule())
        }.buttonStyle(.plain)
    }
            ...
            VStack(alignment: .leading, spacing: 2) {
                HStack(spacing: 6) {
                    ...
                    if !page.supersededBy.isEmpty {
                        ...
                        Text("review")
                            .font(.system(size: 9)).foregroundStyle(SmashBrand.rust)
                    }
                    Text(page.modified.relativeLabel)
                        .font(.system(size: 9.5)).foregroundStyle(.quaternary)
                }
                HStack(spacing: 6) {
                    Text(page.memoryType).font(.system(size: 10)).foregroundStyle(SmashBrand.rust)
    ...
    // MARK: status tab (health of every Smash surface)

    private var statusTab: some View {
        VStack(alignment: .leading, spacing: SmashBrand.inGroup) {
            HStack {
                SectionHeader(title: "Smash status")
                Spacer()
    ...
    private func runtimeBanner(_ warning: String) -> some View {
        HStack(alignment: .top, spacing: 8) {
            ...
            Button("Fix") { store.repairRuntime() }
                .controlSize(.small)
                .tint(SmashBrand.rust)
                .help("Refresh the workspace runtime (smash init)")
        }
        .padding(10)
                ...
                Image(systemName: "plus.circle.fill")
                    ...
                    .foregroundStyle(
                        query.trimmingCharacters(in: .whitespaces).isEmpty
                            ? AnyShapeStyle(.quaternary) : AnyShapeStyle(SmashBrand.rust)
                    )
            }
            .buttonStyle(.borderless)
                        ...
                        VStack(alignment: .leading, spacing: 2) {
                            ...
                            HStack(spacing: 6) {
                                Text(item.memoryType)
                                    ...
                                    .foregroundStyle(SmashBrand.rust)
                        ...
                        VStack(alignment: .leading, spacing: 2) {
                            ...
                            if let trail = capture.decisionTrail, !trail.isEmpty {
                                DisclosureGroup {
                                    ...
                                    Text("How Smash read this session")
                                        .font(.caption2)
                                        .foregroundStyle(SmashBrand.rust)
                                }
                            }
    ...
    /// fortnight's pulse, and the most recent thing Smash did.
        ...
        VStack(alignment: .leading, spacing: SmashBrand.inGroup) {
            ...
            HStack(alignment: .center, spacing: 10) {
                ...
                Image(systemName: "checkmark.seal")
                    ...
                    .foregroundStyle(SmashBrand.rust.opacity(0.75))
    ...
    private var footer: some View {
        VStack(alignment: .leading, spacing: 8) {
            ...
            Divider()
            HStack {
                Text(SmashCLI.workspace)
                    .font(.system(size: 10.5))
                    .foregroundStyle(.quaternary)
                    .lineLimit(1)
                    .truncationMode(.head)
                Spacer()
                Text("SmashBar \(SmashBrand.version)" + (store.smashVersion.isEmpty ? "" : " · Smash \(store.smashVersion)"))
                    .font(.system(size: 10.5))
                    .foregroundStyle(.tertiary)
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 14) {
            ...
            VStack(alignment: .leading, spacing: 2) {
                ...
                Text("Set SMASH_WORKSPACE to point SmashBar at a different workspace.")
                    .font(.caption2)
                    .foregroundStyle(.tertiary)
            }
            ...
            VStack(alignment: .leading, spacing: 2) {
                ...
                Text("SmashBar refreshes the moment your workspace changes — session hooks, agents, and edits all show up instantly.")
                    .font(.caption)
                    .foregroundStyle(.secondary)
            }

            HStack {
                if !store.smashVersion.isEmpty {
                    Text("SmashBar \(SmashBrand.version) · Smash \(store.smashVersion)")
                        .font(.caption)
                        .foregroundStyle(.tertiary)
                }
                Spacer()
