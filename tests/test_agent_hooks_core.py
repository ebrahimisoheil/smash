...
from smash_core.agent_hooks import (  # noqa: E402
...
class AgentHooksCoreTests(unittest.TestCase):
    ...
    def test_build_payload_rejects_unsupported_agent(self):
        with self.assertRaises(ValueError):
            build_agent_hooks_payload(
                ...
                agent="kiro",
                runtime_script=Path("/tmp/smash/smash.py"),
                python_cmd="python3",
            )
        ...
        with tempfile.TemporaryDirectory() as temp:
            ...
            for _ in range(2):
                payload = build_agent_hooks_payload(
                    target=Path(temp),
                    agent="cursor",
                    runtime_script=Path(temp) / "smash.py",
                    python_cmd="python3",
    ...
    def test_build_preview_includes_both_events_and_commands(self):
        payload = build_agent_hooks_payload(
            target=Path("/tmp/my Smash"),
            agent="claude-code",
            runtime_script=Path("/tmp/my Smash/smash.py"),
        ...
        script = str(Path("/tmp/my Smash/smash.py"))
        ...
        with tempfile.TemporaryDirectory() as temp:
            ...
            transcript.write_text(
                "\n".join([
                    ...
                    _transcript_line("user", "Smash memory (local, source-backed) · project demo\n"
                                             "Relevant memories\n- Prefer small commits (preference · user)"),
                    _transcript_line("assistant", "Smash consolidation plan (read-only)\nPending captures: 3"),
