...
from mcp_package.smash_core.cli_memory import (
    render_brief_text,
...
class CliMemoryCoreTests(unittest.TestCase):
    def test_render_remember_created(self):
        ...
        self.assertIn("smash recall", text)
    ...
    def test_render_remember_duplicate(self):
        ...
        self.assertIn("smash explain-memory prefer-release-branches", text)
    ...
    def test_render_update_memory(self):
        ...
        self.assertIn("smash review-memory prefer-release-branches", text)
    ...
    def test_render_set_memory_visibility_text(self):
        ...
        self.assertIn("smash team-sync", text)
    ...
    def test_render_propose_memories_text(self):
        code, text = render_propose_memories_text({
            ...
            "proposals": [{
                ...
                "primary_action": {"command": "python3 smash.py update-memory prefer-release-branches"},
        ...
        self.assertIn("Command: python3 smash.py update-memory", text)
    ...
    def test_render_recall_empty(self):
        code, text = render_recall_text(query="release branches", results=[], project="Smash")

        self.assertEqual(code, 0)
        self.assertIn("Smash memory recall: release branches", text)
        self.assertIn("Project: Smash", text)
    ...
    def test_render_memory_status_archive(self):
        ...
        self.assertIn("Restore: smash restore-memory prefer-local-memory", text)
    ...
    def test_render_memory_inbox(self):
        code, text = render_memory_inbox_text({
            ...
            "items": [{
                ...
                "primary_action": {
                    ...
                    "command": "smash review-memory prefer-local-memory",
                },
                "actions": [
        ...
        }, target="/tmp/Smash", include_archived=True)

        self.assertEqual(code, 0)
        self.assertIn("Smash memory inbox: /tmp/Smash", text)
        self.assertIn("Project: Smash", text)
    ...
    def test_render_explain_memory(self):
        ...
        self.assertEqual(code, 0)
        self.assertIn("Smash memory explanation: Prefer local memory", text)
    ...
    def test_render_brief_text(self):
        ...
        code, text = render_brief_text(payload, query="local memory", project="Smash")

        self.assertEqual(code, 0)
        self.assertIn("Smash memory brief: local memory", text)
        self.assertIn("Project: Smash", text)
    ...
    def test_render_profile_text_empty(self):
        ...
        self.assertIn("smash remember", text)
        self.assertIn("/tmp/Smash", text)
    ...
    def test_render_profile_text_with_sections(self):
        ...
        code, text = render_profile_text({
            ...
            "projects": [],
            "archived": [],
        }, target="/tmp/Smash", project="Smash")

        self.assertEqual(code, 0)
        self.assertIn("Smash memory profile: /tmp/Smash", text)
        self.assertIn("Projects: Smash: 1", text)
    ...
    def test_render_memory_audit_text(self):
        code, text = render_memory_audit_text({
            ...
            "next_actions": [{
                ...
                "recommended": True,
                "command": "smash memory-inbox",
            }],
        }, target="/tmp/Smash")

        self.assertEqual(code, 0)
        self.assertIn("Smash memory audit: /tmp/Smash", text)
