...
EXPECTED_KEY_EDGES = {
    ...
    ("Smash", "knowledge-graph"),
    ("Smash", "retrieval-augmented-generation"),
    ("prefer-local-personal-memory", "agent-memory"),
    ("prefer-local-personal-memory", "Smash"),
    ("retrieval-augmented-generation", "transformers"),
    ("why-Smash-helps-agents", "agent-memory"),
}
...
class DemoSnapshotTests(unittest.TestCase):
    def make_demo(self) -> Path:
        tmp = Path(tempfile.mkdtemp(prefix="smash-demo-snapshot-"))
        target = tmp / "demo"
    ...
    def test_demo_file_snapshot_and_health(self):
        ...
        self.assertTrue((target / "wiki/explorations/why-Smash-helps-agents.md").exists())
        ...
        self.assertIn("Created: explorations/why-Smash-helps-agents.md", log_text)
    ...
    def test_demo_context_snapshot(self):
        ...
        self.assertEqual(relationships["Smash"], "inbound")
