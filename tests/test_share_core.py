...
from mcp_package.smash_core.share import render_share_text, share_page_payload


class ShareCoreTests(unittest.TestCase):
    def make_wiki(self) -> Path:
        root = Path(tempfile.mkdtemp(prefix="Smash-share-core-"))
        wiki = root / "wiki"
    ...
    def test_share_resolves_exact_memory_title(self):
        ...
        self.assertIn("smash serve", payload["serve_command_text"])
    ...
    def test_share_missing_page_returns_candidates(self):
        ...
        self.assertIn("Smash share", text)

    def test_render_share_not_found(self):
        code, text = render_share_text({
            "found": False,
            "query": "missing",
            "error": "no matching Smash page found",
