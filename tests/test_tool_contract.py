...
class ToolContractTests(unittest.TestCase):
    ...
    def test_contract_reports_missing_mcp_docs(self):
        tmp = Path(tempfile.mkdtemp(prefix="Smash-tool-contract-"))
        try:
            (tmp / "mcp_package/smash_mcp").mkdir(parents=True)
            (tmp / "mcp_package/smash_core").mkdir(parents=True)
            (tmp / "mcp_package").mkdir(exist_ok=True)
            shutil.copy2(ROOT / "smash.py", tmp / "smash.py")
            shutil.copy2(ROOT / "mcp_package/smash_core/cli_parser.py", tmp / "mcp_package/smash_core/cli_parser.py")
            shutil.copy2(ROOT / "mcp_package/smash_mcp/server.py", tmp / "mcp_package/smash_mcp/server.py")

            (tmp / "docs").mkdir()
            cli_reference = "\n".join(f"`smash {command}`" for command in sorted(contract.DOCS_CLI_COMMANDS))
            all_mcp_terms = (
