...
from smash_core.web_memory_pages import (  # noqa: E402
    render_brief_page,
...
def test_render_brief_page_escapes_query_and_project():
    payload = {
        ...
        "relevant_count": 1,
        "agent_guidance": ["Use <Smash> first"],
        "relevant_memories": [],
    }
    ...
    assert 'data-copy-text="brief me from Smash about &lt;task&gt; for project &lt;alpha&gt;"' in html
    assert "Copy brief prompt" in html
    assert 'data-copy-text="query Smash for &lt;task&gt;"' in html
    assert "Copy query prompt" in html
    assert "Project:</strong> &lt;alpha&gt;" in html
    assert "Use &lt;Smash&gt; first" in html
...
def test_render_brief_page_guides_empty_memory_recovery():
    ...
    assert "Teach Smash before the next brief" in html
    assert 'href="/ingest"' in html
    assert 'href="/propose"' in html
    assert 'data-copy-text="propose memories about release notes from Smash raw sources"' in html
...
def test_render_memory_dashboard_page_shows_counts_next_actions_and_sections():
    payload = {
        ...
        "by_scope": {"project": 1},
        "next_actions": [{"label": "Review", "detail": "Confirm memory", "command": "smash memory-inbox"}],
        "review": [],
        "captures": [],
        "recent_updates": [],
        "active": [],
    ...
    assert 'data-copy-text="what does Smash remember about project alpha?"' in html
    assert 'data-copy-text="brief me from Smash for project alpha"' in html
    assert 'data-copy-text="audit Smash memory for project alpha"' in html
    assert "<strong>Types:</strong> preference: 2" in html
    assert "smash memory-inbox" in html
...
def test_render_profile_page_lists_memory_sections_and_explain_links():
    ...
    assert 'data-copy-text="what does Smash remember about me?"' in html
    assert 'data-copy-text="start with Smash before we continue"' in html
...
def test_render_memory_audit_page_reports_risks():
    ...
    assert 'data-copy-text="audit Smash memory for project alpha"' in html
    assert 'data-copy-text="review Smash memory inbox for project alpha"' in html
...
def test_render_captures_page_shows_redaction_and_read_warnings():
    ...
    assert 'data-copy-text="review Smash raw captures for project alpha"' in html
...
def test_render_inbox_page_lists_review_items_and_actions():
        ...
        "items": [
            {
                ...
                "actions": [{"label": "Mark reviewed", "command": "smash review-memory memory-one"}],
            }
        ],
    }
    ...
    assert 'data-copy-text="review Smash memory inbox"' in html
    assert "Memory &lt;One&gt;" in html
    assert "Needs &lt;review&gt;" in html
    assert "/explain-memory?memory=memory-one" in html
    assert "smash review-memory memory-one" in html
...
def test_render_memory_explanation_page_shows_trust_context_actions_and_body():
    payload = {
        ...
        "review": {
            ...
            "issues": [{"severity": "warning", "code": "pending", "message": "Needs <review>"}],
            "primary_action": {"label": "Review", "description": "Confirm it"},
            "actions": [{"label": "Forget", "command": "smash forget-memory prefer-reviewable-memory"}],
        },
        "provenance": {
            "source": "<unit test>",
    ...
    assert "smash forget-memory prefer-reviewable-memory" in html
...
def test_render_memory_wins_page_shows_local_proof_signals():
    payload = {
        ...
        "prompts": ["what does Smash remember about me?"],
        "next_actions": [
            {"label": "Use memory", "reason": "Try the value loop.", "command": "smash brief current-task ."}
        ],
    }
    ...
    assert 'data-copy-text="what does Smash remember about me?"' in html
    assert "smash brief current-task ." in html
