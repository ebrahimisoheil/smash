...
import serve
from smash_core.operations import begin_operation
from smash_core.schema import write_schema
...
def post_json(path: str, payload: dict[str, object], local_action: bool = True):
    ...
    if local_action:
        headers["X-Smash-Local-Action"] = "true"
    return run_handler(
        "POST",
        path,
        body,
...
class ServeTests(unittest.TestCase):
    def make_wiki(self) -> Path:
        tmp = Path(tempfile.mkdtemp(prefix="Smash-test-"))
        wiki = tmp / "wiki"
        wiki.mkdir()
    ...
    def test_layout_handles_search_enter_key(self):
        ...
        self.assertIn("localStorage.getItem('Smash-theme')", html)
        self.assertIn("navigator.clipboard.writeText", html)
        self.assertIn("data-copy-text", html)
        self.assertIn("data-raw-source-form", html)
        self.assertIn("/api/raw-source", html)
        self.assertIn("/api/seed-project", html)
        self.assertIn("data-server-action", html)
    ...
    def test_api_startup_banner_points_to_json_endpoints(self):
        text = "\n".join(serve._api_startup_lines(3457))

        self.assertIn("http://127.0.0.1:3457/api", text)
        self.assertIn("http://127.0.0.1:3457/api/health", text)
        self.assertIn("http://127.0.0.1:3457/api/query-Smash", text)
    ...
    def test_all_pages_is_paginated_for_large_wikis(self):
        ...
        self.assertNotIn("Smash Test Wiki Index", filtered)
    ...
    def test_security_headers_include_api_version(self):
        ...
        self.assertIn(("X-Smash-API-Version", serve.API_VERSION), headers)
    ...
    def test_health_page_and_operations_api_show_interrupted_writes(self):
        ...
        self.assertIn(b"smash operations", body)
    ...
    def test_api_root_returns_discovery_payload(self):
        ...
        self.assertIn("/api/query-Smash", payload["endpoints"]["read"])
        ...
        self.assertEqual(payload["write_header"]["X-Smash-Local-Action"], "true")

    def test_api_runner_rejects_html_routes_with_json_error(self):
        self.make_wiki()
        handler = object.__new__(serve.APIHandler)
        handler.command = "GET"
        handler.path = "/health"
        handler.request_version = "HTTP/1.1"
        ...
        self.assertEqual(payload["error"], "API runner only serves /api endpoints")
        self.assertEqual(payload["discovery"], "/api")
    ...
    def test_memory_wins_page_and_api_show_local_value_signals(self):
        ...
        self.assertEqual(status, 200)
        self.assertEqual(payload["schema"], "smash-memory-wins-v1")
    ...
    def test_options_preflight_returns_local_json_405(self):
        ...
        self.assertEqual(payload["error"], "CORS preflight is not supported; Smash is localhost-only")
    ...
    def test_unsupported_methods_return_hardened_json_405(self):
        ...
        for method in ("PUT", "PATCH", "DELETE", "TRACE", "CONNECT"):
            ...
            self.assertEqual(status, 405)
            self.assertEqual(payload["error"], "method not allowed; Smash supports GET, HEAD, and POST")
            self.assertEqual(headers["Allow"], "GET, HEAD, POST")
    ...
    def test_server_args_stay_local_only(self):
        ...
        port, root = serve._parse_serve_args(
            ["--root", "/tmp/smash-demo", "--port", "3012"],
            default_port=3000,
            default_root=Path("/tmp/default"),
        )
        self.assertEqual(port, 3012)
        self.assertEqual(root, Path("/tmp/smash-demo").resolve())
        ...
        with self.assertRaisesRegex(SystemExit, "does not accept a positional target"):
            serve._parse_serve_args(["/tmp/smash-demo"], default_port=3000, default_root=Path("/tmp/default"))
    ...
    def test_home_page_shows_first_agent_prompts(self):
        ...
        self.assertIn("is Smash ready?", html)
        self.assertIn("start with Smash before we continue", html)
        self.assertIn("ingest raw/&lt;file&gt; into Smash", html)
        self.assertIn("what does Smash know about me?", html)
    ...
    def test_prompts_page_and_api_share_starter_prompts(self):
        ...
        self.assertEqual(payload["prompts"][0]["prompt"], "is Smash ready?")
        ...
        self.assertIn("smash health", html)
    ...
    def test_onboard_page_shows_agent_setup_loop(self):
        ...
        self.assertIn("smash seed .", html)
        self.assertIn("MCP and CLI work without the viewer running", html)
        self.assertIn("smash health", html)
        self.assertIn("smash onboard", html)
        self.assertIn("--first-memory", html)
        self.assertIn("--agent codex", html)
        self.assertIn("data-seed-project-form", html)
        self.assertIn("data-first-memory-form", html)
        self.assertIn("is Smash ready?", html)
        self.assertIn(b"Onboard", body)
        self.assertIn(b"--agent codex", body)

    def test_health_page_exposes_direct_repair_actions(self):
        self.make_wiki()

        html = serve._render_health()

        self.assertIn('data-server-action="/api/migrate"', html)
        self.assertIn('data-server-action="/api/rebuild-index"', html)
        self.assertIn('data-server-action="/api/rebuild-backlinks"', html)
    ...
    def test_wiki_page_links_to_local_graph(self):
        ...
        self.assertIn('data-copy-text="query Smash for Agent Memory"', html)
    ...
    def test_memory_brief_page_and_api_include_capture_status(self):
        ...
        self.assertIn('data-copy-text="brief me from Smash about brief for project alpha"', html)
        self.assertIn('data-copy-text="query Smash for brief"', html)
    ...
    def test_query_link_api_returns_context_packet(self):
        ...
        status, payload = run_handler("GET", "/api/query-Smash?q=agent%20memory&budget=small")
    ...
    def test_status_api_returns_readiness_summary(self):
        ...
        self.assertEqual(status, 200)
        self.assertEqual(payload["api_version"], serve.API_VERSION)
        self.assertEqual(payload["version"], serve.SMASH_VERSION)
        self.assertTrue(payload["ready"])
    ...
    def test_memory_action_post_requires_local_action_header(self):
        ...
        self.assertEqual(status, 403)
        self.assertFalse(payload["updated"])
        self.assertIn("X-Smash-Local-Action", payload["error"])
    ...
    def test_propose_memories_post_is_write_free(self):
        ...
        write_page(
            ...
            (
                ...
                "> **TLDR:** User prefers release branches for Smash work.\n\n"
    ...
    def test_propose_page_renders_read_only_workflow(self):
        ...
        html = serve._render_propose(project="Smash", source="raw/first-memory.md")
        ...
        self.assertIn('value="Smash"', html)
    ...
    def test_memory_approval_api_requires_header_and_writes_memory(self):
        wiki = self.make_wiki()
        payload = {
            "memory": "User wants Smash memory approvals to stay explicit.",
        ...
        self.assertIn("X-Smash-Local-Action", denied_payload["error"])
    ...
    def test_memory_approval_api_ignores_duplicate_override_flags(self):
        wiki = self.make_wiki()
        payload = {
            "memory": "User prefers Smash web approvals to be reviewable.",
    ...
    def test_memory_update_api_ignores_conflict_override_flags(self):
        ...
        first_status, first = post_json(
            "/api/remember-memory",
            {
                "memory": "User prefers release branches for Smash work.",
                "title": "Prefer release branches",
                "memory_type": "preference",
                "scope": "project",
                "project": "Smash",
        ...
        second = serve._core_write_memory_page(
            wiki,
            "User prefers dark mode for Smash work.",
            "Prefer dark mode",
            "preference",
            "project",
            None,
        ...
        update_status, update = post_json(
            ...
            {
                "memory": second["name"],
                "text": "User prefers develop branches for Smash work.",
                "source": "web proposal",
                "project": "Smash",
                "allow_conflict": True,
            },
    ...
    def test_raw_source_api_creates_local_source_for_ingest(self):
        ...
        status, payload = post_json(
            "/api/raw-source",
            {
                ...
                "text": "User wants a web path for adding Smash sources.",
            },
        )
        ...
        self.assertEqual(payload["next_prompt"], "ingest raw/project-notes.md into Smash")
    ...
    def test_ingest_page_and_api_show_pending_raw(self):
        ...
        self.assertIn('data-copy-text="ingest raw/new-source.md into Smash"', html)
        self.assertIn("Copy prompt", html)
        self.assertIn("Copy command", html)
        self.assertIn('data-copy-text="smash validate ', html)
        self.assertIn(str(wiki.parent), html)
        self.assertIn("ingest raw/new-source.md into Smash", html)
    ...
    def test_ingest_page_shows_completion_for_represented_raw(self):
        ...
        self.assertIn('data-copy-text="query Smash for represented source"', html)
        self.assertIn("start with Smash before we continue", html)
    ...
    def test_ingest_page_marks_stale_represented_raw(self):
        ...
        self.assertIn('data-copy-text="re-ingest raw/represented-source.md into Smash"', html)
    ...
    def test_rebuild_backlinks_requires_json_post(self):
        ...
        self.assertIn("X-Smash-Local-Action", bad_post_payload["error"])
        self.assertEqual(missing_header_status, 403)
        self.assertFalse(missing_header_payload["rebuilt"])
        self.assertIn("X-Smash-Local-Action", missing_header_payload["error"])
    ...
    def test_validate_post_endpoint_matches_direct_action_flow(self):
        wiki = self.make_wiki()
        for dirname in ("sources", "concepts", "entities", "memories", "comparisons", "explorations"):
            (wiki / dirname).mkdir(exist_ok=True)
        write_page(
    ...
    def test_migrate_post_endpoint_applies_safe_schema_migration(self):
        wiki = self.make_wiki()
        schema_path = wiki / "_link_schema.json"
        schema_path.unlink()

        status, payload = post_json("/api/migrate", {})

        self.assertEqual(status, 200)
        self.assertTrue(payload["ok"])
        self.assertTrue(schema_path.exists())

    def test_seed_project_post_endpoint_writes_source_backed_context(self):
        wiki = self.make_wiki()
        project_root = wiki.parent / "project"
        project_root.mkdir()
        (project_root / "README.md").write_text("# Demo Project\n", encoding="utf-8")

        status, payload = post_json("/api/seed-project", {"project_root": str(project_root)})

        self.assertEqual(status, 200)
        self.assertTrue(payload["wrote"])
        self.assertTrue((wiki.parent / payload["raw_path"]).exists())
        self.assertTrue((wiki.parent / payload["source_page"]).exists())
    ...
    def test_graph_controls_exist_before_graph_script(self):
        ...
        self.assertLess(html.index('id="graph-copy-Smash"'), html.index("var copyLinkButton ="))
