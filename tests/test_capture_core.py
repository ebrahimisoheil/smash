...
from mcp_package.smash_core.capture import (
...
class CaptureCoreTests(unittest.TestCase):
    ...
    def test_capture_title_falls_back_to_first_note_line(self):
        self.assertEqual(
            capture_title("\n\nRemember that Smash is local agent memory.\nMore detail."),
            "Memory capture: Remember that Smash is local agent memory",
        )

    def test_capture_filename_is_unique_and_slugged(self):
        root = Path(tempfile.mkdtemp(prefix="Smash-capture-filename-"))
    ...
    def test_write_session_capture_persists_proposal_only_markdown(self):
        root = Path(tempfile.mkdtemp(prefix="Smash-capture-write-"))

        payload = write_session_capture(
            root,
            text="Remember that Smash uses local markdown memory.",
        ...
        self.assertEqual(payload["project"], "Smash-product")
        self.assertEqual(payload["secret_warnings"], [])
        self.assertIn('project: "Smash-product"', text)
        self.assertIn("## Source Input\n\nraw/first-memory.md", text)
        self.assertIn("## Notes\n\nRemember that Smash uses local markdown memory.", text)
    ...
    def test_resolve_capture_file_accepts_supported_root_relative_forms(self):
        root = Path(tempfile.mkdtemp(prefix="Smash-capture-core-"))
    ...
    def test_resolve_capture_file_rejects_paths_outside_root(self):
        root = Path(tempfile.mkdtemp(prefix="Smash-capture-core-"))
        outside = Path(tempfile.mkdtemp(prefix="Smash-capture-outside-")) / "session.md"
    ...
    def test_capture_notes_from_markdown_extracts_notes_section(self):
        ...
        self.assertEqual(meta["project"], "Smash")
    ...
    def test_capture_proposal_selection_reads_capture_and_selects_index(self):
        root = Path(tempfile.mkdtemp(prefix="Smash-capture-selection-"))
        capture_dir = root / "raw" / "memory-captures"
        capture_dir.mkdir(parents=True)
    ...
    def test_redact_capture_file_redacts_and_reports_labels(self):
        root = Path(tempfile.mkdtemp(prefix="Smash-capture-redact-"))
        capture_dir = root / "raw" / "memory-captures"
        capture_dir.mkdir(parents=True)
        fake_key = "sk-" + "a" * 48
        capture = capture_dir / "session.md"
    ...
    def test_delete_capture_file_requires_confirmation(self):
        root = Path(tempfile.mkdtemp(prefix="Smash-capture-delete-"))
        capture_dir = root / "raw" / "memory-captures"
        capture_dir.mkdir(parents=True)
        capture = capture_dir / "session.md"
        capture.write_text("## Notes\n\nDelete me.\n", encoding="utf-8")
    ...
    def test_capture_mutation_helpers_reject_missing_capture(self):
        root = Path(tempfile.mkdtemp(prefix="Smash-capture-missing-"))
    ...
    def test_render_capture_inbox_text_lists_actions_and_warnings(self):
            ...
            "captures": [
                {
                    ...
                    "commands": {
                        "accept": "python3 smash.py accept-capture alpha . --index 1",
                        "redact": "python3 smash.py redact-capture alpha .",
                        "delete": "python3 smash.py delete-capture alpha . --confirm",
                    },
                }
            ],
        }
        ...
        self.assertIn("Accept: python3 smash.py accept-capture", text)
        self.assertIn("Redact: python3 smash.py redact-capture", text)
    ...
    def test_render_accept_capture_text_reports_success_and_rejection(self):
        ...
        self.assertIn("smash review-memory prefer-local-memory", text)
    ...
    def test_render_session_end_text_lists_review_gated_proposals(self):
        ...
        self.assertIn("Smash session end", text)
...
class CaptureInboxProposalPreviewTests(unittest.TestCase):
    def test_inbox_items_carry_proposal_previews(self):
        import tempfile
        from pathlib import Path
        from mcp_package.smash_core.capture import capture_inbox

        root = Path(tempfile.mkdtemp(prefix="Smash-capture-preview-"))
...
class CaptureProvenanceTests(unittest.TestCase):
    ...
    def test_accept_mines_user_turns_not_assistant_prose(self):
        from mcp_package.smash_core.memory import propose_memories_from_text
