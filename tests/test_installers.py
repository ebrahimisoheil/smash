...
class InstallerTests(unittest.TestCase):
    def test_scaffold_does_not_use_break_system_packages(self):
        ...
        self.assertIn(".smash-mcp-venv", scaffold)
        self.assertIn(".smash-mcp-python", scaffold)
        self.assertIn("SMASH_MCP_INSTALLED=false", scaffold)
        self.assertIn('[ "$SMASH_MCP_INSTALLED" = true ]', scaffold)

    def test_scaffold_installs_short_global_link_command(self):
        scaffold = (ROOT / "integrations/_shared/scaffold.sh").read_text(encoding="utf-8")

        self.assertIn('SMASH_CLI_BIN="$SMASH_CLI_DIR/smash"', scaffold)
        self.assertIn('LEGACY_LINK_CLI_BIN="$SMASH_CLI_DIR/Smash"', scaffold)
        self.assertIn("Removed old Smash wrapper", scaffold)
        self.assertIn("Smash command wrapper", scaffold)
        self.assertIn("not overwriting", scaffold)
        self.assertIn("smash health", scaffold)
        self.assertIn("SMASH_CLI_COMMAND=smash exec python3", scaffold)
        self.assertIn('if [ "$MODE" = "--project" ]', scaffold)
        self.assertIn('python3 "$TARGET_DIR/smash.py" doctor --fix "$TARGET_DIR"', scaffold)
        self.assertNotIn('cp "$SMASH_ROOT/wiki/index.md"', scaffold)
    ...
    def test_powershell_scaffold_uses_venv_and_short_link_command(self):
        ...
        self.assertIn(".smash-mcp-venv", scaffold)
        self.assertIn(".smash-mcp-python", scaffold)
        self.assertIn("smash.cmd", scaffold)
        self.assertIn("Smash.cmd", scaffold)
        self.assertIn("Removed old Smash wrapper", scaffold)
        self.assertIn("Smash command wrapper", scaffold)
        self.assertIn("set SMASH_CLI_COMMAND=smash", scaffold)
        self.assertIn('$env:SMASH_CLI_COMMAND = "smash"', scaffold)
        self.assertIn("Get-Command py", scaffold)
        self.assertIn("-m venv", scaffold)
        self.assertIn("doctor --fix $TargetDir", scaffold)
        self.assertNotIn('Copy-LinkFile (Join-Path $SmashRoot "wiki\\index.md")', scaffold)
    ...
    def test_installers_read_resolved_mcp_python_marker(self):
        for installer in INSTALLERS:
            with self.subTest(installer=installer.name):
                ...
                self.assertIn(".smash-mcp-python", text)

        for installer in POWERSHELL_INSTALLERS:
            with self.subTest(installer=installer.name):
                text = installer.read_text(encoding="utf-8")
                self.assertIn("Smash-ReadMcpPython", text)
    ...
    def test_installers_print_mode_specific_next_steps(self):
        ...
        self.assertIn("View wiki: python3 smash.py serve", instructions)
        self.assertIn("View wiki: smash serve", instructions)
        self.assertIn("Print starter prompts: python3 smash.py next", instructions)
        self.assertIn("Print starter prompts: smash next", instructions)
        self.assertIn("Try in your agent:", instructions)
        self.assertIn("is Smash ready?", instructions)
        self.assertIn("start with Smash before we continue", instructions)
        self.assertIn("ingest raw/<file> into Smash", instructions)
        self.assertIn("what does Smash know about me?", instructions)
        self.assertIn("what does Smash remember about this project?", instructions)
        ...
        self.assertIn("function Smash-PrintNextSteps", instructions_ps1)
        self.assertIn("py smash.py next", instructions_ps1)
        self.assertIn("Try in your agent:", instructions_ps1)
        self.assertIn("is Smash ready?", instructions_ps1)

        for installer in POWERSHELL_INSTALLERS:
            with self.subTest(installer=installer.name):
                text = installer.read_text(encoding="utf-8")
                self.assertIn("instructions.ps1", text)
                self.assertIn("Smash-PrintNextSteps", text)
    ...
    def test_codex_and_kiro_update_existing_mcp_registration(self):
        ...
        self.assertNotIn("! grep -q '\\[mcp_servers.Smash\\]'", codex)
        self.assertNotIn("Smash MCP already registered", kiro)
    ...
    def test_codex_mcp_registration_pattern_compiles_and_replaces_block(self):
        ...
        existing_config = (
            '[mcp_servers.Smash]\n'
            'command = "python3"\n'
            'args = ["-m", "smash_mcp", "--wiki", "/old/wiki"]\n'
            '\n'
            '[profiles.default]\n'
            'model = "gpt-5"\n'
        )
        replacement = (
            '[mcp_servers.Smash]\n'
            'command = "/Users/g/.smash-mcp-venv/bin/python"\n'
            'args = ["-m", "smash_mcp", "--wiki", "/Users/g/Smash/wiki"]\n'
        )

        updated = pattern.sub(replacement, existing_config)

        self.assertIn('command = "/Users/g/.smash-mcp-venv/bin/python"', updated)
