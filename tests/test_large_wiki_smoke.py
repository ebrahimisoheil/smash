...
from smash_core.wiki import build_backlinks, build_wiki_cache, close_wiki_cache, graph_data  # noqa: E402
...
class LargeWikiSmokeTests(unittest.TestCase):
    def test_smart_query_and_graph_handle_hundreds_of_pages(self):
        root = Path(tempfile.mkdtemp(prefix="Smash-large-wiki-"))
        wiki = root / "wiki"
        wiki.mkdir()
    ...
    def test_large_wiki_smoke_reports_benchmark_health(self):
        root = Path(tempfile.mkdtemp(prefix="Smash-large-wiki-health-"))
        ...
        self.assertEqual(payload["viewer"]["serve_command"], f"python3 smash.py serve {root}")
