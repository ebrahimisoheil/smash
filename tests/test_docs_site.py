...
class DocsSiteTests(unittest.TestCase):
    ...
    def test_github_pages_site_references_existing_local_assets(self):
        ...
        why_html = (ROOT / "docs/why-smash.html").read_text(encoding="utf-8")
        ...
        self.assertIn("browser version of <code>smash onboard</code>", ui_html)
        self.assertIn("MCP clients keep working after you close the browser", ui_html)
        self.assertIn("assets/smash-ui-tour.gif", ui_html)

        cli_html = (ROOT / "docs/cli.html").read_text(encoding="utf-8")
        self.assertIn("assets/smash-cli-tour.gif", cli_html)

        mcp_html = (ROOT / "docs/mcp.html").read_text(encoding="utf-8")
        self.assertIn("assets/smash-mcp-agent-chat.gif", mcp_html)
    ...
    def test_github_pages_site_has_no_external_runtime_dependencies(self):
        ...
        for page in self.docs_pages():
            ...
            self.assertNotIn('<Smash rel="stylesheet" href="http', lower)
    ...
    def test_github_pages_analytics_is_docs_only_and_manual(self):
        ...
        self.assertIn("does not run inside Smash, read local wiki data, or capture source/memory content", security)
...
class FoundingIdentityTests(unittest.TestCase):
    """New releases layer onto Smash's founding story; they must never bury it.

    These are the identity claims that made Smash through 1.5.0. If a landing
    ...
    PILLARS = {
        ...
        "proof": "the first-run proof loop (smash proof)",
    }
