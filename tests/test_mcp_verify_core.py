...
from mcp_package.smash_core.mcp_verify import (
    build_mcp_verify_status,
    default_mcp_venv_python,
    display_command,
    ensure_smash_mcp_runtime,
...
class ProvisionLinkExtrasTests(unittest.TestCase):
    def test_installs_pinned_extras_into_managed_venv(self):
        ...
        self.assertIn("smash-mcp[semantic,semantic-quality,rerank]==1.7.0", commands[1])
...
class EnsureLinkMcpRuntimeTests(unittest.TestCase):
    def test_configured_python_that_matches_wins(self):
        ...
        result = ensure_smash_mcp_runtime("/usr/bin/python3", "1.7.0", import_check=check)
    ...
    def test_existing_venv_is_used_when_configured_python_is_stale(self):
        with tempfile.TemporaryDirectory() as temp:
            ...
            # creates the file where ensure_smash_mcp_runtime will look.
            ...
            result = ensure_smash_mcp_runtime(
    ...
    def test_provisioning_creates_venv_and_reports_pip_failures(self):
        ...
        self.assertEqual(commands[0][:3], ["/usr/bin/python3", "-m", "venv"])
        self.assertIn("smash-mcp==1.7.0", commands[1])
    ...
    def test_no_provisioning_without_the_flag(self):
        ...
        result = ensure_smash_mcp_runtime("/usr/bin/python3", "1.7.0", import_check=check, run=run)
...
class McpVerifyCoreTests(unittest.TestCase):
    ...
    def test_guidance_reports_missing_sdk_and_version_mismatch(self):
        issues, actions = mcp_verify_guidance(
            target=Path("/tmp/Smash"),
            init_command=["python3", "smash.py", "init", "/tmp/Smash"],
        ...
        self.assertEqual([action["tool"] for action in actions], ["reinstall_smash_mcp", "upgrade_smash_mcp"])
        self.assertIn("/tmp/Smash Python/bin/python", actions[0]["command_text"])
    ...
    def test_render_ready_status(self):
        code, text = render_mcp_verify_text({
            ...
            "smash_mcp": {"installed": True, "version": "1.2.0", "mcp_sdk": True, "error": None},
            "wiki": {"path": "/tmp/smash/wiki", "exists": True},
            "config": {"mcpServers": {"Smash": {"command": "/tmp/python", "args": ["-m", "smash_mcp"]}}},
            "next_actions": [],
        })

        self.assertEqual(code, 0)
        self.assertIn("Smash MCP verification: /tmp/Smash", text)
        self.assertIn("smash-mcp: installed (1.2.0)", text)
        self.assertIn('"command": "/tmp/python"', text)
        self.assertIn("Result: ready", text)

    def test_render_missing_package_status(self):
        action = {
            "tool": "install_smash_mcp",
            "command_text": "/tmp/python -m pip install --upgrade smash-mcp",
        }
        code, text = render_mcp_verify_text({
            "ready": False,
            "target": "/tmp/Smash",
            ...
            "expected_version": "1.2.0",
            "version_matches": False,
            "smash_mcp": {"installed": False, "version": None, "mcp_sdk": False, "error": "No module named smash_mcp"},
            "wiki": {"path": "/tmp/smash/wiki", "exists": True},
            "config": {},
            "next_actions": [action],
        })
        ...
        self.assertIn("smash-mcp: missing", text)
        self.assertIn("Install: /tmp/python -m pip install --upgrade smash-mcp", text)
    ...
    def test_display_command_quotes_paths(self):
        text = display_command(["/tmp/Smash Python/bin/python", "-m", "pip"])

        self.assertIn("/tmp/Smash Python/bin/python", text)
        self.assertIn("-m", text)
        self.assertIn("pip", text)

    def test_display_command_uses_non_conflicting_default_link_command(self):
        text = display_command(["Smash", "health", "/tmp/Smash"])

        self.assertEqual(text, "smash health /tmp/Smash")

    def test_display_command_can_use_source_checkout_command(self):
        set_link_command_override(["python3", "/repo/smash.py"])

        text = display_command(["Smash", "health", "/tmp/Smash"])

        self.assertEqual(text, "python3 /repo/smash.py health /tmp/Smash")

    def test_display_command_rewrites_lnk_when_source_checkout_command_is_set(self):
        set_link_command_override(["python3", "/repo/smash.py"])

        text = display_command(["smash", "doctor", "/tmp/Smash"])

        self.assertEqual(text, "python3 /repo/smash.py doctor /tmp/Smash")
    ...
    def test_expand_command_prefix_preserves_command_path_syntax(self):
        ...
        self.assertIn("Smash-python", expand_command_prefix("~/Smash-python"))

    def test_resolve_mcp_python_uses_marker(self):
        root = Path(tempfile.mkdtemp(prefix="smash-mcp-verify-"))
        self.addCleanup(lambda: shutil.rmtree(root, ignore_errors=True))
        (root / ".smash-mcp-python").write_text("/tmp/Smash-python\n", encoding="utf-8")

        python = resolve_mcp_python(root, root / "wiki", None, default_python="/usr/bin/python")

        self.assertEqual(python, "/tmp/Smash-python")
    ...
    def test_build_status_ready(self):
        ...
        self.assertEqual(status["config"]["mcpServers"]["Smash"]["command"], "/tmp/python")

    def test_build_status_reports_missing_wiki_and_version_mismatch(self):
        status = build_mcp_verify_status(
            target=Path("/tmp/Smash"),
            wiki_dir=Path("/tmp/smash/missing-wiki"),
            expected_version="1.2.0",
            init_command=["python3", "smash.py", "init", "/tmp/Smash"],
            default_python="/tmp/python",
            import_check=lambda _python: {
        ...
        self.assertEqual([action["tool"] for action in status["next_actions"]], ["upgrade_smash_mcp", "init_wiki"])
