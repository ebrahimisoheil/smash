...
from smash_core.mcp_connect import (  # noqa: E402
    agent_alias_matches,
...
def _broken_runtime(python_cmd, expected_version, *, provision=False):
    return {
        "ready": False,
        "python": python_cmd,
        "status": {"installed": False, "version": None, "mcp_sdk": False, "error": "No module named smash_mcp"},
        "provisioned": False,
        "notes": [f"{python_cmd}: smash-mcp not importable"],
    }
...
class McpConnectCoreTests(unittest.TestCase):
    ...
    def test_build_codex_preview_uses_marker_python(self):
        with tempfile.TemporaryDirectory() as temp:
            ...
            wiki = root / "wiki"
            wiki.mkdir()
            (root / ".smash-mcp-python").write_text("/tmp/Smash Python/bin/python\n", encoding="utf-8")
            ...
            payload = build_mcp_connect_payload(
                ...
                init_command=["Smash", "init", str(root)],
                default_python="python3",
                runtime_check=_ready_runtime,
            )

        self.assertEqual(payload["agent"], "codex")
        self.assertEqual(payload["python"], "/tmp/Smash Python/bin/python")
        self.assertIn("[mcp_servers.Smash]", str(payload["snippet"]))
    ...
    def test_write_codex_config_replaces_existing_link_block(self):
        with tempfile.TemporaryDirectory() as temp:
            ...
            config.write_text("[mcp_servers.Smash]\ncommand = \"old\"\n\n[ui]\ntheme = \"dark\"\n", encoding="utf-8")
    ...
    def test_write_json_config_preserves_existing_keys(self):
        ...
        self.assertEqual(data["mcpServers"]["Smash"]["command"], "/tmp/python")
        self.assertFalse(data["mcpServers"]["Smash"]["disabled"])
    ...
    def test_vscode_uses_servers_top_key_and_stdio_type(self):
        ...
        self.assertTrue(payload["write"]["ok"])
        self.assertEqual(data["servers"]["Smash"]["type"], "stdio")
        self.assertEqual(
            data["servers"]["Smash"]["args"],
            ["-m", "smash_mcp", "--wiki", str(wiki), "--surface", "slim"],
        )
    ...
    def test_write_repoints_to_provisioned_venv_and_persists_marker(self):
        def venv_runtime(python_cmd, expected_version, *, provision=False):
            return {
                "ready": True,
                "python": "/home/user/.smash-mcp-venv/bin/python",
                "status": {"installed": True, "version": expected_version, "mcp_sdk": True, "error": None},
                "provisioned": True,
                "notes": ["provisioned ~/.smash-mcp-venv"],
            }
        ...
        with tempfile.TemporaryDirectory() as temp:
            ...
            data = json.loads(config.read_text(encoding="utf-8"))
            marker = (root / ".smash-mcp-python").read_text(encoding="utf-8").strip()

        self.assertTrue(payload["write"]["ok"])
        self.assertEqual(data["mcpServers"]["Smash"]["command"], "/home/user/.smash-mcp-venv/bin/python")
        self.assertEqual(marker, "/home/user/.smash-mcp-venv/bin/python")
    ...
    def test_agent_alias_matches_names_and_aliases_only(self):
        ...
        self.assertFalse(agent_alias_matches("smash-demo"))
    ...
    def test_read_agent_link_server_from_json_config(self):
            ...
            config.write_text(json.dumps({
                "mcpServers": {
                    "Smash": {
                        ...
                        "args": ["-m", "smash_mcp", "--wiki", "/home/u/Smash/wiki", "--surface", "slim"],
                    }
                }
            }), encoding="utf-8")
        ...
        self.assertEqual(server["wiki"], "/home/u/Smash/wiki")
