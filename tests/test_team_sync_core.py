...
from smash_core.team_sync import build_team_sync_payload, render_team_sync_text  # noqa: E402
...
class TeamSyncCoreTests(unittest.TestCase):
    def test_plan_for_workspace_without_git_includes_safe_setup(self):
        root = Path(tempfile.mkdtemp(prefix="Smash-team-sync-"))
        (root / "wiki").mkdir()
        (root / "wiki" / "_link_schema.json").write_text("{}", encoding="utf-8")
        (root / ".gitignore").write_text("raw/*\n.smash-backups/\n", encoding="utf-8")

        payload = build_team_sync_payload(root, remote="git@example.com:team/smash-memory.git")
    ...
    def test_git_workspace_with_raw_protection_is_ready(self):
        ...
        (root / "wiki").mkdir()
        (root / "wiki" / "_link_schema.json").write_text("{}", encoding="utf-8")
        (root / "SMASH.md").write_text("# Smash\n", encoding="utf-8")
        (root / ".gitignore").write_text("raw/*\n.smash-backups/\n", encoding="utf-8")
        (root / ".git").mkdir()
        (root / ".git" / "config").write_text(
            '[remote "origin"]\n\turl = git@example.com:team/smash-memory.git\n',
            encoding="utf-8",
        )
