...
SMASH_SPEC = importlib.util.spec_from_file_location("link_cli_for_mcp_tests", ROOT / "smash.py")
link_cli = importlib.util.module_from_spec(SMASH_SPEC)
assert SMASH_SPEC.loader is not None
SMASH_SPEC.loader.exec_module(link_cli)
...
def import_mcp_server(wiki_dir: Path, surface: str = "full"):
    ...
    module_name = f"smash_mcp_server_contract_{surface}_{id(wiki_dir)}"
    try:
        sys.argv = ["smash_mcp.server", "--wiki", str(wiki_dir), "--surface", surface]
        spec = importlib.util.spec_from_file_location(module_name, ROOT / "mcp_package/smash_mcp/server.py")
...
class McpContractTests(unittest.TestCase):
    def setUp(self):
        tmp = Path(tempfile.mkdtemp(prefix="smash-mcp-contract-"))
    ...
    def test_cross_agent_continuity_cli_memory_recalled_by_slim_mcp(self):
        ...
        with redirect_stdout(out):
            code = link_cli.remember(
                self.target,
                "User prefers pnpm as the package manager for Smash project work.",
        ...
        try:
            payload = json.loads(slim_server.recall("package manager preference", mode="memory", project="Smash"))
        finally:
    ...
    def test_link_status_contract(self):
        ...
        self.assertEqual(payload["version"], self.server.SMASH_VERSION)
    ...
    def test_link_operations_contract(self):
        ...
        self.assertIn("smash operations", payload["next_actions"][0]["command"])
    ...
    def test_starter_prompts_contract(self):
        ...
        self.assertTrue(any(command.startswith("smash health ") for command in payload["commands"]))
    ...
    def test_missing_wiki_message_points_to_current_setup_paths(self):
        previous_argv = sys.argv[:]
        missing = Path(tempfile.mkdtemp(prefix="smash-mcp-missing-")) / "missing" / "wiki"
        module_name = f"smash_mcp_server_missing_{id(missing)}"
        try:
            sys.argv = ["smash_mcp.server", "--wiki", str(missing)]
            spec = importlib.util.spec_from_file_location(module_name, ROOT / "mcp_package/smash_mcp/server.py")
            module = importlib.util.module_from_spec(spec)
            assert spec.loader is not None
            err = StringIO()
            ...
            self.assertIn("smash init", text)
            self.assertIn("python3 smash.py init", text)
    ...
    def test_help_flag_prints_usage_instead_of_starting_the_server(self):
        ...
        previous_argv = sys.argv[:]
        missing = Path(tempfile.mkdtemp(prefix="smash-mcp-help-")) / "missing" / "wiki"
        module_name = f"smash_mcp_server_help_{id(missing)}"
        try:
            sys.argv = ["smash_mcp.server", "--wiki", str(missing), "--help"]
            spec = importlib.util.spec_from_file_location(module_name, ROOT / "mcp_package/smash_mcp/server.py")
            module = importlib.util.module_from_spec(spec)
            assert spec.loader is not None
            out = StringIO()
    ...
    def test_version_flag_does_not_require_wiki_or_mcp_sdk(self):
        previous_argv = sys.argv[:]
        missing = Path(tempfile.mkdtemp(prefix="smash-mcp-version-")) / "missing" / "wiki"
        module_name = f"smash_mcp_server_version_{id(missing)}"
        try:
            sys.argv = ["smash_mcp.server", "--wiki", str(missing), "--version"]
            spec = importlib.util.spec_from_file_location(module_name, ROOT / "mcp_package/smash_mcp/server.py")
            module = importlib.util.module_from_spec(spec)
            assert spec.loader is not None
            out = StringIO()
            ...
            self.assertEqual(cm.exception.code, 0)
            self.assertIn("smash-mcp", out.getvalue())
            self.assertEqual(err.getvalue(), "")
        finally:
            sys.modules.pop(module_name, None)
            sys.argv = previous_argv
    ...
    def test_backup_wiki_contract_reports_archive_failure(self):
        ...
        self.assertEqual(list((self.target / ".smash-backups").glob("*.tar.gz")), [])
    ...
    def test_get_context_contract(self):
        ...
        self.assertIn("smash", page_names)
    ...
    def test_get_backlinks_contract(self):
        ...
        self.assertIn("smash", payload["inbound"])
    ...
    def test_get_graph_contract(self):
        ...
        self.assertIn(("agent-memory", "Smash"), edges)
    ...
    def test_accept_capture_contract(self):
        ...
        recall = json.loads(self.server.recall_memory("MCP capture approval", project="Smash"))
    ...
    def test_memory_log_contract(self):
        ...
        self.assertEqual(payload["schema"], "smash-memory-log-v1")
        self.assertGreaterEqual(payload["count"], 1)
    ...
    def test_rebuild_backlinks_contract(self):
        ...
        self.assertIn("smash", rebuilt["backlinks"]["agent-memory"])
