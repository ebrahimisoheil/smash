...
from smash_core.cli_parser import build_cli_parser, dispatch_cli_command  # noqa: E402
...
class CliParserCoreTests(unittest.TestCase):
    ...
    def test_query_alias_and_budget_options(self):
        ...
        args = parser.parse_args(["query-Smash", "agent memory", "/tmp/Smash", "--budget", "small", "--json"])

        self.assertEqual(args.command, "query-Smash")
        self.assertEqual(args.query, "agent memory")
        self.assertEqual(args.target, "/tmp/Smash")
    ...
    def test_onboard_command_options(self):
        ...
        self.assertEqual(args.project, "Smash")
        self.assertEqual(args.port, 3456)
        self.assertTrue(args.json)
    ...
    def test_health_json_option(self):
        ...
        args = parser.parse_args(["health", "/tmp/Smash", "--json"])
    ...
    def test_session_end_command_options(self):
        ...
        alias = parser.parse_args(["end", "notes.md", "/tmp/Smash"])
    ...
    def test_wins_command_options(self):
        ...
        args = parser.parse_args(["wins", "/tmp/Smash", "--limit", "4", "--project", "alpha", "--json"])
    ...
    def test_team_sync_command_options(self):
        ...
        args = parser.parse_args([
            ...
            "/tmp/Smash",
            "--remote",
            "git@example.com:team/smash-memory.git",
            "--json",
        ])

        self.assertEqual(args.command, "team-sync")
        self.assertEqual(args.target, "/tmp/Smash")
        self.assertEqual(args.remote, "git@example.com:team/smash-memory.git")
        self.assertTrue(args.json)
    ...
    def test_dispatch_routes_team_sync_arguments(self):
        ...
        args = parser.parse_args(["team-sync", "/tmp/Smash", "--remote", "git@example.com:team/Smash.git", "--json"])
        ...
        self.assertEqual(calls[0][0][0], Path("/tmp/Smash"))
        self.assertEqual(calls[0][1]["remote"], "git@example.com:team/Smash.git")
        self.assertTrue(calls[0][1]["json_output"])

    def test_welcome_project_and_json_options(self):
        parser = build_cli_parser()

        args = parser.parse_args(["welcome", "/tmp/Smash", "--project", "Client Launch", "--json"])
    ...
    def test_next_alias_routes_to_prompts(self):
        ...
        args = parser.parse_args(["next", "/tmp/Smash", "--project", "Client Launch", "--json"])
    ...
    def test_dispatch_routes_query_alias_to_query_handler(self):
        ...
        self.assertEqual(calls[0][0], Path("/tmp/Smash"))
    ...
    def test_dispatch_routes_start_to_start_handler(self):
        ...
        self.assertEqual(calls[0][1]["project"], "Smash")
        self.assertTrue(calls[0][1]["json_output"])
    ...
    def test_dispatch_routes_session_end_to_session_end_handler(self):
        ...
        self.assertEqual(calls[0][2]["project"], "Smash")
        self.assertTrue(calls[0][2]["json_output"])

    def test_dispatch_routes_try_arguments(self):
        parser = build_cli_parser()
        args = parser.parse_args(["try", "/tmp/smash-demo", "--force", "--serve", "--port", "3456", "--json"])
        calls = []
        ...
        self.assertEqual(calls[0][0], Path("/tmp/smash-demo"))
    ...
    def test_dispatch_routes_operations_arguments(self):
        parser = build_cli_parser()
        args = parser.parse_args(["operations", "/tmp/Smash", "--limit", "5", "--recover", "remember-1.json", "--confirm", "--json"])
        calls = []
    ...
    def test_dispatch_routes_share_arguments(self):
        parser = build_cli_parser()
        args = parser.parse_args(["share", "Prefer local memory", "/tmp/Smash", "--port", "3456", "--host", "localhost", "--json"])
        calls = []
    ...
    def test_dispatch_routes_snapshot_arguments(self):
        ...
        self.assertTrue(calls[0][1]["force"])
        self.assertEqual(calls[0][1]["title"], "Team Smash")
        self.assertTrue(calls[0][1]["json_output"])
    ...
    def test_dispatch_routes_welcome_arguments(self):
        parser = build_cli_parser()
        args = parser.parse_args(["welcome", "/tmp/Smash", "--project", "alpha", "--json"])
        calls = []
    ...
    def test_dispatch_routes_next_alias_to_prompts_handler(self):
        parser = build_cli_parser()
        args = parser.parse_args(["next", "/tmp/Smash", "--project", "alpha", "--json"])
        calls = []
