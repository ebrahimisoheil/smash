...
def make_release_root() -> Path:
    root = Path(tempfile.mkdtemp(prefix="Smash-release-test-"))
    (root / "mcp_package/smash_mcp").mkdir(parents=True)
    (root / "mcp_package/smash_core").mkdir(parents=True)
    (root / "mcp_package/pyproject.toml").write_text(
        '[project]\nname = "smash-mcp"\nversion = "1.0.5"\n',
        encoding="utf-8",
    )
    (root / "mcp_package/smash_mcp/__init__.py").write_text(
        '__version__ = "1.0.5"\n',
        encoding="utf-8",
    )
    (root / "mcp_package/smash_core/version.py").write_text(
        'SMASH_VERSION = "1.0.5"\n',
        encoding="utf-8",
    )
...
class PrepareReleaseTests(unittest.TestCase):
    def test_prepare_release_updates_versions_and_changelog(self):
        ...
        self.assertEqual(
            {path.relative_to(root).as_posix() for path in changed},
            {
                ...
                "mcp_package/smash_mcp/__init__.py",
                "mcp_package/smash_core/version.py",
                "mcp_package/server.json",
                "CHANGELOG.md",
            },
        )
        self.assertIn('version = "1.0.6"', (root / "mcp_package/pyproject.toml").read_text(encoding="utf-8"))
        self.assertIn('__version__ = "1.0.6"', (root / "mcp_package/smash_mcp/__init__.py").read_text(encoding="utf-8"))
        self.assertIn('SMASH_VERSION = "1.0.6"', (root / "mcp_package/smash_core/version.py").read_text(encoding="utf-8"))
    ...
    def test_release_commands_use_version_tag(self):
        ...
        self.assertIn("TWINE_USERNAME=__token__ python3 -m twine upload dist/smash_mcp-1.0.6*", commands)
