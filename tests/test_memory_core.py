...
from smash_core.memory import (  # noqa: E402
    slugify,
...
from smash_core.operations import pending_operations  # noqa: E402


class MemoryCoreTests(unittest.TestCase):
    def test_default_project_for_target_uses_git_root_name(self):
        root = Path(tempfile.mkdtemp(prefix="smash-memory-project-")) / "Smash Product"
        wiki = root / "wiki"
        wiki.mkdir(parents=True)
        (root / ".git").mkdir()
        (wiki / "index.md").write_text("# Index\n", encoding="utf-8")

        self.assertEqual(default_project_for_target(root), "Smash-product")
        self.assertEqual(default_project_for_target(wiki), "Smash-product")
        self.assertEqual(default_project_for_target(Path(tempfile.mkdtemp(prefix="Smash-no-project-"))), "")

    def test_memory_records_profile_and_recall(self):
        root = Path(tempfile.mkdtemp(prefix="smash-memory-core-"))
        wiki = root / "wiki"
        ...
        self.assertEqual(release_memory["project"], "Smash-product")
        self.assertEqual(profile["by_project"], {"Smash-product": 1})
    ...
    def test_retrieval_context_finds_memory_but_stays_out_of_claim_and_output(self):
        ...
        from smash_core.memory import is_existing_memory_echo
        self.assertFalse(
    ...
    def test_abstention_verdict_matches_evidence(self):
        ...
        from smash_core.memory import recall_abstention
    ...
    def test_recall_matches_common_developer_paraphrases(self):
        ...
        recalled = recall_memories(records, "auth setup", project="Smash")
    ...
    def test_memory_audit_report_builds_shared_risk_factors(self):
        ...
        audit = memory_audit_report(profile, inbox, captures, actions, project="Smash Product")

        self.assertEqual(audit["status"], "needs_attention")
        self.assertEqual(audit["project"], "Smash-product")
        self.assertEqual(
    ...
    def test_memory_audit_next_actions_formats_cli_mcp_and_web_modes(self):
        ...
        self.assertEqual(cli_actions[0]["command"], 'python3 smash.py memory-inbox "/tmp/Smash" --project "Smash-product"')
        ...
        self.assertIn('project="Smash-product"', mcp_actions[0]["command"])
        self.assertEqual(mcp_actions[1]["tool"], "capture_inbox")
        self.assertEqual(web_actions[0]["href"], "/inbox?project=Smash-product")
        self.assertEqual(web_actions[1]["href"], "/captures?project=Smash-product")
    ...
    def test_recall_and_profile_filter_project_memories(self):
        records = [
            ...
            {
                "name": "Smash-branching",
                "path": "wiki/memories/Smash-branching.md",
                "title": "Smash branching",
                "memory_type": "preference",
                "scope": "project",
                "project": "Smash",
                "status": "active",
                "tldr": "User prefers release branches for Smash.",
                "snippet": "User prefers release branches for Smash.",
                "body": "User prefers release branches for Smash.",
            },
            {
        ...
        recalled = recall_memories(records, "branches", project="Smash")
        profile = memory_profile(records, project="Smash")

        self.assertEqual([record["name"] for record in recalled], ["Smash-branching"])
        self.assertEqual(profile["project"], "Smash")
        self.assertEqual(profile["memory_count"], 2)
        self.assertEqual(profile["by_scope"]["user"], 1)
        self.assertEqual(profile["by_scope"]["project"], 1)
        self.assertEqual(profile["by_project"]["Smash"], 1)
        ...
        records = [
            {
                ...
                "status": "active",
                "tldr": "User prefers release branches for Smash work.",
                "snippet": "User prefers release branches for Smash work.",
                "body": "User prefers release branches for Smash work.",
            }
        ]
    ...
    def test_project_duplicate_proposal_command_preserves_project(self):
        records = [
            {
                ...
                "project": "Smash",
                "status": "active",
                "tldr": "Project uses release branches for Smash work.",
                "snippet": "Project uses release branches for Smash work.",
                "body": "Project uses release branches for Smash work.",
            }
        ]

        payload = propose_memories_from_text(
            "This project uses release branches for Smash work.",
            records,
            source="unit test",
            project="Smash",
        )
        ...
        self.assertEqual(action["arguments"]["project"], "Smash")
        self.assertIn("--project Smash", action["command"])
    ...
    def test_memory_proposal_command_uses_explicit_target(self):
        payload = propose_memories_from_text(
            ...
            [],
            source="unit test",
            command_target="/tmp/Smash demo",
        )
        action = payload["proposals"][0]["primary_action"]

        self.assertEqual(action["kind"], "remember")
        self.assertIn("Smash demo", action["command"])
        ...
        records = [
            {
                ...
                "scope": "project",
                "status": "active",
                "tldr": "User prefers develop branches for Smash work.",
                "snippet": "User prefers develop branches for Smash work.",
                "body": "User prefers develop branches for Smash work.",
            }
        ]
    ...
    def test_memory_resolution_logs_and_recall_state(self):
        root = Path(tempfile.mkdtemp(prefix="smash-memory-resolution-"))
        wiki = root / "wiki"
        memories = wiki / "memories"
        memories.mkdir(parents=True)
        ...
        with patch("smash_core.memory.memory_records", return_value=[]) as mocked_records:
            path, record, error = resolve_memory_page(wiki, "wiki/memories/prefer-focused-commits.md")
        self.assertIsNone(error)
        self.assertEqual(path, (memories / "prefer-focused-commits.md").resolve())
        self.assertEqual(record["title"], "Prefer focused commits")
    ...
    def test_memory_explanation_reports_audit_payload_and_graph(self):
        root = Path(tempfile.mkdtemp(prefix="smash-memory-explain-"))
        wiki = root / "wiki"
        memories = wiki / "memories"
        memories.mkdir(parents=True)
        (memories / "prefer-focused-commits.md").write_text(
    ...
    def test_memory_lifecycle_mutations_update_files_and_callbacks(self):
        root = Path(tempfile.mkdtemp(prefix="smash-memory-lifecycle-"))
        wiki = root / "wiki"
        memories = wiki / "memories"
        memories.mkdir(parents=True)
    ...
    def test_write_memory_page_refuses_secret_looking_text(self):
        root = Path(tempfile.mkdtemp(prefix="smash-memory-secret-"))
        wiki = root / "wiki"
        wiki.mkdir(parents=True)
    ...
    def test_forget_memory_redacts_log_references(self):
        from smash_core.log import append_log, verify_log_integrity

        root = Path(tempfile.mkdtemp(prefix="smash-memory-forget-"))
        wiki = root / "wiki"
    ...
    def test_write_memory_page_stores_bounded_context_in_frontmatter(self):
        root = Path(tempfile.mkdtemp(prefix="smash-memory-ctx-"))
        wiki = root / "wiki"
        wiki.mkdir(parents=True)
    ...
    def test_write_memory_page_creates_index_log_and_blocks_duplicates(self):
        root = Path(tempfile.mkdtemp(prefix="smash-memory-write-"))
        wiki = root / "wiki"
        wiki.mkdir(parents=True)
        logged: list[tuple[str, str, str, list[str]]] = []
        rebuilds = []
    ...
    def test_write_memory_page_allows_explicit_team_visibility(self):
        root = Path(tempfile.mkdtemp(prefix="smash-memory-visibility-"))
        wiki = root / "wiki"
        wiki.mkdir(parents=True)
