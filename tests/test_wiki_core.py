...
from smash_core.wiki import (  # noqa: E402
    build_backlinks,
...
class WikiCoreTests(unittest.TestCase):
    def make_wiki(self) -> Path:
        root = Path(tempfile.mkdtemp(prefix="Smash-wiki-core-"))
        wiki = root / "wiki"
        wiki.mkdir()
    ...
    def test_cache_search_context_and_graph(self):
        ...
        (wiki / "_backlinks.json").write_text(
            json.dumps({"backlinks": {"agent-memory": ["Smash"]}, "forward": {"Smash": ["agent-memory"]}}),
            encoding="utf-8",
        )
        ...
        self.assertIn("references", cache["text_words_index"]["Smash"])
        self.assertIn("Links to [[Smash]]", cache["body_index"]["agent-memory"])
        ...
        self.assertEqual([page["name"] for page in context["pages"]], ["agent-memory", "Smash", "retrieval"])
        self.assertEqual(context_read_counts, {})
        self.assertEqual(cache["forward_links_index"]["agent-memory"], ["Smash", "retrieval"])
    ...
    def test_build_backlinks_from_cache_matches_body_scan(self):
        ...
        write_page(
            ...
            "concepts/agent-memory.md",
            "---\ntype: concept\ntitle: Agent Memory\n---\n"
            "# Agent Memory\n\nLinks to [[Smash]], [[Smash]], [[retrieval]], and [[missing]].\n",
        )
        write_page(
            wiki,
            "entities/Smash.md",
    ...
    def test_build_wiki_cache_reports_read_warnings(self):
        ...
        self.assertFalse((wiki.parent / ".smash-cache/wiki-cache-v2.json").exists())
    ...
    def test_build_wiki_cache_uses_persistent_page_records_when_unchanged(self):
        ...
        self.assertTrue((wiki.parent / ".smash-cache/wiki-cache-v2.json").exists())
        if first["search_backend"] == "sqlite-fts":
            ...
            self.assertTrue((wiki.parent / ".smash-cache/page-fts-v1.sqlite").exists())
    ...
    def test_build_wiki_cache_does_not_create_persistent_cache_for_missing_wiki(self):
        ...
        self.assertFalse((root / ".smash-cache").exists())
    ...
    def test_rebuild_index_generates_category_catalog(self):
        ...
        self.assertIn("- [[session]] - Source notes for Smash. (source)", index_text)
    ...
    def test_index_build_closes_owned_cache(self):
        ...
        with patch("smash_core.wiki.build_wiki_cache", return_value=cache):
