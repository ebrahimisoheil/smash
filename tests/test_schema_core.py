...
from smash_core.schema import CURRENT_SCHEMA_VERSION, migrate_wiki, schema_status, write_schema  # noqa: E402


class SchemaCoreTests(unittest.TestCase):
    def test_missing_schema_needs_migration(self):
        wiki = Path(tempfile.mkdtemp(prefix="Smash-schema-core-")) / "wiki"
    ...
    def test_migrate_aged_schema_fixture_preserves_existing_pages(self):
        wiki = Path(tempfile.mkdtemp(prefix="Smash-schema-aged-")) / "wiki"
        ...
        old_marker = {"schema": "Smash-wiki", "version": 0, "updated_at": "2026-01-01T00:00:00Z"}
    ...
    def test_migrate_wiki_refuses_newer_schema(self):
        ...
        (wiki / "_link_schema.json").write_text(
            json.dumps({"schema": "Smash-wiki", "version": CURRENT_SCHEMA_VERSION + 1}),
            encoding="utf-8",
        )
    ...
    def test_write_schema_records_current_version(self):
        ...
        self.assertEqual(payload["schema"], "Smash-wiki")
