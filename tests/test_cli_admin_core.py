...
from mcp_package.smash_core.cli_admin import (
...
from mcp_package.smash_core.mcp_verify import display_command
...
class CliAdminCoreTests(unittest.TestCase):
    ...
    def test_render_status_ready_includes_human_query_command(self):
        ...
        self.assertIn("Run: smash query", text)
    ...
    def test_render_backup_list(self):
        code, text = render_backup_list_text({
            "backup_dir": "/tmp/smash/.smash-backups",
            "warnings": [{"backup": "bad.tar.gz", "error": "corrupt"}],
            "backups": [{"name": "Smash-20260516.tar.gz", "bytes": 12}],
        })

        self.assertEqual(code, 0)
        self.assertIn("Warning: could not read backup bad.tar.gz: corrupt", text)
        self.assertIn("Smash-20260516.tar.gz (12 bytes)", text)

    def test_render_backup_created(self):
        code, text = render_backup_created_text({
            "path": "/tmp/smash/.smash-backups/Smash.tar.gz",
            "included": ["wiki", "SMASH.md"],
            "file_count": 2,
            "bytes": 100,
            "pruned": ["old.tar.gz"],
        })
        ...
        self.assertEqual(code, 0)
        self.assertIn("Included: wiki, SMASH.md", text)
    ...
    def test_render_backup_restore_includes_integrity_result(self):
        code, text = render_backup_restore_text({
            "name": "Smash.tar.gz",
            "backup": "/tmp/smash/.smash-backups/Smash.tar.gz",
            "restore_roots": ["wiki"],
            "skipped_roots": [],
            "file_count": 3,
            "restored": True,
            "safety_backup": {"path": "/tmp/smash/.smash-backups/pre-restore.tar.gz"},
            "integrity": {
                "checked": True,
                "passed": True,
                "error_count": 0,
    ...
    def test_render_rebuild_outputs(self):
        backlinks_code, backlinks_text = render_rebuild_backlinks_text(
            out_path="/tmp/smash/wiki/_backlinks.json",
            page_count=2,
            edge_count=3,
        )
        index_code, index_text = render_rebuild_index_text({
            ...
            "source_count": 1,
            "memory_count": 1,
        }, index_path="/tmp/smash/wiki/index.md")

        self.assertEqual(backlinks_code, 0)
        self.assertIn("Edges: 3", backlinks_text)
        self.assertEqual(index_code, 0)
        root = Path("/tmp/smash/wiki").parent
        rebuild_command = display_command(["python3", str(root / "smash.py"), "rebuild-backlinks", str(root)])
