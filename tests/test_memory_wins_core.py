...
from smash_core.memory_wins import memory_wins_payload  # noqa: E402
...
class MemoryWinsCoreTests(unittest.TestCase):
    def test_memory_wins_payload_summarizes_local_signals_without_telemetry(self):
        root = Path(tempfile.mkdtemp(prefix="smash-memory-wins-"))
        wiki = root / "wiki"
    ...
    def test_memory_wins_project_filter_keeps_global_memory_and_matching_project(self):
        root = Path(tempfile.mkdtemp(prefix="smash-memory-wins-project-"))
        wiki = root / "wiki"
        wiki.mkdir(parents=True)
