...
from smash_core.web_health import render_health_page  # noqa: E402
...
def test_render_health_page_shows_readiness_operations_and_commands(tmp_path):
            ...
            "next_actions": [
                {
                    ...
                    "command": f"smash operations {tmp_path}",
                }
            ],
    ...
    assert "Repair In Browser" in html
    assert 'data-server-action="/api/migrate"' in html
    assert 'data-server-action="/api/rebuild-index"' in html
    ...
    assert "smash operations" in html
    assert "smash benchmark" in html
