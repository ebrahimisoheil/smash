...
class ReleaseHygieneTests(unittest.TestCase):
    def test_read_changelog_versions(self):
        tmp = Path(tempfile.mkdtemp(prefix="Smash-changelog-test-"))
    ...
    def test_version_values_require_server_package_version(self):
        ...
        release_hygiene.check_version_values(
            findings,
            {
                ...
                "mcp_package/smash_mcp/__init__.py": "1.0.5",
                "mcp_package/smash_core/version.py": "1.0.5",
                "mcp_package/server.json": "1.0.5",
            },
            set(),
        )

        self.assertIn("version mismatch: server.json has no smash-mcp package version", findings)
    ...
    def test_agent_contract_requires_core_public_workflow_terms(self):
        tmp = Path(tempfile.mkdtemp(prefix="Smash-agent-contract-test-"))
        good = tmp / "good.md"
        bad = tmp / "bad.md"
        required_terms = (
            "status",
    ...
    def test_installed_agent_instructions_include_project_seed_recovery(self):
        global_text = (ROOT / "integrations/_shared/smash-instructions.md").read_text(encoding="utf-8")
        project_text = (ROOT / "integrations/_shared/smash-instructions-project.md").read_text(encoding="utf-8")

        self.assertIn("no useful project context", global_text)
        self.assertIn("smash seed . ~/Smash", global_text)
        self.assertIn("does not create durable memory", global_text)
        self.assertIn("no useful project context", project_text)
        self.assertIn("python3 smash.py seed . .", project_text)
    ...
    def test_tracked_path_hygiene_blocks_build_artifacts_and_secret_names(self):
        ...
        skip_wheel = release_hygiene.check_tracked_path_hygiene(
            findings,
            Path("mcp_package/dist/smash_mcp-1.2.0-py3-none-any.whl"),
        )
        ...
        self.assertIn(
            "build artifact should not be tracked: mcp_package/dist/smash_mcp-1.2.0-py3-none-any.whl",
            findings,
        )
    ...
    def test_release_hygiene_formats_windows_paths_as_posix(self):
        ...
        release_hygiene.check_tracked_path_hygiene(findings, PureWindowsPath("mcp_package/dist/smash_mcp.whl"))
        release_hygiene.check_outbound_network_hygiene(
            findings,
            PureWindowsPath("mcp_package/smash_core/socket_client.py"),
            "import socket\n",
        )

        self.assertIn("build artifact should not be tracked: mcp_package/dist/smash_mcp.whl", findings)
        self.assertIn("outbound network code in mcp_package/smash_core/socket_client.py: socket import", findings)
        self.assertNotIn("\\", "\n".join(findings))
    ...
    def test_outbound_network_hygiene_blocks_runtime_http_clients(self):
        ...
        release_hygiene.check_outbound_network_hygiene(
            findings,
            Path("mcp_package/smash_core/example.py"),
        ...
        release_hygiene.check_outbound_network_hygiene(
            findings,
            Path("mcp_package/smash_core/stdlib.py"),
            "import http.client\n",
        )
        release_hygiene.check_outbound_network_hygiene(
            findings,
            Path("mcp_package/smash_core/urllib_alias.py"),
            "from urllib import request\n",
        )
        release_hygiene.check_outbound_network_hygiene(
            findings,
            Path("mcp_package/smash_core/socket_client.py"),
            "import socket\n",
        )
        release_hygiene.check_outbound_network_hygiene(
            findings,
        ...
        self.assertIn("outbound network code in mcp_package/smash_core/example.py: requests import", findings)
        self.assertIn("outbound network code in integrations/example/install.sh: curl command", findings)
        self.assertIn("outbound network code in mcp_package/smash_core/stdlib.py: http.client import", findings)
        self.assertIn("outbound network code in mcp_package/smash_core/urllib_alias.py: urllib request import", findings)
        self.assertIn("outbound network code in mcp_package/smash_core/socket_client.py: socket import", findings)
