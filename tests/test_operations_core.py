...
from smash_core.operations import (  # noqa: E402
    begin_operation,
    operation_journal,
    operation_report,
    pending_operations,
...
class OperationsCoreTests(unittest.TestCase):
    def test_operation_journal_clears_marker_on_success(self):
        wiki = Path(tempfile.mkdtemp(prefix="Smash-operations-core-")) / "wiki"
        ...
        self.assertEqual(list((wiki / ".smash-operations").glob("remember-*")), [])
    ...
    def test_operation_report_renders_clear_state(self):
        ...
        self.assertIn("No pending, failed, or interrupted Smash operations.", text)
    ...
    def test_operation_report_renders_stale_marker_guidance(self):
        ...
        self.assertIn("smash validate", text)
        self.assertIn(str(wiki.parent.resolve()), text)
