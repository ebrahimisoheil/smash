...
from smash_core.prompts import starter_prompt_payload, welcome_payload  # noqa: E402
...
class PromptsCoreTests(unittest.TestCase):
    ...
    def test_global_wiki_gets_personal_memory_prompts(self):
        root = Path(tempfile.mkdtemp(prefix="Smash-prompts-core-"))
        ...
        self.assertIn("what does Smash know about me?", prompts)
        self.assertIn("seed this project into Smash", prompts)
        self.assertIn("propose memories from raw/<file>", prompts)
        self.assertTrue(str(payload["shortcut"]).startswith("smash next "))
        self.assertTrue(any(command.startswith("smash seed . ") for command in payload["commands"]))
    ...
    def test_git_project_gets_project_memory_prompts(self):
        ...
        self.assertIn("remember that this project uses Smash for local agent memory", prompts)
    ...
    def test_welcome_payload_returns_short_proof_path(self):
        ...
        self.assertIn("Agent can find Smash", payload["steps"][0]["proves"])
        self.assertTrue(any(command.startswith("smash serve ") for command in payload["commands"]))
        self.assertTrue(any(str(root.resolve()) in command for command in payload["commands"]))
