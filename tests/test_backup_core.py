...
from smash_core.backup import BackupError, RestoreError, create_backup, list_backups, restore_backup


class BackupCoreTests(unittest.TestCase):
    def make_root(self) -> Path:
        root = Path(tempfile.mkdtemp(prefix="Smash-backup-core-"))
    ...
    def test_backup_failure_removes_partial_archive(self):
        ...
        self.assertEqual(list((root / ".smash-backups").glob("*.tar.gz")), [])
    ...
    def test_backup_skips_symlinks(self):
        ...
        os.symlink(outside, root / "wiki/concepts/outside-Smash.md")

        result = create_backup(root)

        with tarfile.open(result["path"], "r:gz") as tar:
            names = set(tar.getnames())
        self.assertNotIn("wiki/concepts/outside-Smash.md", names)
    ...
    def test_restore_backup_rejects_unsafe_tar_paths(self):
        root = self.make_root()
        archive = root / ".smash-backups" / "bad.tar.gz"
