...
from smash_core.audit_export import build_compliance_export, log_entries, write_compliance_export  # noqa: E402
from smash_core.log import append_log  # noqa: E402
from smash_core.schema import migrate_wiki  # noqa: E402
from smash_core.wiki import build_backlinks  # noqa: E402


class AuditExportCoreTests(unittest.TestCase):
    def make_wiki(self) -> Path:
        root = Path(tempfile.mkdtemp(prefix="Smash-audit-export-"))
        wiki = root / "wiki"
        migrate_wiki(wiki)
        (wiki / "index.md").write_text("# Index\n", encoding="utf-8")
        (wiki / "log.md").write_text("# Smash Wiki Log\n\n", encoding="utf-8")
        memory = (
            "---\n"
            "type: memory\n"
    ...
    def test_build_compliance_export_excludes_memory_body(self):
        ...
        self.assertEqual(payload["schema"], "Smash-compliance-export-v1")
