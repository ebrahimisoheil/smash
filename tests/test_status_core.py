...
from smash_core import status as status_core  # noqa: E402
from smash_core.operations import begin_operation  # noqa: E402
from smash_core.status import link_status  # noqa: E402
from smash_core.schema import write_schema  # noqa: E402
from smash_core.wiki import build_backlinks, build_wiki_cache  # noqa: E402
...
class StatusCoreTests(unittest.TestCase):
    def make_wiki(self) -> Path:
        root = Path(tempfile.mkdtemp(prefix="Smash-status-core-"))
        wiki = root / "wiki"
    ...
    def test_link_status_warns_when_workspace_runtime_is_older(self):
        with tempfile.TemporaryDirectory() as temp:
            root = Path(temp)
            (root / "smash.py").write_text("# runtime copy\n", encoding="utf-8")
            ...
            self.assertIn("smash init", stale["detail"])

    def test_link_status_leaves_newer_workspace_runtimes_alone(self):
        with tempfile.TemporaryDirectory() as temp:
            root = Path(temp)
            (root / "smash.py").write_text("# dev checkout copy\n", encoding="utf-8")
            (root / "smash_core").mkdir()
            (root / "smash_core" / "version.py").write_text(
                'SMASH_VERSION = "9.9.9"\n', encoding="utf-8",
            )
    ...
    def test_link_status_reports_missing_structure(self):
        wiki = Path(tempfile.mkdtemp(prefix="Smash-status-core-")) / "wiki"
