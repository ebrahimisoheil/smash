...
SPEC = importlib.util.spec_from_file_location("link_cli", ROOT / "smash.py")
link_cli = importlib.util.module_from_spec(SPEC)
assert SPEC.loader is not None
SPEC.loader.exec_module(link_cli)

from smash_core.operations import begin_operation  # noqa: E402
...
class LinkCliTests(unittest.TestCase):
    def test_init_creates_empty_wiki(self):
        tmp = Path(tempfile.mkdtemp(prefix="Smash-init-test-"))
        target = tmp / "my-Smash"
        ...
        self.assertTrue((target / "api.py").exists())
        self.assertTrue((target / "serve.py").exists())
        self.assertTrue((target / "smash.py").exists())
        self.assertTrue((target / "SMASH.md").exists())
        self.assertIn("raw/*", (target / ".gitignore").read_text(encoding="utf-8"))
        self.assertTrue((target / "smash_core/frontmatter.py").exists())
        ...
        self.assertIn("smash health", out.getvalue())
        self.assertIn("smash serve", out.getvalue())
    ...
    def test_init_copies_core_from_installed_runtime_layout(self):
        ...
        runtime = tmp / "runtime"
        runtime.mkdir()
        for name in ("api.py", "serve.py", "smash.py", "SMASH.md", ".smashignore"):
            (runtime / name).write_text(f"# {name}\n", encoding="utf-8")
        (runtime / "smash_core").mkdir()
        (runtime / "smash_core/frontmatter.py").write_text("# core\n", encoding="utf-8")
        target = tmp / "my-Smash"
    ...
    def test_prompts_prints_first_run_agent_prompts(self):
        tmp = Path(tempfile.mkdtemp(prefix="Smash-prompts-test-"))
        target = tmp / "my-Smash"
        ...
        self.assertEqual(code, 0)
        self.assertIn("Smash starter prompts:", out.getvalue())
        self.assertIn("is Smash ready?", out.getvalue())
        self.assertIn("start with Smash before we continue", out.getvalue())
        self.assertIn("remember that I prefer local-first agent memory", out.getvalue())
        self.assertIn("what does Smash know about me?", out.getvalue())
    ...
    def test_prompts_json_supports_project_examples(self):
        ...
        self.assertIn("seed this project into Smash", prompts)
        self.assertTrue(any("this project uses Smash" in prompt for prompt in prompts))
        self.assertIn("what does Smash remember about this project?", prompts)

    def test_proof_creates_and_recalls_cross_agent_memory(self):
        tmp = Path(tempfile.mkdtemp(prefix="Smash-proof-test-"))
        ...
        self.assertIn("Cross-agent Smash proof", payload["memory"]["title"])
        self.assertTrue((target / "wiki/memories").is_dir())
        self.assertIn("start with Smash", payload["prompts"]["agent_b"])

    def test_onboard_json_seeds_memory_and_personalizes_prompt(self):
        tmp = Path(tempfile.mkdtemp(prefix="Smash-onboard-test-"))
        target = tmp / "my-Smash"
    ...
    def test_seed_project_initializes_workspace_and_writes_source_context(self):
        tmp = Path(tempfile.mkdtemp(prefix="Smash-seed-test-"))
        project = tmp / "client-app"
        target = tmp / "my-Smash"
        project.mkdir()
        (project / "README.md").write_text(
    ...
    def test_welcome_prints_short_first_use_path(self):
        tmp = Path(tempfile.mkdtemp(prefix="Smash-welcome-test-"))
        target = tmp / "my-Smash"
        ...
        self.assertEqual(code, 0)
        text = out.getvalue()
        self.assertIn("Smash welcome:", text)
    ...
    def test_serve_reports_missing_wiki(self):
        ...
        self.assertIn("Smash wiki missing", out.getvalue())
        self.assertIn("smash init", out.getvalue())
    ...
    def test_api_runs_target_api_runner(self):
        tmp = Path(tempfile.mkdtemp(prefix="Smash-api-test-"))
        target = tmp / "demo"
        create_demo_quiet(target)

        with patch.object(link_cli.subprocess, "run") as run:
            run.return_value.returncode = 0
            code = link_cli.serve_api(target, port=3011)

        self.assertEqual(code, 0)
        run.assert_called_once_with([
            sys.executable,
            str(link_cli.ROOT / "api.py"),
            "--root",
            str(target.resolve()),
            "--port",
            "3011",
    ...
    def test_api_reports_missing_wiki(self):
        tmp = Path(tempfile.mkdtemp(prefix="Smash-api-test-"))

        out = StringIO()
        with redirect_stdout(out):
            code = link_cli.serve_api(tmp / "missing")
    ...
    def test_demo_creates_preingested_wiki(self):
        tmp = Path(tempfile.mkdtemp(prefix="smash-demo-test-"))
        target = tmp / "demo"

        create_demo_quiet(target)

        self.assertTrue((target / ".smash-demo").exists())
        self.assertTrue((target / "api.py").exists())
        self.assertTrue((target / "serve.py").exists())
        self.assertTrue((target / "smash.py").exists())
        self.assertTrue((target / "smash_core/frontmatter.py").exists())
        self.assertTrue((target / "smash_core/memory.py").exists())
        self.assertTrue((target / "SMASH.md").exists())
        ...
        self.assertTrue((target / "wiki/entities/Smash.md").exists())
        self.assertTrue((target / "wiki/_link_schema.json").exists())
        guide = (target / "START_HERE.md").read_text(encoding="utf-8")
        self.assertIn("query Smash for why Smash helps agents", guide)
        self.assertIn('python3 smash.py query "why does Smash help agents?" . --budget small', guide)

        backlinks = json.loads((target / "wiki/_backlinks.json").read_text(encoding="utf-8"))
        self.assertIn("backlinks", backlinks)
        self.assertIn("forward", backlinks)
        self.assertIn("agent-memory", backlinks["backlinks"])
        self.assertIn("Smash", backlinks["backlinks"])
        self.assertIn("agent-memory", backlinks["forward"]["Smash"])
    ...
    def test_demo_output_uses_copied_runtime_path(self):
        ...
        self.assertEqual(code, 0)
        self.assertIn(str(target.resolve() / "smash.py"), out.getvalue())
        self.assertIn(str(target.resolve()), out.getvalue())

    def test_import_obsidian_copies_notes_to_raw(self):
        tmp = Path(tempfile.mkdtemp(prefix="Smash-obsidian-cli-"))
        target = tmp / "Smash"
        vault = tmp / "vault"
        ...
        self.assertIn("Ask your agent: ingest raw/obsidian/vault into Smash", out.getvalue())

    def test_compliance_export_writes_redacted_audit_json(self):
        tmp = Path(tempfile.mkdtemp(prefix="Smash-compliance-cli-"))
        target = tmp / "demo"
        ...
        self.assertEqual(data["schema"], "Smash-compliance-export-v1")
        self.assertIn("memory_profile", data)
        self.assertNotIn("body", json.dumps(data["memories"]))
        self.assertIn("Smash compliance export", out.getvalue())
    ...
    def test_doctor_accepts_demo_wiki(self):
        tmp = Path(tempfile.mkdtemp(prefix="Smash-doctor-test-"))
        target = tmp / "demo"
        create_demo_quiet(target)
    ...
    def test_ingest_status_accepts_demo_wiki(self):
        tmp = Path(tempfile.mkdtemp(prefix="smash-ingest-test-"))
        target = tmp / "demo"
        create_demo_quiet(target)
    ...
    def test_ingest_status_reports_pending_raw_file(self):
        ...
        self.assertIn("Ask your agent: ingest raw/new-source.md into Smash", out.getvalue())
        self.assertIn("Run: smash validate", out.getvalue())
    ...
    def test_ingest_status_reports_represented_completion(self):
        ...
        self.assertIn("Retrieval check: query Smash for agent memory session", text)
        self.assertIn("Next check: start with Smash before we continue", text)
    ...
    def test_ingest_status_reports_stale_represented_raw_file(self):
        ...
        self.assertIn("Ask your agent: re-ingest raw/agent-memory-session.md into Smash", text)
    ...
    def test_ingest_status_warns_before_secret_raw_ingest(self):
        ...
        self.assertNotIn("Ask your agent: ingest raw/secret-note.md into Smash", out.getvalue())
    ...
    def test_ingest_status_blocks_unreadable_raw_ingest(self):
        ...
        with (
            patch(
                "smash_core.ingest.secret_file_scan",
        ...
        self.assertNotIn("Ask your agent: ingest raw/locked-note.md into Smash", text)
    ...
    def test_ingest_status_json(self):
        ...
        self.assertEqual(data["guidance"]["agent_prompt"], "ingest raw/new-source.md into Smash")
    ...
    def test_ingest_status_reports_stale_backlinks(self):
        ...
        self.assertIn("Run: smash rebuild-backlinks", out.getvalue())

    def test_status_reports_demo_readiness(self):
        tmp = Path(tempfile.mkdtemp(prefix="Smash-status-test-"))
        target = tmp / "demo"
        create_demo_quiet(target)
        ...
        self.assertEqual(code, 0)
        self.assertIn(f"Version: {link_cli.SMASH_VERSION}", out.getvalue())
        ...
        self.assertEqual(status_payload["version"], link_cli.SMASH_VERSION)
    ...
    def test_health_combines_status_and_operations(self):
        tmp = Path(tempfile.mkdtemp(prefix="smash-health-test-"))
        target = tmp / "demo"
        create_demo_quiet(target)
    ...
    def test_operations_reports_interrupted_write_markers(self):
        tmp = Path(tempfile.mkdtemp(prefix="Smash-operations-test-"))
        target = tmp / "demo"
        create_demo_quiet(target)
        begin_operation(
            target / "wiki",
        ...
        self.assertEqual(code, 1)
        self.assertIn("Smash operations:", out.getvalue())
        self.assertIn("remember | pending | stale", out.getvalue())
        self.assertIn("smash validate", out.getvalue())
    ...
    def test_status_prints_readiness_warnings(self):
        ...
        payload = {
            "ready": False,
            "version": link_cli.SMASH_VERSION,
    ...
    def test_main_prints_version(self):
        ...
        self.assertIn(f"Smash {link_cli.SMASH_VERSION}", out.getvalue())
    ...
    def test_backup_creates_local_archive_without_raw_by_default(self):
        tmp = Path(tempfile.mkdtemp(prefix="Smash-backup-test-"))
        target = tmp / "demo"
        create_demo_quiet(target)
        ...
        self.assertEqual(code, 0)
        self.assertIn("Smash backup created:", out.getvalue())
        archives = list((target / ".smash-backups").glob("*.tar.gz"))
    ...
    def test_backup_reports_controlled_error_on_archive_failure(self):
        ...
        self.assertEqual(list((target / ".smash-backups").glob("*.tar.gz")), [])
    ...
    def test_backup_list_reports_unreadable_archive_warning(self):
        ...
        archive = next((target / ".smash-backups").glob("*.tar.gz"))
    ...
    def test_migrate_repairs_schema_marker(self):
        tmp = Path(tempfile.mkdtemp(prefix="Smash-migrate-test-"))
        target = tmp / "demo"
        create_demo_quiet(target)
    ...
    def test_validate_accepts_demo_wiki(self):
        tmp = Path(tempfile.mkdtemp(prefix="Smash-validate-test-"))
        target = tmp / "demo"
        create_demo_quiet(target)
    ...
    def test_remember_creates_memory_page_and_updates_backlinks(self):
        tmp = Path(tempfile.mkdtemp(prefix="smash-memory-test-"))
        target = tmp / "demo"
        create_demo_quiet(target)
        ...
        with redirect_stdout(out):
            code = link_cli.remember(
                target,
                "User prefers release branches for Smash work.",
    ...
    def test_remember_blocks_conflict_by_default(self):
        ...
        with redirect_stdout(conflict_out):
            conflict_code = link_cli.remember(
                target,
                "User prefers develop branches for Smash work.",
    ...
    def test_update_memory_merges_text_and_resets_review(self):
        ...
        self.assertIn("prefer-local-personal-memory", backlinks["backlinks"]["Smash"])

    def test_set_memory_visibility_updates_frontmatter_and_log(self):
        tmp = Path(tempfile.mkdtemp(prefix="smash-memory-visibility-cli-"))
        target = tmp / "demo"
        create_demo_quiet(target)
    ...
    def test_propose_memories_from_session_note_without_writing(self):
        ...
        session_note.write_text(
            "\n".join([
                "- I prefer release branches for Smash work.",
    ...
    def test_profile_summarizes_memories(self):
        ...
        self.assertEqual(code, 0)
        self.assertIn("Smash memory profile", out.getvalue())
    ...
    def test_brief_primes_agent_memory(self):
        ...
        self.assertEqual(code, 0)
        self.assertIn("Smash memory brief: local personal memory", out.getvalue())
    ...
    def test_start_combines_readiness_and_brief(self):
        tmp = Path(tempfile.mkdtemp(prefix="Smash-start-test-"))
        target = tmp / "demo"
        create_demo_quiet(target)
        ...
        self.assertEqual(code, 0)
        self.assertIn("Smash start:", out.getvalue())
        self.assertIn("Ready: yes", out.getvalue())
        self.assertIn("Smash memory brief: local personal memory", out.getvalue())
        self.assertIn("Prefer local personal memory", out.getvalue())
    ...
    def test_query_builds_context_packet(self):
        tmp = Path(tempfile.mkdtemp(prefix="Smash-query-test-"))
        target = tmp / "demo"
        create_demo_quiet(target)
    ...
    def test_graph_summary_reports_bounded_context(self):
        tmp = Path(tempfile.mkdtemp(prefix="Smash-graph-summary-test-"))
        target = tmp / "demo"
        create_demo_quiet(target)
    ...
    def test_benchmark_reports_local_query_timings(self):
        tmp = Path(tempfile.mkdtemp(prefix="Smash-benchmark-test-"))
        target = tmp / "demo"
        create_demo_quiet(target)
    ...
    def test_memory_audit_reports_backlog_without_secret_values(self):
        ...
        self.assertNotIn(fake_key, json_out.getvalue())
        self.assertIn("Smash memory audit", text_out.getvalue())
    ...
    def test_capture_session_writes_raw_note_and_proposes_only(self):
        ...
        with redirect_stdout(out):
            code = link_cli.capture_session(
                target,
                f"Remember that the user prefers release branches for Smash work. Test key {fake_key}",
        ...
        self.assertEqual(payload["project"], "Smash")
        self.assertTrue(payload["path"].startswith("raw/memory-captures/"))
        self.assertIn('project: "Smash"', capture_text)
    ...
    def test_session_end_writes_proposal_only_capture(self):
        tmp = Path(tempfile.mkdtemp(prefix="Smash-session-end-test-"))
        target = tmp / "demo"
        create_demo_quiet(target)
        before_memories = list((target / "wiki/memories").glob("*.md"))
        ...
        with redirect_stdout(out):
            code = link_cli.session_end(
                target,
                "We decided Smash session-end should propose memories but never save them without approval.",
                project="Smash",
                json_output=True,
            )
    ...
    def test_accept_capture_carries_procedure_trigger_into_frontmatter(self):
        ...
        tmp = Path(tempfile.mkdtemp(prefix="Smash-trigger-accept-"))
        target = tmp / "demo"
        create_demo_quiet(target)
    ...
    def test_memory_inbox_filters_by_project(self):
        ...
        with redirect_stdout(StringIO()):
            ...
            alpha_code = link_cli.remember(
                target,
                "Alpha project stores deployment context in Smash.",
            ...
            beta_code = link_cli.remember(
                target,
                "Beta project stores design context in Smash.",
    ...
    def test_explain_memory_reports_trust_state_and_graph(self):
        ...
        self.assertIn("Smash", payload["graph"]["forward"])
    ...
    def test_verify_mcp_ready(self):
        tmp = Path(tempfile.mkdtemp(prefix="Smash-verify-test-"))
        target = tmp / "demo"
        create_demo_quiet(target)
        ...
        with redirect_stdout(out):
            code = link_cli.verify_mcp(
                ...
                import_check=lambda _: {"installed": True, "version": link_cli.SMASH_VERSION, "error": None},
            )

        self.assertEqual(code, 0)
        self.assertIn(f"smash-mcp: installed ({link_cli.SMASH_VERSION})", out.getvalue())
    ...
    def test_verify_mcp_uses_installer_python_marker(self):
        ...
        target = tmp / "demo"
        create_demo_quiet(target)
        (target / ".smash-mcp-python").write_text("/tmp/smash-mcp-venv/bin/python\n", encoding="utf-8")
        checked = []
        ...
        with redirect_stdout(out):
            code = link_cli.verify_mcp(
                target,
                import_check=lambda cmd: checked.append(cmd) or {"installed": True, "version": link_cli.SMASH_VERSION, "error": None},
            )

        self.assertEqual(code, 0)
        self.assertEqual(checked, ["/tmp/smash-mcp-venv/bin/python"])
        self.assertIn('"command": "/tmp/smash-mcp-venv/bin/python"', out.getvalue())
    ...
    def test_verify_mcp_json(self):
        ...
        self.assertEqual(data["expected_version"], link_cli.SMASH_VERSION)
        self.assertTrue(data["version_matches"])
        self.assertEqual(data["issues"], [])
        self.assertEqual(data["next_actions"], [])
        self.assertTrue(data["smash_mcp"]["mcp_sdk"])
        self.assertEqual(data["smash_mcp"]["version"], link_cli.SMASH_VERSION)
        self.assertEqual(data["config"]["mcpServers"]["Smash"]["command"], "/tmp/python")
    ...
    def test_verify_mcp_json_reports_repair_actions(self):
        ...
        with redirect_stdout(out):
            code = link_cli.verify_mcp(
                target,
                json_output=True,
                python_cmd="/tmp/Smash Python/bin/python",
        ...
        self.assertFalse(data["smash_mcp"]["mcp_sdk"])
        ...
        self.assertEqual(
            ...
            ["reinstall_smash_mcp", "upgrade_smash_mcp"],
        )
        self.assertEqual(
            data["next_actions"][0]["command"],
            [
                "/tmp/Smash Python/bin/python",
                "-m",
                "pip",
                "install",
                "--upgrade",
                f"smash-mcp=={link_cli.SMASH_VERSION}",
            ],
        )
        self.assertEqual(
    ...
    def test_check_smash_mcp_import_requires_mcp_sdk(self):
        stdout = json.dumps({
            "installed": True,
            "version": link_cli.SMASH_VERSION,
            "mcp_sdk": False,
        ...
        with patch.object(
            link_cli._core_check_smash_mcp_import.__globals__["subprocess"],
            "run",
            return_value=subprocess.CompletedProcess(["/tmp/python"], 0, stdout=stdout, stderr=""),
        ) as run:
            payload = link_cli._core_check_smash_mcp_import("/tmp/python")

        self.assertTrue(payload["installed"])
        self.assertEqual(payload["version"], link_cli.SMASH_VERSION)
    ...
    def test_verify_mcp_reports_version_mismatch(self):
        ...
        self.assertEqual(code, 1)
        text = out.getvalue()
        self.assertIn("smash-mcp: installed (0.9.0)", text)
        self.assertIn(f"Expected version: {link_cli.SMASH_VERSION}", text)
    ...
    def test_verify_mcp_reports_missing_mcp_sdk_dependency(self):
        ...
        self.assertEqual(code, 1)
        text = out.getvalue()
        self.assertIn(f"smash-mcp: installed ({link_cli.SMASH_VERSION})", text)
        self.assertIn("MCP SDK: missing", text)
        self.assertIn("Import error: No module named mcp", text)
        self.assertIn(f"/tmp/python -m pip install --upgrade smash-mcp=={link_cli.SMASH_VERSION}", text)
    ...
    def test_verify_mcp_reports_missing_package(self):
        ...
        with redirect_stdout(out):
            code = link_cli.verify_mcp(
                target,
                python_cmd="/tmp/python",
                import_check=lambda _: {"installed": False, "version": None, "error": "No module named smash_mcp"},
            )

        self.assertEqual(code, 1)
        self.assertIn("smash-mcp: missing", out.getvalue())
        self.assertIn("/tmp/python -m pip install --upgrade smash-mcp", out.getvalue())
    ...
    def test_verify_mcp_reports_missing_wiki(self):
        ...
        self.assertIn("python3 smash.py init", out.getvalue())
    ...
    def test_rebuild_index_repairs_missing_catalog_entries(self):
        tmp = Path(tempfile.mkdtemp(prefix="Smash-index-test-"))
        target = tmp / "demo"
        create_demo_quiet(target)
...
class AgentHookCliTests(unittest.TestCase):
    ...
    def test_hook_session_start_prints_memory_brief(self):
        tmp = Path(tempfile.mkdtemp(prefix="Smash-hook-test-"))
        target = tmp / "demo"
        create_demo_quiet(target)
        ...
        self.assertEqual(code, 0)
        text = out.getvalue()
        self.assertIn("Smash memory (local, source-backed)", text)
    ...
    def test_hook_session_start_cursor_emit_wraps_additional_context(self):
        ...
        self.assertEqual(code, 0)
        payload = json.loads(out.getvalue())
        self.assertIn("Smash memory (local, source-backed)", payload["additional_context"])
    ...
    def test_hook_session_end_never_recaptures_existing_memory(self):
        tmp = Path(tempfile.mkdtemp(prefix="Smash-echo-test-"))
        target = tmp / "demo"
        create_demo_quiet(target)
    ...
    def test_hook_session_end_ignores_assistant_prose(self):
        tmp = Path(tempfile.mkdtemp(prefix="Smash-assistant-prose-"))
        target = tmp / "demo"
        create_demo_quiet(target)
        transcript = tmp / "transcript.jsonl"
        ...
        transcript.write_text(
            "\n".join([
                json.dumps({"type": "user", "message": {"role": "user", "content":
                    "Smash memory (local, source-backed) · project demo\nRelevant memories\n"
    ...
    def test_hook_session_end_captures_user_stated_decision(self):
        tmp = Path(tempfile.mkdtemp(prefix="Smash-user-decision-"))
        target = tmp / "demo"
        create_demo_quiet(target)
        transcript = tmp / "transcript.jsonl"
        transcript.write_text(
    ...
    def test_session_end_explain_prints_decision_trail(self):
        tmp = Path(tempfile.mkdtemp(prefix="Smash-explain-test-"))
        target = tmp / "demo"
        create_demo_quiet(target)
        transcript = tmp / "transcript.jsonl"
        transcript.write_text(
            json.dumps({"type": "user", "message": {"role": "user", "content":
                "Smash memory (local, source-backed) · injected brief echo"}}) + "\n" +
            json.dumps({"type": "user", "message": {"role": "user", "content": "hi"}}),
            encoding="utf-8",
        )
        ...
        self.assertEqual(code, 0)
        text = out.getvalue()
        self.assertIn("dropped 1 carrying Smash's own injected output", text)
        self.assertIn("trivial session", text)

    def test_conflict_output_offers_supersede_command(self):
        tmp = Path(tempfile.mkdtemp(prefix="Smash-supersede-ux-"))
    ...
    def test_recall_text_shows_applicability_and_steps(self):
        tmp = Path(tempfile.mkdtemp(prefix="Smash-recall-text-"))
        target = tmp / "wiki-root"
        with redirect_stdout(StringIO()):
            link_cli.init_wiki(target)
            link_cli.remember(
    ...
    def test_connect_hooks_preview_includes_session_hooks_payload(self):
        ...
        self.assertIn(str(Path(target.name) / "smash.py"), session_hooks["events"]["SessionStart"])
...
class DefaultWorkspaceFallbackTests(unittest.TestCase):
    ...
    the default workspace instead of dead-ending — onboard creates ~/Smash
    and the next thing every new user types is `smash remember` with no path.
    """
    ...
    def test_pathless_consumer_command_falls_back_to_workspace(self):
        tmp = Path(tempfile.mkdtemp(prefix="Smash-ws-fallback-"))
        workspace = tmp / "workspace"
        (workspace / "wiki").mkdir(parents=True)
        elsewhere = tmp / "elsewhere"
        elsewhere.mkdir()
        previous_cwd = Path.cwd()
        previous_env = os.environ.get("SMASH_WORKSPACE")
        try:
            os.chdir(elsewhere)
            os.environ["SMASH_WORKSPACE"] = str(workspace)
            ...
            if previous_env is None:
                os.environ.pop("SMASH_WORKSPACE", None)
            else:
                os.environ["SMASH_WORKSPACE"] = previous_env

    def test_creator_commands_and_explicit_targets_never_redirect(self):
        tmp = Path(tempfile.mkdtemp(prefix="Smash-ws-noredirect-"))
        workspace = tmp / "workspace"
        (workspace / "wiki").mkdir(parents=True)
        elsewhere = tmp / "elsewhere"
        elsewhere.mkdir()
    ...
    def test_cwd_with_wiki_wins_over_workspace(self):
        tmp = Path(tempfile.mkdtemp(prefix="Smash-ws-cwdwins-"))
...
class LnkCommandDisplayTests(unittest.TestCase):
    @unittest.skipIf(
        os.name == "nt",
        "POSIX shim scenario: a #!/bin/sh `smash` on PATH is a Homebrew "
    ...
    def test_shim_on_path_switches_generated_commands_to_lnk(self):
        # A `smash` on PATH that wraps THIS runtime means the user installed a
        # launcher (e.g. Homebrew): generated commands must say `smash`, never
        # interpreter + install path.
        tmp = Path(tempfile.mkdtemp(prefix="Smash-shim-"))
        shim = tmp / "smash"
        shim.write_text(f'#!/bin/sh\nexec python3 "{link_cli.ROOT / "smash.py"}" "$@"\n', encoding="utf-8")
        shim.chmod(0o755)
        ...
        try:
            ...
            rendered = link_cli._display_command(["smash", "status"])
            self.assertEqual(rendered, "smash status")
        finally:
            os.environ["PATH"] = previous_path
            link_cli._core_set_link_command_override(None)

    def test_no_shim_keeps_source_checkout_command(self):
        tmp = Path(tempfile.mkdtemp(prefix="Smash-noshim-"))
        previous_path = os.environ.get("PATH", "")
        try:
            os.environ["PATH"] = str(tmp)  # no smash anywhere
