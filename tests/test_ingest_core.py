...
from smash_core.ingest import collect_ingest_status, raw_ingest_findings, render_ingest_status_text, source_matches_by_raw  # noqa: E402
from smash_core.wiki import build_backlinks  # noqa: E402
...
class IngestCoreTests(unittest.TestCase):
    def test_collect_ingest_status_reports_missing_structure(self):
        root = Path(tempfile.mkdtemp(prefix="smash-ingest-core-"))
        ...
        self.assertIn("Run an installer or initialize this directory: smash init", text)
    ...
    def test_collect_ingest_status_reports_pending_raw(self):
        ...
        self.assertEqual(payload["guidance"]["agent_prompt"], "ingest raw/new-note.md into Smash")
        ...
        self.assertTrue(any(command.startswith("smash rebuild-index ") for command in payload["plan"]["post_checks"]))

        text = render_ingest_status_text(str(root), payload)
        self.assertIn(f"smash rebuild-index {resolved_root}", text)
        self.assertIn(f"Smash ingest status: {root}", text)
        self.assertIn("Raw files: 1", text)
        self.assertIn("Pending raw files:\n- raw/new-note.md", text)
        self.assertIn("Ask your agent: ingest raw/new-note.md into Smash", text)
        self.assertIn(f"Run: smash rebuild-index {resolved_root}", text)
        self.assertIn(f"- smash health {resolved_root}", text)
    ...
    def test_collect_ingest_status_reports_represented_raw(self):
        ...
        self.assertEqual(payload["completion"]["next_prompt"], "start with Smash before we continue")
    ...
    def test_collect_ingest_status_reports_stale_represented_raw(self):
        ...
        self.assertEqual(payload["guidance"]["agent_prompt"], "re-ingest raw/source.md into Smash")
    ...
    def test_source_matches_by_raw_compiles_one_pattern_per_chunk(self):
        ...
        with patch("smash_core.ingest.re.compile", wraps=re.compile) as mocked_compile:
