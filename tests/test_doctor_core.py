...
import smash_core.doctor as doctor_module  # noqa: E402
from smash_core.doctor import (  # noqa: E402
    DoctorReport,
    apply_doctor_fixes,
    build_doctor_report,
...
from smash_core.log import append_log  # noqa: E402
...
class DoctorCoreTests(unittest.TestCase):
    def test_render_healthy_report_with_fixes_and_warnings(self):
        report = DoctorReport("/tmp/Smash", fix_requested=True)
        ...
        self.assertIn("Smash doctor: /tmp/Smash", text)
    ...
    def test_apply_doctor_fixes_initializes_workspace(self):
        root = Path(tempfile.mkdtemp(prefix="Smash-doctor-fixes-"))
    ...
    def test_apply_doctor_fixes_refreshes_stale_workspace_runtime(self):
        root = Path(tempfile.mkdtemp(prefix="Smash-doctor-runtime-"))
        (root / "smash.py").write_text("# old runtime\n", encoding="utf-8")
        (root / "smash_core").mkdir()
        (root / "smash_core" / "version.py").write_text(
            'SMASH_VERSION = "1.0.0"\n', encoding="utf-8",
        )
        ...
        refreshed = (root / "smash_core" / "version.py").read_text(encoding="utf-8")
        self.assertNotIn('"1.0.0"', refreshed)
        self.assertGreater((root / "smash.py").stat().st_size, 100)
    ...
    def test_build_doctor_report_uses_shared_health_checks(self):
        root = Path(tempfile.mkdtemp(prefix="Smash-doctor-report-"))
    ...
    def test_build_doctor_report_fails_on_stale_operation_marker(self):
        ...
        self.assertFalse(report.healthy)
        self.assertTrue(any("incomplete Smash operations need review" in error for error in report.errors))
    ...
    def test_page_health_helpers_find_doctor_findings(self):
        root = Path(tempfile.mkdtemp(prefix="Smash-doctor-core-"))
        wiki = root / "wiki"
    ...
    def test_repair_source_page_validation_shape(self):
        root = Path(tempfile.mkdtemp(prefix="Smash-doctor-repair-"))
        page = root / "source.md"
        page.write_text(
    ...
    def test_repair_validation_findings_repairs_source_pages_only(self):
        root = Path(tempfile.mkdtemp(prefix="Smash-doctor-repair-findings-"))
        wiki = root / "wiki"
