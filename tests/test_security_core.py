...
from mcp_package.smash_core.security import (
    clean_text_input,
...
class SecurityCoreTests(unittest.TestCase):
    ...
    def test_secret_file_warnings_streams_across_chunks(self):
        tmp = Path(tempfile.mkdtemp(prefix="Smash-security-core-"))
        fake_key = "sk-" + "a" * 48
    ...
    def test_sensitive_filename_scan_skips_configured_dirs(self):
        root = Path(tempfile.mkdtemp(prefix="Smash-sensitive-name-"))
        (root / "raw").mkdir()
    ...
    def test_iter_scannable_files_skips_binary_suffixes(self):
        root = Path(tempfile.mkdtemp(prefix="Smash-scannable-"))
    ...
    def test_find_sensitive_values_reports_matches(self):
        root = Path(tempfile.mkdtemp(prefix="Smash-sensitive-values-"))
        fake_key = "sk-" + ("A" * 24)
