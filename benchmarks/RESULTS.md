# Smash recall quality benchmark

Smash's recall is measured, not asserted. This document holds the current
numbers, exactly how they were produced, and how to reproduce them on your
own machine. There are three tracks:

1. **Smash recall benchmark** — our own deterministic, fully auditable
...
recall time, keep embeddings in plain JSON under `.smash-cache/`, and use no
vector database or service:

| tier    | install                                     | model                             | load time | best for                      |
| ------- | ------------------------------------------- | --------------------------------- | --------- | ----------------------------- |
| fast    | `pip install "smash-mcp[semantic]"`         | model2vec potion-base-8M (~30 MB) | ~0.1 s    | CLI, session-start hooks      |
| quality | `pip install "smash-mcp[semantic-quality]"` | all-MiniLM-L6-v2 ONNX (~90 MB)    | ~5 s      | MCP server, long-lived agents |

The quality tier is preferred automatically when installed
(`SMASH_SEMANTIC_PROVIDER` overrides).

## Track 1: Smash recall benchmark
...
phrasing variants). Queries are grouped by _measured_ overlap: a case counts
...
Full suite, Apple M4, macOS 26.5.1, Python 3.14, run 2026-07-08, Smash
`develop` (post-1.5.0).
...
Two of Smash's most-questioned design choices now have independent academic
...
being verbatim text supplemented by distilled artifacts, which is Smash's
...
ablation below. Neither paper is affiliated with Smash.
...
query; we measure whether Smash ranks the annotated evidence turns highly.
...
ranked list at recall time _hurt_ (hit@10 0.685 → 0.550) — neighbors displace
...
lexical and semantic rankings (hit@10 0.616), Okapi BM25 replacing Smash's
...
all-MiniLM-L6-v2; `SMASH_SEMANTIC_MODEL=nomic-ai/nomic-embed-text-v1.5-Q` is
...
store stays _trustworthy_ as sessions accumulate — the axis on which
...
mid-stream revisions, plus agent echoes, Smash's own injected briefs, and
memory-free noise sessions — every event ground-truth labeled, no LLM):

- **gated** — Smash's real pipeline: extraction drops Smash-injected output,
...
- **ungated** — the same extractor and retrieval with governance off: every
  ...
  **governance ablation of Smash itself**, not a reimplementation of any
...
the system's own prompt text re-ingested). Smash's junk rate is zero **by
construction**, and CI enforces it: the hygiene gate fails any change that
stores junk or loses to the ungated baseline.

Honest notes: gated contradiction exposure is 0.333, not zero — Smash only
...
construction" means zero _self-inflicted_ junk through automatic capture
...
with a Smash backend, so the numbers are directly comparable to the raw
result files mem0 publishes in that repository. Full provenance notes,
the Smash backend adapter, and every judgment live in our benchmark
...
| ------------------------ | --------- | ---------------------- | --------- |
| **Smash (local files)**  | haiku-4-5 | haiku-4-5              | **84.8%** |
...
identical judge model that scored Smash, using the harness's own judge
...
were written by gpt-5 while Smash's came from a budget model: both
asymmetries favor mem0, and Smash still leads (multi-hop: 85.1 vs 82.3).
Their 91.6% headline configuration uses top-200 (~7k tokens/call) —
more than twice Smash's token budget.

The result also holds under a second, independent judge — Tencent
Hunyuan 3 (295B open weights, unrelated to either lab): **Smash 85.5%
...
unrelated referee, and we say so), Smash's haiku answers score **80.6%**,
and swapping Smash's answerer from haiku to Hunyuan 3 itself also lands
...
What _is_ directly measurable without any judge: replaying Smash's
deterministic ingest maps every retrieved memory to its source session,
so we can check whether retrieval surfaced the gold evidence. **Smash
...
- **Pure paraphrases are much better, not solved.** The quality tier
  ...
  of pure paraphrases still miss the top 5 on our corpus. Smash labels every
...
- **Track 1 is self-authored.** It is deterministic, auditable, and gated
  against regressions in CI, but it was written by the Smash project.
...
```bash
git clone https://github.com/ebrahimisoheil/smash && cd Smash
