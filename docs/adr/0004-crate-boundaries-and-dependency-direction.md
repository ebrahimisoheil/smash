...
|---|---|---|---|
| `V2/crates/contracts` | `smash-contracts` | serde, uuid, time, garde, utoipa | axum, sqlx, tokio |
| `V2/crates/core` | `smash-core` | `smash-contracts`, thiserror, async-trait | axum, tower, sqlx, reqwest, lancedb |
| `V2/crates/storage` | `smash-storage` | `smash-core`, sqlx, aws-sdk-s3, lancedb | axum |
| `V2/crates/api` | `smash-api` | `smash-core`, `smash-storage`, axum, tower, utoipa | — |
| `V2/crates/worker` | `smash-worker` | `smash-core`, `smash-storage`, tokio | axum |
| `V2/crates/mcp` | `smash-mcp` | `smash-core`, `smash-storage` | axum |
...
`smash-contracts` ← `smash-core` ← `smash-storage` ← {`smash-api`,
`smash-worker`, `smash-mcp`}. Nothing depends "sideways" or "up" — `api`,
...
graph everywhere except as a direct dependency of `smash-api` (verified
empirically: today those two crates have exactly one dependent each in the
whole graph — `smash-api` — so the ban has zero false positives). This is a
CI failure, not a review habit: session A1's acceptance check is planting an
`axum` import in `smash-core`, confirming `cargo deny check` fails, then
removing it.
...
above plus a dedicated CI step that walks `cargo tree -p smash-core` and
`cargo tree -p smash-contracts`, asserting none of those crate names appear
...
tool (they are legitimately several hops deep in `smash-storage`'s own
dependency tree, e.g. `lancedb` vendors `reqwest` via
`lance-namespace-reqwest-client`).

`smash-api` and `smash-worker` produce the `api` and `worker` binaries
respectively, built from one dependency-cached multi-stage `Dockerfile`
(`V2/Dockerfile`). `smash-mcp` produces an `mcp` binary but is not yet wired
...
- Adding a legitimate new dependency to `smash-api` (e.g. a new Tower layer)
  never requires touching `deny.toml`. Adding `axum` to any crate other than
  `smash-api` breaks CI immediately, by design.
- `smash-storage` is the only crate allowed to know about Postgres, S3, or
  LanceDB concretely; `smash-core` defines the ports (traits) that
  `smash-storage` implements, once Phase B introduces real domain services.
...
- **Single monolithic crate** (`smash`) with modules instead of crates —
  ...
  the way `cargo deny` can prevent `smash-core`'s `Cargo.toml` from ever
...
- **`worker` and `mcp` sharing one crate** (since both depend on exactly
  `smash-core` + `smash-storage` today) — rejected because they are
...
- **Enforcing the full "must not depend on" table via `cargo-deny` bans
  ...
  reqwest` showed `reqwest` reachable from `smash-storage` through four
  ...
  plus a targeted `cargo tree -p smash-core` CI check achieves the same
