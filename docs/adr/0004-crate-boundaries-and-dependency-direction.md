...
The workspace at `` has six crates plus two non-Cargo units:

| Path | Crate name | Depends on | Must NOT depend on |
|---|---|---|---|
| `crates/contracts` | `engrave-contracts` | serde, uuid, time, garde, utoipa | axum, sqlx, tokio |
| `crates/core` | `engrave-core` | `engrave-contracts`, thiserror, async-trait | axum, tower, sqlx, reqwest, lancedb |
| `crates/storage` | `engrave-storage` | `engrave-core`, sqlx, aws-sdk-s3, lancedb | axum |
| `crates/api` | `engrave-api` | `engrave-core`, `engrave-storage`, axum, tower, utoipa | — |
| `crates/worker` | `engrave-worker` | `engrave-core`, `engrave-storage`, tokio | axum |
| `crates/mcp` | `engrave-mcp` | `engrave-core`, `engrave-storage` | axum |
| `apps/web` | (Next.js, not a Cargo crate) | — | direct DB access |
| `eval` | fixtures/benchmark dir, not a Cargo crate | — | — |
...
`deny.toml` bans `axum` and `tower-http` from the resolved dependency
...
`.github/workflows/rust-ci.yml` and the long comment in `deny.toml`
...
(`Dockerfile`). `engrave-mcp` produces an `mcp` binary but is not yet wired
...
- `apps/web` and `eval` are placeholders: the former is a
