...
All notable changes to Smash are tracked here.

Release sections use `MAJOR.MINOR.PATCH` versions that match `smash-mcp` on PyPI and the MCP Registry. Keep `Unreleased` for work merged after the latest published version.

## [Unreleased]

## [2.0.0] - 2026-07-27

Smash gets a face. The memory layer is unchanged in shape — plain Markdown,
review-gated writes, no LLM in the write path — but it is no longer only a
CLI and an MCP server: **SmashBar** puts the review gate in your macOS menu
...
from 1.7 works exactly as before; upgrading is `brew upgrade Smash`.

### Added

- **SmashBar 1.0** — Smash's memory, ambient in your macOS menu bar (`apps/SmashBar`, Swift/SwiftUI, the `smash --json` CLI is its entire backend). The review gate stops being a place you go and starts being something that meets you:
  ...
  - **Live agent pulse**: SmashBar detects agent sessions writing transcripts right now and shows a breathing "N agents active · project" row; the menu-bar icon carries a quiet green dot while memory is being made.
  ...
  - **Inbox**: review/accept memories and captures with "Will save" previews, decision trails ("How Smash read this session"), and per-proposal Accept menus.
  - Distributed as an unsigned cask with a postflight quarantine-strip — `brew install --cask ebrahimisoheil/smash/smashbar` — zero Gatekeeper friction, zero signing fees.
- Homepage refresh: version banner, per-agent onboard picker (real `smash onboard --agent <yours> --write` commands), origin-story section, and the three demo terminals as alternating side-by-side rows.

### Fixed

- `smash semantic` status reported "Indexed memories: 16 of 6" — the index embeds every memory (archived included; they stay inert in default recall), so the denominator is now the total memory count, not active-only.
...
- Captured proposals are ranked by durability, so the one a one-click Accept saves (and `accept-capture --index 1`) is the substance, not a meta-preamble. A session that says "I want to set some conventions… From now on I only deploy on Fridays…" used to surface the vague "wants to set conventions" first (it classifies as a preference just as strongly); proposals now sort concrete directives ("I only deploy…", "always run…", "I prefer…") ahead of statements that are merely _about_ making rules. Ordering only — every proposal still appears for review.
- Onboarding reads cleaner: `smash onboard` on a fresh workspace collapses the dozen "created wiki/…" scaffold lines into one summary, and the README Quick Start is now a single two-command hero path (`smash proof` then `smash onboard --agent <yours> --write`) instead of three parallel entry points.
...
- Session captures are now reviewable and correctly attributed. Three fixes to how Smash reads a session: (1) standing rules survive long sessions — the hook mines a head+tail window instead of only the most recent ~6k characters, so "from now on I only deploy through the release script" said early in a two-hour session is still captured; (2) accept-time re-mining reads the user's own turns only (recorded in a `## Proposal Source` block), closing a path where the assistant's prose could be re-proposed as your preference; (3) every capture records a decision trail (`## How Smash Read This Session`) — messages kept, echoes dropped, proposals found — surfaced on the dashboard Captures page and in `capture-inbox --json` alongside the proposal previews and a "mined from your own turns" attribution flag.
- Session captures now carry retrieval context end to end: when the session-end hook proposes a memory, the neighboring sentences around the claim's origin travel with the proposal, through accept-capture, into the memory page's `context` frontmatter — the same ±1-neighbor window that lifted LoCoMo hit@10 0.685→0.737 in the retrieval benchmark. Context helps recall _find_ the memory (scoring + embeddings) but is never part of the claim: echo/duplicate/conflict checks, slim output, and the visible page body all exclude it. Also exposed explicitly as `smash remember --context` and the `context` parameter on both MCP remember tools; capped at 600 characters.

- The semantic and rerank tiers are now reachable from a Homebrew install: the Homebrew Python refuses direct pip installs (PEP 668), so every printed `pip install "smash-mcp[semantic]"` command failed on a fresh Mac. `smash semantic --setup` now detects an externally-managed runtime, provisions the extras into `~/.smash-mcp-venv` (pinned to Smash's version), and reruns the setup under that Python — one command from lexical-only to hybrid + rerank. Status and verify-mcp guidance stop printing pip commands the interpreter would refuse. The Homebrew `smash` shim (tap revision 1) runs through the managed venv whenever it hosts smash-mcp, so the CLI gains the tiers too.
- Upgrades no longer drift silently: session hooks run each workspace's own runtime copy, which used to stay old after `brew upgrade`. `smash health`/`status` now warn (`stale_runtime`) with the exact refresh command; `doctor`, `init`, `onboard`, and `connect --hooks --write` refresh the copy automatically. Newer workspace copies (dogfooded source checkouts) are never downgraded.
- CI now walks the cold install on a clean macOS runner and on Windows: onboard with hooks and MCP, self-provisioning of `~/.smash-mcp-venv`, the session-start brief, and a live MCP stdio handshake — after first asserting the runner has no `smash_mcp` importable, so a polluted environment can never mask install-path breakage again.
- Docs answer the three questions every newcomer asks — does Smash read my conversations, what survives a context clear, and which project a memory belongs to — and GitHub issue templates route bugs (with `smash doctor` output), friction reports, and feature requests.
- MCP now works out of the box: `smash connect --write` (and `smash onboard --agent ... --write`) verifies that the configured Python can actually serve `smash-mcp` at Smash's version before writing any agent config, reuses an existing `~/.smash-mcp-venv` when it matches, and provisions that venv automatically otherwise. A config that cannot start is never written; the chosen Python is persisted via the `.smash-mcp-python` marker and reported in the command output. Previously a Homebrew install wrote MCP configs pointing at a Python without the package, so the server failed silently in every agent.
- `smash verify-mcp <agent>` (for example `smash verify-mcp claude-code`) now reads the agent's actual config file and verifies the exact Smash server it is configured to run — Python, smash-mcp version, and wiki — instead of treating the agent name as a directory. When no Smash server is configured it points at the `smash connect ... --write` command.
...
- Added `--trigger` to `smash remember` and `trigger` to the MCP `remember`/`remember_memory` tools.
...
- Updated installed agent instructions, MCP instructions, SMASH.md, and docs so agents offer to save a recipe after notable multi-step work.
- Extended project seeding to architecture decision records: `smash seed` now also reads `docs/adr/`, `docs/decisions/`, and `adr/` files, and deterministically mines each ADR's Decision section into proposal-only `decision` memory candidates with paste-ready save commands — a repo's existing rationale becomes governed, recallable, supersedable decision memory only after the user approves each one.
- Made the safety layer visible: conflict refusals now lead with the paste-ready `--supersedes <name>` resolution; terminal recall shows applicability warnings ("out of context here — verify"), recipe triggers, and step previews; and `smash hook session-end --explain` prints the full decision trail (messages dropped as Smash's own output, proposals dropped as echoes or duplicates, trivial-session skips) so silent protections are inspectable.
- Split the semantic default by surface: short-lived CLI commands prefer the instant-load fast tier while the MCP server prefers the quality tier (explicit `SMASH_SEMANTIC_PROVIDER` always wins), so installing the quality extra no longer slows interactive commands.
- Added `smash connect --hooks-settings <path>` so hooks can be installed project-scoped (for example into a repo's `.claude/settings.json`), enabling per-workspace memory for multi-workspace users; `--supersedes` accepts a memory title as well as its name.
...
- Sharpened benchmark honesty: the hygiene baseline is labeled a governance ablation of Smash itself (with an open invitation to run the stream through other systems), the conflict-detector score is disclosed as a fit on its development set, and "zero junk" is defined precisely as zero self-inflicted junk.
- Added `smash recipes` to list saved procedure memories with their triggers, and `smash recall --type` to filter recall by memory type.
...
- Added supersedes chains: `smash remember --supersedes <name>` (and `supersedes` on the MCP remember tools) replaces an outdated memory atomically — the successor records `supersedes` lineage, the predecessor is archived with `superseded_by` and a supersession reason, and conflict refusals now point at this path instead of only offering coexistence overrides. `explain-memory` walks the full lineage chain in both directions.
- Added temporal recall: `smash recall --as-of YYYY-MM-DD` reconstructs what was active on a past date from existing lifecycle fields (capture, supersession/archive, expiry) — answering the temporal-memory frontier at personal scale with zero graph database.
...
- Added conditional memory: scope situational memories with `applies_when` conditions (`project:<slug>`, `path:<glob>`, `task:<phrase>`; OR semantics, validated at write time) via `smash remember --applies-when` and the MCP remember tools. Recall demotes out-of-context matches and labels every conditional memory with `applicability: matched|out_of_context` so agents never apply one project's conventions in another; startup briefs exclude out-of-context memories entirely. Session hooks evaluate `path:` conditions against the session's working directory. Research context: memory mis-scoping is a documented top failure mode of agent memory systems; Smash's conditions are deterministic frontmatter, not classifier guesses.
- Made "don't know" a first-class recall verdict: MCP recall now returns an `abstention` object (recommended when nothing matches or the best match is weak-confidence), the MCP instructions teach agents that "my memory doesn't cover that" is correct behavior rather than failure, and CLI recall warns above hint-grade results. Also added independent research context to `benchmarks/RESULTS.md`: recent controlled ablations find verbatim memory beats LLM-extracted artifacts by 15.9–22 points (arXiv:2601.00821) and ranking beats graph structure (arXiv:2603.15599) — external support for Smash's two most-questioned design choices.
...
- Fixed the biggest first-session friction: workspace-consuming commands run with no target in a directory that has no Smash wiki now fall back to the default workspace (`SMASH_WORKSPACE` or `~/Smash`) with a one-line notice, so `smash onboard` followed by a pathless `smash remember`/`smash recall` just works. Creator commands (`init`, `demo`, `try`, `proof`, `onboard`) never redirect, an explicit target always wins, and a wiki in the current directory still takes precedence.
- Fixed the plumbing leak in generated commands: when a `smash` launcher on PATH runs this same runtime (e.g. the Homebrew install), every generated command now says `smash ...` instead of the interpreter and Cellar path — including the viewer's copy-command buttons.
- `smash remember` without any workspace now points at `smash onboard` instead of a bare "missing wiki directory" dead end.
- The landing page now serves its actual pitch (headline, what Smash does, install command, links) to text fetchers, LLM crawlers, and noscript readers instead of "Unpacking..."; the hero install command gained `&& smash try` so the blessed first step is unmissable; and the Memory Dashboard explains what review means (unreviewed memories recall as provisional; reviewing earns full trust).
- Added a second animated demo to Getting Started (`docs/assets/smash-truth.svg`, self-contained SVG like its 1.6 sibling): the "memory that stays true" arc — a new memory conflicts with an old one, `--supersedes` replaces it with lineage, recall returns only the current truth, and `--as-of` answers what was true back then.
- `smash --help` now presents commands in seven task-shaped groups (Start here / Memory / Review & governance / Agents / Workspace / Sharing / Utilities) instead of a 60-command wall, and leads with `smash.py try`. A guard test keeps every registered command visible in exactly one group.
- `smash semantic` now reports the rerank tier's state (active / installed but model not fetched / not installed) with the exact next command, and `smash semantic --setup` fetches the rerank model alongside the embedding model when the extra is installed — previously installing `smash-mcp[rerank]` produced silently nothing because no setup path existed.
- Added an optional local rerank tier (`pip install "smash-mcp[rerank]"`): a tiny (0.08 GB) local ONNX cross-encoder re-orders the top recall candidates by reading each query-memory pair, blended with the retrieval order via reciprocal-rank fusion — never substituted, since pure reranking collapsed hit@1 in ablation. Measured on the default embedder: LoCoMo any-evidence hit@10 0.737 → 0.794 and multi-hop evidence recall 0.350 → 0.403; bundled-benchmark token-overlap hit@1 0.749 → 0.839 and pure-paraphrase hit@5 0.338 → 0.436. Applies only to explicit recall and MCP recall calls (~0.5 s at 50 candidates) — session hooks and briefs never pay the latency. Same local-first guarantees as the semantic tiers: offline-only model load, `SMASH_RERANK=off` to disable, `SMASH_RERANK_MODEL` to override.
- Embedding-model findings from the same study (documented in `benchmarks/RESULTS.md`): model rankings invert by text shape — nomic-embed-text-v1.5-Q beats the default on long conversational archives (LoCoMo hit@10 0.787 vs 0.737) but loses on Smash's claim-shaped memory pages (bundled hit@1 0.713 vs 0.749), so all-MiniLM-L6-v2 stays the default and nomic is the documented `SMASH_SEMANTIC_MODEL` alternative for conversational imports.
...
- Added an animated "aha" demo to the Getting Started page: a self-contained SVG (`docs/assets/smash-aha.svg`, plain text, no external runtime, animates in any browser) showing the two moments Smash is built for — recall that matches by meaning rather than keywords, and memory injected into a new agent session automatically. The README shows the matching recorded GIF (`docs/assets/smash-aha.gif`), rendered from real `smash` commands via a checked-in charmbracelet vhs tape (`docs/media/smash-aha.tape`) so it is reproducible, not synthetic.

- Fixed first-ten-minutes friction found by walking Smash cold as a brand-new user:
  - `smash onboard` now surfaces the automatic-memory path: it explains `--hooks` and prints the ready-to-run `--agent <hook-capable> --hooks --write` command, and each hook-capable agent preview offers "Make memory automatic (recommended)". Previously the flagship 1.6 feature was invisible in the guided setup.
  ...
  - Generated commands in source-checkout mode use a friendly `python3 smash.py` instead of the raw interpreter path (e.g. `python@3.14`); Homebrew users still see plain `smash`.
  - `smash proof` now says its workspace is a throwaway demo and points to `smash onboard` for real memory, with a plain "what this means for you" line.
  - `scripts/prepare_release.py` reminds maintainers to bump the Homebrew tap so `brew install` never serves an older Smash than the docs describe.

- Completed 1.6 coverage across the second-tier docs and shipped skills: the official CLI skills now teach the hooks-installed rule, the consolidation pass, semantic match labels, and the `smash semantic` status check; the memory contract documents the hooked loop and honest recall signals; concepts covers hybrid retrieval and the automatic lifecycle; troubleshooting gains "hooks not firing" and "semantic recall not working" sections; and the scale page links the measured benchmarks.

### Added

- Added `smash connect <agent> --hooks` to install agent session hooks alongside MCP config for Claude Code, Codex, and Cursor: every new session starts with a bounded Smash memory brief injected automatically, and session end stores proposal-only session notes with memory candidates, so the memory loop no longer depends on the agent remembering to call Smash. Codex has no session-end hook event, so it gets the session-start brief only; Cursor uses its flat `hooks.json` schema and JSON `additional_context` envelope.
- Added `smash consolidate` and MCP `review(action="consolidate")` for a read-only backlog plan: pending capture counts, memories needing review, duplicate-capture groups, and paste-safe accept/discard/review commands — nothing is merged, deleted, or saved without the user approving each action.
...
- Added optional hybrid semantic recall (`pip install "smash-mcp[semantic]"` + `smash semantic --setup`): a small local static-embedding model retrieves close paraphrases that token matching misses, across CLI recall, memory briefs, MCP recall, and smart query packets. Lexical recall stays the default and the fallback.
- Kept the local-first guarantee for semantic recall: the model loads offline-only so a query can never trigger a download (only the explicit `--setup` may fetch the model once), embeddings live in plain JSON under `.smash-cache/`, similarity is computed in-process with no vector database or service, and `SMASH_SEMANTIC=off` disables the layer.
...
- Added `smash semantic` for the layer's status (provider, model, index state, mode) with explicit setup/rebuild actions and next-step guidance.
...
- Improved `smash semantic` diagnostics: status names the Python interpreter being checked, and when the Smash MCP Python differs, errors print the exact venv-side setup command; quality-tier setup states the ~5s short-lived-CLI load tradeoff explicitly.
...
- Added `smash hook session-start` to print the bounded session-start memory brief (readiness, relevant memories with confidence, pending review and capture state, and retrieval guidance) for agent hook runtimes; it scopes the brief to the hook's working directory project and never fails the agent session.
- Added `smash hook session-end` to turn an agent transcript into review-gated memory: it extracts bounded user/assistant text (skipping tool calls and outputs), skips trivial sessions, and stores proposal-only session notes through the same duplicate/conflict-safe capture path as `smash session-end`.
- Added idempotent, non-destructive session-hook writing to `~/.claude/settings.json` that preserves existing user hooks and settings, replaces only Smash's own hook entries on rerun, and skips re-injection on session resume.

### Fixed

- `python -m smash_mcp --help` now prints usage and the MCP config snippet instead of silently starting the stdio server (which hung in a terminal with no output). The parser still ignores unknown arguments so an agent launch config can never crash the server.
...
- Added weak-match guidance to recall packets and CLI recall output: when every matched memory is a weak lexical match, Smash now says so and tells the agent to verify with the user instead of presenting it as a known preference.
- Added `smash session-end` (alias `smash end`) as the agent-agnostic end-of-session lifecycle command: it stores proposal-only session notes, returns a small set of memory candidates, and refuses to create durable memory without user approval.
...
- Added `smash seed [project-dir] [target]` to create a source-backed project context page from allowlisted repo files such as `README.md`, `AGENTS.md`, `CLAUDE.md`, `.cursorrules`, and editor rule files, with secret scanning and no silent durable-memory writes.
- Added `smash onboard --seed-project [dir]` so first-run setup can create the initial source-backed project context page without making users discover a separate command.
- Added benchmark value evidence: `smash benchmark` now estimates broad wiki body text versus the bounded Smash query packet so users can see concrete context-budget savings alongside speed and scale checks.
...
- Added MCP prompt and resource coverage so clients can expose Smash recall, remember, ingest, and review workflows as native agent actions instead of requiring users to memorize tool names.
- Added MCP `link_start` and `Smash://instructions` so clients can attach the portable startup loop: check readiness, run one empty-query recall brief, then use bounded recall before broad context reads.
...
- Added `smash onboard` to create or repair a real `~/Smash` workspace, check health, optionally seed a first memory, preview or write agent MCP config, and print first prompts in one guided flow.
- Added `smash proof`, a clean cross-agent continuity demo that creates a local proof workspace, writes one reviewed memory, and recalls it through the same bounded path used by CLI, skills, and MCP.
...
- Added `smash operations --recover <marker> --confirm` so leftover crash snapshots from interrupted writes can be previewed and applied instead of becoming dead local state.
...
- Added cache-backed backlink rebuild logic so rebuilds reuse parsed page data while preserving existing body-only/full-Smash behavior.
...
- Redesigned the local web console to the Smash design system: cream/ink/rust editorial palette with a warm-dark theme (never pure black), serif headings with mono labels on system font stacks, a tab-strip nav, ledger memory cards, health status cards, and a segment-meter confidence indicator on recalled memories (weak matches carry a 'verify before trusting' note).

- Restored the designed animated landing page on GitHub Pages (live memory-graph hero) and refreshed its content for the slim MCP surface: canonical six tools, `smash proof`/`smash try`/`smash start`/`smash onboard` commands, and recall confidence labels.
...
- Added `smash start`, a CLI startup loop that combines readiness, validation state, and a local memory brief for agents using skills or shell instead of MCP.
...
- Made the slim MCP surface the default server surface and aligned README, SMASH.md, installed agent instructions, MCP docs, status actions, and query follow-ups around the canonical `status`, `recall`, `remember`, `ingest`, `review`, and `admin` vocabulary.
- Reworded starter prompts around the clearer `start with Smash before we continue` flow so first-run users ask for the same startup recall loop exposed by MCP `link_start`.
...
- Tightened `smash onboard --write` guidance so config-writing is explicit, repeatable, and clear about which agent files are affected.
- Improved `smash serve` startup guidance so the terminal points new users to `/onboard`, `/health`, `/graph`, and clearly states that MCP and CLI work without the viewer running.
...
- Tightened README and public docs onboarding around a proof-first flow: `smash proof` for the core memory aha, `smash try` for the richer demo, and `smash onboard` for real setup.
- Improved `smash try` human-readable output so the first-run proof reads like a product moment instead of a debug checklist.
...
- Updated the local `/onboard` checklist to surface project context seeding before first memory and agent wiring, matching the new `smash onboard --seed-project .` path.
...
- Updated `smash start`, MCP `link_start`, and slim MCP `admin` so empty project recall points agents toward source-backed project seeding before broad file reads.
- Updated `smash start` to include a tiny token-bounded context preview from the same hybrid query packet used by CLI, skills, and MCP once source-backed project context exists.
...
- Fixed direct `serve.py` argument parsing so a positional target now fails with guidance instead of silently serving the wrong wiki root; use `--root` directly or `smash serve <target>`.
...
- Fixed first-run `init` output to print the installed `smash` command instead of the collision-prone `Smash` command.

## [1.4.0] - 2026-06-14

### Added

- Added official CLI skills under `skills/` so agents can lazy-load Smash workflows without MCP setup.
- Added `smash try` as a one-command demo proof loop that creates the demo, checks readiness, runs query/brief examples, and prints first agent prompts.
- Added `smash connect <agent>` to preview or write MCP client config for Codex, Kiro, Claude Code, Cursor, Antigravity, VS Code, and Copilot.
...
- Added `smash import-obsidian <vault>` to copy Obsidian Markdown notes into `raw/obsidian/` with secret scanning before the normal ingest workflow.
- Added `smash compliance-export` for redacted readiness, validation, memory-review, operation, and log exports for team or security review.
- Added `smash restore-backup` to preview and confirm local backup restores with unsafe-tar checks, raw restore opt-in, and pre-restore safety backups.
- Added `smash team-sync` to print a safe Git sharing plan for reviewed team memory without pushing private raw sources automatically.
- Added `smash share <page-or-memory>` to print a local viewer permalink and agent prompt for a specific Smash page.
- Added `smash snapshot` to export a static, read-only HTML snapshot for demos or reviews while excluding raw sources, captures, live state, and memory pages by default.
...
- Added `smash set-memory-visibility` and MCP `set_memory_visibility` so existing memories can move between private, project, and team sharing intent after explicit user approval.
- Added `smash memory-log`, MCP `memory_log`, `/memory-log`, and `/api/memory-log` for recent memory lifecycle changes without exposing raw source or memory bodies.
...
- Added `smash wins`, MCP `memory_wins`, `/wins`, and `/api/wins` for local, non-telemetry proof signals about what Smash memory is carrying.
...
- Added `python -m smash_mcp --version` so MCP package installs can be verified before a wiki exists.
- Added an Obsidian guide for opening Smash's Markdown wiki as a vault and rebuilding indexes after manual edits.
...
- Changed the installed CLI command from `Smash` to `smash` to avoid the POSIX/macOS `Smash` utility collision while preserving source-checkout `python3 smash.py ...` usage.
- Tightened `smash team-sync` readiness so unreviewed memories or active `visibility: private` memories block "ready" status before Git sharing.
- Tightened `smash snapshot --include-memories` so private memories stay excluded unless `--include-private-memories` is explicitly passed.
- Broadened Windows CI from a small portability subset to most non-installer/non-server tests.
- Clarified that the Homebrew formula lives in the separate `ebrahimisoheil/homebrew-Smash` tap.
...
- Added copyable agent prompts across page, search, ingest, brief, memory dashboard, profile, audit, capture, and inbox views so browser-first users do not need to memorize Smash phrasing.
- Added copy buttons to memory action commands, capture commands, and memory next actions in the local web UI.
- Added empty-wiki home recovery actions that Smash to ingest and copy the first ingest prompt.
- Added graph empty-state recovery actions that Smash to ingest and copy the first ingest prompt.
- Added empty memory-brief recovery actions that Smash to ingest/proposal review and copy a proposal prompt.
- Added empty memory-profile recovery actions that Smash to ingest/proposal review and copy a remember prompt.
...
- Added `Smash next` as a short alias for starter prompts so first-run users have one memorable next-step command.
- Added the `Smash next` prompt command to demo output so the first terminal screen points to the agent-first workflow.
- Added the `Smash next` shortcut to starter prompt payloads and the local prompts page.
...
- Updated installer and agent-instruction next steps to point at `Smash next`.
- Simplified quick-start examples around `Smash next`, with `Smash welcome` kept as the optional guided proof path.
...
- Added `Smash health` as a single CLI readiness command combining status validation with interrupted-write state.
...
- Added `Smash version` as a discoverable command alias for `Smash --version`.
...
- Updated CLI query no-context recovery commands to include the explicit Smash root so they work from any terminal directory.
- Updated starter prompt local-check commands to include the explicit Smash root so CLI, web, and MCP prompt payloads are paste-safe outside the wiki directory.
- Updated operation recovery commands to include the explicit Smash root for interrupted-write repair flows.
- Updated health page repair and next-action commands to include the explicit Smash root where the page can infer it.
- Updated starter prompts, installers, ingest post-checks, and quick-start docs to use `Smash health` as the primary readiness command.
- Updated init and demo next-step commands so setup guidance remains paste-safe outside the current checkout directory.
- Updated ingest guidance commands and post-checks to include the explicit Smash root across CLI, JSON, web, and MCP payloads.
- Updated rebuild-index follow-up guidance to include the concrete `smash.py` runtime and Smash root for the next backlinks repair step.
...
- Updated `Smash status` text output to pair MCP-style next actions with concrete local commands for terminal users.
- Updated the health page memory-review fallback to copy a target-aware `Smash memory-inbox` command.
- Updated `smash-mcp` missing-wiki startup guidance to point at `Smash init`, source checkout init, integration installers, or `--wiki`.
...
- Makes Smash easier to install and try as a real product: Homebrew tap support, GitHub Pages docs, cleaner README/product positioning, and visual walkthrough assets.
- Tightens external-user workflows across CLI, MCP, and the local web UI so copied commands work from any terminal directory and agents get explicit Smash targets.
- Adds clearer local health and recovery surfaces with `/health`, `Smash operations`, interrupted-write inspection, and bounded log rotation.
...
- Added tap-ready Homebrew Formula packaging and maintainer instructions for publishing `ebrahimisoheil/homebrew-Smash`.
...
- Added product-facing demo visuals for UI, CLI, and MCP flows so new users can understand Smash before installing it.
...
- Added `Smash operations`, `/api/operations`, and operation markers so interrupted or failed local writes can be inspected before manual cleanup.
...
- Updated generated CLI guidance in memory, capture, web, and MCP payloads to include explicit Smash root targets instead of relying on `.`.
...
- Updated local web audit and capture commands to point at the served Smash root.
...
- Fixed capture accept/delete/review follow-up commands that assumed the terminal was already inside the Smash root.
- Fixed memory review/update/archive/restore/forget follow-up commands that assumed the terminal was already inside the Smash root.

## [1.1.0] - 2026-05-08

### Highlights

- Reframes Smash as local personal memory for agents, with the Markdown wiki as the inspectable storage layer.
- Adds the first-use path around `Smash init`, `Smash serve`, the managed `Smash` command, demo proof prompts, and readiness checks.
...
- Added Memory Mode foundation with `wiki/memories/`, `smash.py remember`, `smash.py recall`, and MCP `remember_memory`/`recall_memory` tools.
- Added a first-run demo memory page so Smash presents as local agent memory, not only a wiki.
- Added Memory Profile views through `smash.py profile`, MCP `memory_profile`, `/profile`, and `/api/memory-profile`.
...
- Hardened `smash.py doctor` secret-content checks so unreadable scannable files fail closed instead of being skipped.
...
- Added shared atomic write helpers and migrated Smash state writes for schema markers, memory pages, indexes, backlinks, captures, raw source creation, logs, and demo files.
...
- Reused cached forward-Smash data during context retrieval to avoid an extra primary-page disk read.
...
- Added search backend reporting to Smash status payloads so agents and users can see whether local search is using SQLite FTS or the token fallback.
- Added `smash.py status` so the same readiness summary is available before MCP or the local web server is connected.
- Added `smash.py status --validate` to installer next-step output so new users have one readiness command after setup.
- Added `content_page_count` to Smash status and first-run guidance for structurally ready but empty wikis.
- Added status warnings so cache or memory-read degradation is visible in CLI, HTTP, and MCP readiness payloads.
- Added shared Markdown renderer coverage under `smash_core.markdown` so the local web UI's sanitized Markdown behavior is tested outside the HTTP monolith.
...
- Added a `Why Smash?` product page that explains where Smash fits versus human-first notes apps, hosted memory APIs, stateful-agent runtimes, temporal graph memory systems, and plain file search.
...
- Moved bundled demo wiki content into `smash_core.demo` so the CLI module no longer carries the full demo payload inline.
- Moved the local web UI CSS/JavaScript assets into `smash_core.web_assets` so `serve.py` stays focused on routing and rendering.
- Moved memory and raw-capture card rendering into `smash_core.web_memory` so memory UI escaping and actions are covered outside the HTTP server.
- Moved the shared local web layout shell into `smash_core.web_layout` so header/nav/theme/search behavior is tested outside the HTTP server.
- Moved local HTTP guard parsing and Host validation into `smash_core.web_http` with isolated tests.
- Moved local viewer security header policy into `smash_core.web_http` so browser hardening stays core-tested outside the HTTP server.
- Moved graph payload, category, and legend helpers into `smash_core.web_graph` so graph-scale behavior is tested outside the HTTP monolith.
- Added a managed `~/.local/bin/Smash` command for global installs so users can run `Smash status --validate`, `Smash query`, and `Smash brief` without remembering wiki paths.
- Added a shared Smash runtime version and `Smash --version`; CLI and local HTTP status now report the same release version as the package.
- Switched MCP status version reporting to the shared Smash runtime version so source checkouts and installed packages cannot drift.
- Added `Smash init` to create or repair a normal Smash wiki without loading demo content.
- Added `Smash serve` to start the local web viewer without remembering `serve.py` paths.
- Made `Smash verify-mcp` require the installed `smash-mcp` version to match the local Smash runtime before reporting ready.
- Made `Smash verify-mcp` print shell-quoted install and upgrade commands using the exact Python executable being verified.
- Made `Smash verify-mcp` import-check the MCP SDK dependency so broken partial installs no longer report ready.
- Made `Smash verify-mcp --json` return structured issue codes and repair actions for agent/tooling consumers.
- Improved local server startup errors with bounded port validation in both `Smash serve` and `serve.py`, plus clear next-port guidance when a port is already in use.
- Added `Smash benchmark` to measure local cache, search, smart query, and graph timings on a user's current wiki.
- Extended `Smash benchmark` and large-wiki smoke to prove bounded agent payload timings for graph summaries and page lists.
- Extended `Smash benchmark` and large-wiki smoke to prove the graph page's initial browser payload stays bounded on huge wikis.
- Added an ignored `.smash-cache/` persistent page-record cache so unchanged large wikis can warm search/context indexes without rereading every Markdown page.
- Extended the first-use smoke to run `Smash graph-summary` and `Smash benchmark` so the demo value loop is release-gated.
- Made the local graph viewer start with a bounded overview for very large wikis, with an explicit full-graph load control.
- Hardened local write APIs by rejecting browser `Origin`/`Referer` headers that do not point at the local Smash viewer.
...
- Added an interactive-readiness verdict and threshold warnings to `Smash benchmark` so larger local wikis are easier to evaluate.
...
- Added wiki schema markers with safe `Smash migrate`/MCP `migrate_wiki` migrations for future local format changes.
- Added first-run agent prompts to installer output so new users can immediately try brief, remember, and query workflows.
- Added `Smash prompts` to print the first-run natural agent prompts and local readiness checks on demand.
...
- Added guided `Smash ingest-status` output with structured JSON guidance, exact agent prompts, and follow-up validation commands.
...
- Added ingest completion cards that show which raw files are represented, Smash to their source pages, and provide copyable memory/query prompts for post-ingest validation.
- Added the same represented-source completion summary to `Smash ingest-status` for terminal-first users.
- Added MCP `ingest_status` so MCP-only agents can inspect pending raw files and validation guidance.
- Added `Smash rebuild-index`, MCP `rebuild_index`, and `POST /api/rebuild-index` to regenerate the human-readable wiki catalog from current pages.
...
- Added bounded backlink/page-Smash payloads for MCP and HTTP so hub pages do not flood agent context.
...
- Added agent memory briefs with `smash.py brief` and MCP `memory_brief` so agents can prime themselves with relevant local memory before a task.
- Added smart Smash query packets with `smash.py query`, MCP `query_link`, and `/api/query-Smash` so agents can retrieve budgeted memory, ranked wiki results, and graph context without reading the whole wiki.
...
- Added provenance metadata to smart query memory and wiki packets so agents can explain why Smash knows something without loading full pages.
...
- Added `smash.py validate` as an ingest gate for agent-generated wiki pages, covering required frontmatter, type/directory alignment, required sections, dead links, and stale backlinks.
...
- Added memory review and raw capture backlog checks to `smash.py doctor`, while excluding proposal-only raw captures from ingest-status pending source counts.
...
- Added `smash.py capture-session` to save long session notes under `raw/memory-captures/` and return proposal-only memory candidates for human approval.
...
- Added `smash.py accept-capture` to turn an approved raw-capture proposal into a durable memory through duplicate/conflict-safe writes.
- Added MCP `accept_capture` for approving saved capture proposals through the same duplicate/conflict-safe workflow.
- Added `smash.py redact-capture` to replace secret-looking values in saved raw captures while logging only warning labels and counts.
- Added MCP `redact_capture` so agents can redact saved raw captures after user approval.
- Added `smash.py delete-capture` with explicit confirmation for removing saved raw captures without logging capture contents.
- Added MCP `delete_capture` with explicit confirmation for removing saved raw captures.
- Added `smash.py capture-inbox` to list saved raw captures, secret warnings, and accept/redact/delete commands.
...
- Added a real MCP stdio smoke test for the built `smash-mcp` wheel in CI.
...
- Blocked normal ingest guidance for raw files Smash cannot read and safety-scan, with explicit CLI, HTTP, and MCP payload diagnostics.
...
- Added benchmark health summaries and recommendations so `Smash benchmark` produces clearer proof-of-readiness output.
...
- Clarified README guidance for `Smash verify-mcp`, including version parity, MCP SDK dependency, wiki path, and config checks.
...
- Added `Smash prompts` coverage to the first-use smoke so CI validates the first-run agent prompt path.
...
- Hardened release hygiene so `server.json` cannot silently lose the `smash-mcp` package version entry.
...
- Extracted shared memory proposal logic into `smash_core` so CLI, HTTP, and MCP proposal behavior stays aligned.
- Extracted shared raw capture path resolution and notes parsing into `smash_core` so CLI and MCP capture operations use the same root-escape guard.
- Extracted shared frontmatter parsing and typed update helpers into `smash_core` for safer memory metadata writes.
- Extracted shared memory record loading, review inbox, profile, and recall helpers into `smash_core`.
- Extracted shared memory resolution, log lookup, and recall-state helpers into `smash_core`.
- Extracted shared memory lifecycle mutations for archive, restore, review, and update workflows into `smash_core`.
- Extracted shared memory creation for `remember` and `remember_memory` into `smash_core`.
- Extracted shared wiki indexing, search, context, graph, and backlink helpers into `smash_core`.
- Extracted shared search ranking and optional SQLite FTS helpers into `smash_core.search` so wiki indexing stays separate from search execution.
- Extracted shared memory explanation/audit payloads into `smash_core`.

### Fixed

- Tightened README onboarding and release examples around Smash's local memory product value.
- Simplified onboarding docs and installed instructions around natural agent prompts and the short `Smash` command instead of path-heavy maintenance commands.
...
- Fixed installer MCP setup reporting so failed upgrades no longer masquerade as success by reusing an unrelated older global `smash-mcp`.
- Fixed project-mode installer output so MCP wiki paths are absolute and next-step hints point at the project wiki instead of `~/Smash`.
...
- Hardened HTTP memory mutation endpoints with an explicit `X-Smash-Local-Action: true` header required by non-UI clients.
- Refreshed the checked-in demo backlink index so `smash.py doctor .` reports a healthy graph.
...
- Fixed `smash.py verify-mcp` to use the installer-recorded MCP Python when present.
...
- Added `smash.py ingest-status` to show pending raw sources and stale graph indexes.
- Added `smash.py doctor --fix` for safe structure creation and backlink repair.
- Added `smash.py verify-mcp` to validate local MCP readiness and print client config.
- Added first 10 minutes onboarding docs.
- Added golden demo snapshot tests and direct MCP contract tests.

### Changed

- Moved raw capture inbox parsing, project filtering, snippet redaction, and command generation into shared `smash_core.capture` helpers.
...
- Hardened installers to avoid silently using `--break-system-packages`; they now fall back to `~/.smash-mcp-venv` and register MCP with the resolved Python.
...
- Hardened `smash-mcp` tool inputs for empty queries/topics and invalid search limits.
...
- Added `smash.py demo` for a pre-ingested sample wiki.
- Added `smash.py doctor` health checks for structure, backlinks, source hygiene, graph integrity, and secret-looking files.
- Added CI release gates for tests, demo health, installer syntax, package build, version consistency, and release hygiene.

### Changed

- Published `smash-mcp` 1.0.5 package metadata for PyPI and the MCP Registry.
...
- Fixed MCP package reinstall behavior so rerunning installers upgrades `smash-mcp`.
